...
# 🔴 全部用 ${VAR:-default} 而不是硬設 —— 呼叫端（例如 check_so_error.sh）
#    需要把某一個端點指到別的地方時，硬設會**靜默蓋掉**外部給的值，
#    而症狀是「測試跑了但測的是別的東西」。2026-08-30 實際踩到：
#    check_so_error 想指向沒人監聽的埠，卻被這裡蓋回 15002，於是連得上、
#    一次 reconnect failed 都沒有 —— 幸好那支腳本有「零筆資料不算通過」的守衛。
#
# 依二進位名稱決定要跑哪一支 —— 吊機用 :5002 與另一組端點。
if [[ "$BIN" == *crane* ]]; then
  CMD_PORT=5002
  export FCV_EP_USR_A_HOST="${FCV_EP_USR_A_HOST:-127.0.0.1}"  FCV_EP_USR_A_PORT="${FCV_EP_USR_A_PORT:-15030}"
  export FCV_EP_USR_B_HOST="${FCV_EP_USR_B_HOST:-127.0.0.1}"  FCV_EP_USR_B_PORT="${FCV_EP_USR_B_PORT:-15031}"
  export FCV_EP_USR_M_HOST="${FCV_EP_USR_M_HOST:-127.0.0.1}"  FCV_EP_USR_M_PORT="${FCV_EP_USR_M_PORT:-15034}"
  export FCV_EP_DSZL_L_HOST="${FCV_EP_DSZL_L_HOST:-127.0.0.1}" FCV_EP_DSZL_L_PORT="${FCV_EP_DSZL_L_PORT:-15032}"
  export FCV_EP_DSZL_R_HOST="${FCV_EP_DSZL_R_HOST:-127.0.0.1}" FCV_EP_DSZL_R_PORT="${FCV_EP_DSZL_R_PORT:-15033}"
else
  CMD_PORT=5001
fi

export FCV_EP_USR20_HOST=127.0.0.1  FCV_EP_USR20_PORT=15020
export FCV_EP_USR21_HOST=127.0.0.1  FCV_EP_USR21_PORT=15021   # 2026-09-03 QX-DO24 專用
