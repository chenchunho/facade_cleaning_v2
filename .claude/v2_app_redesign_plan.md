...
- 吸盤：4 顆，**推桿 slave 1,2=左 / 3,4=右**（單顆機構同 v1：ZDT/SMC 推桿 + 真空 + JC100）
- 真空：**2 區**，左閥（1,2）/ 右閥（3,4）。**無中心吸盤**
- 位移：**單側吊機繩放/收 + SD76 計米量測**（無 DM2J）
- 水平：**IMU roll/pitch + 左右計米繩長差**，兩者都用
- 橫向：**無**（只垂直上下）
...
### do_step_down_（左半 + 右半；前提：4 杯吸附、水平）
```
左半週期：
...
### do_step_up_
對稱：第 3/7 步改 `crane_cmd_("retract_left/right <Δ>")` 把該側往上拉。其餘同 down。
