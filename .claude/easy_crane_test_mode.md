...
| 檔案 | 原值 | 測試模式值 | 還原 |
|---|---|---|---|
| `user_lib/WASH_ROBOT.h:104` `CRANE_IP` | `"192.168.1.101"` | `"127.0.0.1"`（shim 與 washrobot 同機 .5.19，2026-04-24 更新）| **改回 `"192.168.1.101"`** |
| `user_lib/WASH_ROBOT.h:147` `WATCHDOG_TIMEOUT_MS` | `2000` | `60000` | **改回 2000** |
| `user_lib/WASH_ROBOT.cpp` init() 開頭 `bool dbg = true;` | `false`（2026-04-21c 原）/ `true`（2026-04-21c~04-24p 階段）| `true`（env var 可改）| **改回 `bool dbg = false`**；env var 機制保留 |
| `Crane_easy_PI/main.cpp:319,320` relay / dy500 init `debug` | `false` | `true` | **改回 false** |
