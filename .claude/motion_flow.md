...
> ---
> ⚠️ **2026-08-27 標註：本文件描述的是 v1，已與現行程式碼脫節。**
>
> §2 的硬體表仍寫 `DM2J×5` / `ZDT×9`，但 v2 的腳早就沒有 DM2J 滑軌、吸盤是 4 顆、
> 位移改由吊機繩＋SD76 計米完成。**接手時不要照本檔的硬體表配置設備。**
>
> | 世代 | 權威文件 | 對應 |
> |---|---|---|
> | v1 | **本檔** | 已凍結，保留作為尺蠖步態與狀態機設計的原始記載 |
> | v2 | `.claude/v2_app_redesign_plan.md` | ✅ **現行程式碼**（4 吸盤／2 區真空／吊機繩位移／無 DM2J） |
> | v3 | `.claude/洗窗機器人設計彙整.md` | 🆕 新架構方向（四輪滾動＋螺旋槳貼牆＋橫向滑台，沿用既有硬體改寫）|
>
> 📌 本檔**不再更新**。要改運動規格請動 v2／v3 那兩份。
> 🔴 但**不要刪**：狀態機、phase 切分、指令協定、參數常數的推導過程只有這裡有，
> v2/v3 兩份都建立在它之上。
> ---
...
### Crane 端硬體拓樸（Crane RPi 192.168.1.101，2026-05-15 re-layout）

**控制 bus：USR-TCP232-304 #A @ 192.168.1.30**（所有「寫 / 命令」）

| Slave | 裝置 | 用途 |
|---|---|---|
| 1 | SE3 變頻器 #1 | 左鋼索（士林 SE3-210，取代原 ZS_DIO 繼電器 2026-05-07）|
| 2 | SE3 變頻器 #2 | 右鋼索（2026-05-15 從 USR_B 移到 USR_A、slave 1→2）|
| 3 | CLV900 變頻器 | 中間絞盤（未安裝，預留）|
| 4 | SD76 #3 (reserved) | 中間管線計米（未安裝，預留 ）|

**感測 bus：USR-TCP232-304 #B @ 192.168.1.31**（所有「讀 / 長度回授」）

| Slave | 裝置 | 用途 |
|---|---|---|
| 1 | SD76 #1 | 左鋼索計米（2026-05-15 從 USR_A 移到 USR_B、slave 2→1）|
| 2 | SD76 #2 | 右鋼索計米 |

**張力感測（直插 switch，獨佔 Modbus TCP :502）**

| IP | 裝置 | 用途 |
|---|---|---|
| 192.168.1.32 | DSZL-107 #1（X518）| 左鋼索張力 |
| 192.168.1.33 | DSZL-107 #2（X518）| 右鋼索張力 |

**拓樸理由：** 控制 bus / 感測 bus 物理分離 — meter_loop 輪詢 SD76 不會撞到 SE3 dispatch（修掉 2026-05-15 看到的 200-300ms drift 問題）。Trade-off：雙 SE3 共一條 RS485 bus → Modbus RTU half-duplex 序列化（drift floor ~30-50ms）。
...
```
頂樓水源 ──(水管，經中間管線)──▶ 進水球閥 ──▶ 10L 水箱 ──▶ CH6 泵浦 ──▶ 機械臂噴頭 ──▶ 牆面 ──▶ 自然流下至地面
                                  ↑
                                  2026-06-05 起搬到 crane PQW (.34 slave 12 CH4)
                                  washrobot 透過 crane_cmd_("water_inlet on/off") 控制
                                  原 washrobot PQW CH7 空著不接
```
- 水箱溢流處理（浮球/溢流孔）待確認 → Open Q10
- 清洗時進水球閥持續 ON 補水、CH6 持續 ON 出水

---

## 3. PQW 繼電器接線（本體）

> 🔴 **2026-09-01 全面更正。** 本節原本畫的是 **v1 接線**（CH1=泵浦／CH2=腳組閥／
> CH3=身體組／CH4=中心／CH6=水泵／CH7=進水閥，位置寫 `.22`），**每一條都已過時**。
> 期間發生的搬動：06-05 進水閥移交吊機、07-07 四吸盤改線、08-27 單閥化＋PQW 搬到 `.20`
> ＋水泵讓位、08-29 滾筒改回 CH5。**權威版是 `app/WASH_ROBOT.h` 的 `CH_*` 常數**，
> 本表與之同步；接線前請以該檔為準。

**位置：`192.168.1.20`（`cli_20_`）slave 12，實體 16CH**（`PQW_TOTAL_CH = 16`）。
與 ZDT 推桿 5~8 共用 `.20` 匯流排。

| CH | 用途 | 常數 | 現況 |
|---|---|---|---|
| **1** | **真空閥 VT307** —— 唯一一顆，同時控全部 4 顆吸盤 | `CH_VALVE_RIGHT`（`CH_VALVE_LEFT` 亦指向此） | 使用中 |
| **2** | **真空幫浦 A 組** dp0105 —— `init` 開啟，運行期間常開 | `CH_PUMP` / `CH_PUMP_A` | 使用中 |
| **3** | **真空幫浦 B 組** | `CH_PUMP_B` | 🔴 **實體存在但程式未啟用**（per user：先用 A 組，B 之後再規劃） |
| 4 | 空 —— v1 中心吸盤閥；v2 無中心吸盤 | — | 未接（實測確認） |
| **5** | **上滑台機械手臂清潔滾筒** | `CH_BRUSH` | 使用中 |
| **6** | **正壓閥** —— 脫離時除了關真空閥回大氣壓，另主動灌正壓加速脫離。**4 顆吸盤共用**，通電 500 ms | `CH_BREAK_VACUUM` | 使用中 |
| 7 | 空 —— 原進水球閥，2026-06-05 移交吊機 PQW（`.34` slave 12 CH4） | — | 未接（實測確認） |
| 8 | 空 | — | 未接（實測確認） |
| 9~13 | 空 | — | — |
| **14** | **水箱泵浦** | `CH_WATER_PUMP` | 已設定，**實體尚未接管路** |
| 15~16 | 空 | — | — |

> ✅ **CH1~CH8 於 2026-09-01 逐一通電實測**（`relay <ch> on|off` 送出 + `relay_status`
> 回讀，使用者現場回報實體反應）。8 格裡 **6 格與程式一致、1 格語意不足（CH6）、
> 1 格記載錯誤（CH3）**。CH9~CH16 未逐一實測，仍為程式記載。

```
[電磁閥邏輯]
  CH1 = OFF → VT307 不通 → 吸盤對大氣（無吸力）
  CH1 = ON  → VT307 通   → 吸盤接負壓（吸附）
  CH6 = ON  → 對吸盤灌氣，主動破壞密封（收推桿前用）
```

### 🔴 四件動這張表之前必須知道的事

**⓪ CH3 是真空幫浦 B 組，不是空通道 —— 而且它從來沒被啟動過。**
本檔與 `WASH_ROBOT.h` 原本都把 CH3 記成「空（原左腳閥，08-27 單閥化後讓出）」，
2026-09-01 逐一通電實測推翻。實際後果有兩層：
- **真空系統只有一半在運轉** —— `init` 只開 `CH_PUMP_A`(CH2)。📌 這很可能與吸盤密封
  長期要靠 `smart_extend_subset_` 反覆補伸才吸得住有關，在查明前不要再直接歸因於機構。
- 🔴 **`WASH_ROBOT.h` 曾埋著一句地雷**：「若之後又改回兩顆獨立閥，只要把 `CH_VALVE_LEFT`
  改回 3」——照做會讓二十幾處閥呼叫去驅動幫浦 B 組，症狀是「閥指令回 OK、實際在開關幫浦」。
  該句已刪除。要改回雙閥請用 CH4 / CH7 / CH8（實測為空）。


**① CH6（破真空）與 CH14（水泵）絕不可同號。**
2026-08-27 把水泵從 CH6 挪到 CH14 是**強制讓位、不是整理**：清洗流程的滾筒段會主動
`pqw_set_relay_verified_(CH_WATER_PUMP, true)`。水泵若仍指向 CH6，**清洗時開水泵 ＝ 開破真空
＝ 4 顆吸盤同時失去真空 → 機器在貼牆狀態下脫落。**

**② 單閥化的安全後果：「只放開單側」在硬體上已不可能。**
`do_step_down_` / `do_step_up_` / `do_cross_obstacle_` 的核心前提是「一側解真空、另一側
維持吸附當防墜錨點」。CH1 一開就是 4 顆一起失去真空，**那個前提直接破掉**。GUI 入口已於
2026-08-26 移除，但後端指令仍在 —— 🔴 **不要用 raw command 呼叫 `step_down` / `step_up` /
`cross_obstacle_*`**（`run` 的 `alt` gait 已在程式內擋掉，回 `ERR alt_gait_disabled_single_valve`）。
v2 正式走法 `do_step_sync_` 本來就是 4 顆同放同吸，不受影響。

**③ CH5 曾被誤記成 CH15 一個多月。**
2026-07-24 改成 15，打到沒接東西的繼電器，現象是「清洗流程照跑但滾筒不轉」，而當時
`do_step_sync_rail_sweep_` 沒檢查回傳值，log 完全看不出來。2026-08-29 實體確認後改回 5。
📌 **教訓：繼電器改號一定要有實體確認，而且呼叫端要檢查回傳值。**

### ⚠️ 目前沒有繼電器狀態回讀指令

driver 有 `readAllStatus()`（Modbus FC 0x01 讀線圈），但**沒有開成指令**，`cmd_status`
也不含繼電器欄位 —— 現在只能從「送過哪些指令」推斷實際狀態。要確認實體現況需要新增一支
`relay_status`。⚠️ 不要另開 TCP 連線直接查 `.20`：那條匯流排上還有 ZDT 推桿與程式自己的
輪詢，插入的幀會與運行中的交易競爭。
...
10. **水箱溢流處理**（2026-04-14 新增）— 10L 水箱若進水球閥持續補水是否會溢流？需使用者確認：
    - (a) 水箱有**浮球閥**（機械式，滿自動關）→ 軟體無需管，球閥清洗時常開即可
    - (b) 水箱有**溢流孔**接外排 → 軟體也無需管
    - (c) 都沒有 → 需軟體控制球閥間歇開關（依耗水速率）
    - 影響 Phase 4-C 與 Phase 2 Init 的進水策略
    - 2026-06-05 起進水球閥從 washrobot 改到 crane 端（crane PQW .34 slave 12 CH4），washrobot 透 `crane_cmd_("water_inlet on/off")` 控制
