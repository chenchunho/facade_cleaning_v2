...
6. ZDT 1,2,5,6 → 10 cm（腳推桿伸出貼牆）
7. CH2 = ON
8. 檢查 JC-100 (1,2,5,6) 真空度
...
2. ZDT 3,4,7,8,9 → 0（身體 + 中心推桿收回）
3. DM2J slave 1, 3 同步（反向）→ 身體相對腳下移 STEP_CM
4. ZDT 3,4,7,8,9 → 10 cm
5. CH3 = ON，CH4 = ON
6. 檢查 JC-100 (3,4,7,8,9) 真空度
   不足：retry_count++; 回 step 1
   成功 5 次仍失敗 → 停機回報人工
```
...
2. 關閉水系統
   CH5 OFF（刷子停）、CH6 OFF（水泵停）、CH7 OFF（水閥關）
3. 破真空（吸盤脫附）
...
- `stop` 無論何時直接中止，回到 `attached`（若有效）或 `error`
...
- 頂樓人員發現異常時，**按 E-stop 斷電** → 機器人自動脫離牆面 → 再由吊機將其拉回頂樓
...
| 姿態平衡偏移 `balance_deg > IMU_STOP_DEG` (45°) | 不詢問，立即停機 + crane stop + 回報人工 |
...
| `IMU_STOP_DEG` | 45.0 | 平衡偏移立即停機門檻（不詢問，直接 crane stop + 人工處理）|
...
5. **Phase 1 自動化** — 4 輪貼牆是人工還是吊機程式自動？DM2J 輪子（slave 2,4）在 Phase 1 要怎麼驅動？
   - 清洗動作細節已確認 2026-04-13：CH5 刷洗滾筒 / CH6 水泵 / CH7 水箱球閥
   - 機械手臂（USB→CAN）本版**不整合**
...
8. **中間絞盤變頻器** — 型號、Modbus 暫存器表、通訊參數待補（RPM 控制、方向、停止、狀態查詢）→ 影響新驅動實作與 `MIDDLE_WINCH_RPM` 參數
...
EVT imu_stop balance=<deg>               # balance_deg > IMU_STOP_DEG，已強制停機
```
