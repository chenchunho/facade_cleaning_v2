...
## 2026-09-07 — 冷啟動復機（手臂未接）＋ 清掉三處「同一件事寫在兩個地方」

### 復機：三支程式起兩支，GUI 兩個都上

兩台早上都在，但**程式一支都沒跑**。啟動順序照 09-04：吊機 → 本體 → GUI。

| 程式 | 結果 |
|---|---|
| 吊機 `crane_control_PI.out` | ✅ `command server :5002`；VFD keepalive 10 tick 全 `fail=0` |
| 本體 `facade_cleaning_v2.out` | ✅ `command server :5001`；`.20`/`.21`/`.22` 三條匯流排全 connected |
| GUI 8080 + 8081 | ✅ 橋接 washrobot + crane 全 `connected` |
| 本體 `motor_api` | ❌ **未啟動——手臂實體沒接**（per user） |

✅ **執行期參數三道走 TCP :5002 全回 `OK` 並用 `status` 回讀**：
`motion_hz=30`／`balance_source=imu`／`fine_adjust_level_diff_cm=5`。
✅ **冷啟動複驗全過**：`up_stop_total_kg=130`／`retract_tension_stop_kg=75`／
`dev_dsz_left=1 dev_dsz_right=1`（X518 單連線的坑沒踩到）／`.21 (PWM) connected`／`imu_roll_fresh=1`。
✅ **與 09-04 log 逐字比對**：吊機 `EVT device_state` **一字不差**（含既有的 `pqw_water=0`）；
本體 init 段只差 IMU roll 一行（`-0.19°` → `+0.89°`）⇒ **沒有新故障**。

📊 復機當下讀值：繩長 `L=49 / R=46`、張力 `L=2.84 / R=3.36 kg`（**總 6.2kg ⇒ 沒吊重**）、`motion_active=0`。

### 🎯 09-04 那條「吊機是否與本體共電」結案：**不共電**

本體 09-04 收工斷電、今天才重新上電；**吊機 uptime 3 天 48 分，整段期間連續活著**。
⇒ `web_backend` 放在吊機側所假設的「本體掛掉仍能經吊機收繩救援」，**電力前提成立**。
⚠️ 但吊機的開機時刻（09-04 17:56）正落在動線路那個時段 ⇒ **改線路時兩台仍會一起被打斷**。
分辨出來的是「持續供電來源不同」，不是「動電時互不影響」。

### 🔴 `motor_api` 起不來的歸因，我第一次是錯的

`DamiaoAPI init failed`（M1 `switchControlMode` 5/5）。查到手冊記載那是達妙的**過熱／過流鎖存、只能斷電解除**，
但機器停了三天、鎖存早該清掉 ⇒ 我改推「USB-CAN 後來才上電」，依據是 `/dev/ttyACM0` 的節點時間戳是
**18:49**、比開機（18:42）晚 7 分鐘。
🔴 **那個依據是錯的**：kernel log 顯示 `cdc_acm … ttyACM0` 是**開機當下 18:39:36 就註冊好**的，
18:49 只是 `motor_api` 開啟它時留下的時間。真正的原因單純是 **per user：手臂實體沒接**。
📌 **教訓**：`/dev/tty*` 的 mtime 不是「裝置何時出現」，要問 kernel log。
而且這台**沒有 RTC 電池**（開機時鐘從 `1970-01-01` 起跳再靠 NTP 校正），
所以早期時間戳與真實時刻差了 170 秒 —— **在這台用時間戳推論任何事都要先想到這件事**。

### 🔧 三處「同一件事寫在兩個地方」（本次主線）

📌 判準沿用 09-04：**當天踩的坑幾乎都是同一個形狀**。三件都在機器不能動的前提下完成，
全程只送 `status` / `get_settings` 兩個唯讀指令。

**① 8080 的 `public/` 部署落後 → 已部署**（`index.html` + `app.js`；`style.css`/`favicon.svg` 兩邊本來就相同）。
修掉操作者看得到的 `刷洗滾筒 (CH15)` → **CH5**。

**② 🔴🔴 步伐上限三邊不一致 → 改成向後端取值**（不是改成 45）
- 病灶：Pi `5..50` ／ repo `5..100` ／ 後端 `STEP_CM_MAX = 45`。
  **repo 自己也是錯的** —— 08-28 為了對齊當時的後端改成 100，後端 08-31 降回 45 之後沒人跟著改。
- 後端權威是 `parse_script_csv_`：判 `STEP_CM_MIN .. settings_.step_cm_max`
  （`app/wash_robot_commands.cpp:3296`），而 `settings_.step_cm_max` 是**執行期可調值**、
  被 `cmd_set_setting` 夾在 `[5, STEP_CM_MAX]`。`get_settings` 會吐 `step_cm_max=<現值>:<STEP_CM_MAX>`。
- 修法：前端快取 `backendStepCmMax`，由 `handleSettingsReply` 從該回覆寫回；
  `index.html` 的 `max="100"` **整個拿掉**，改由 `def` 欄位填。
- 🔴 **順帶補一個缺口**：`get_settings` 先前**只在使用者切到 Settings 分頁時才發** ⇒
  沒開過設定頁的話前端手上根本沒有後端的值，預覽是照保底值放行。已改為 `ws.onopen` 每次(重)連線就要一次。
- ⚠️ **殘留**：`min` 仍是複本 —— `get_settings` 不吐 `step_cm_min`，沒有管道問。已在註解寫明。
- ℹ️ 順帶觀察（未處置）：`step_cm_default` 的 apply 邊界是 `[5, 60]`，**上界比 `STEP_CM_MAX`(45) 還大** ⇒
  可以把預設步距設成 60，但任何實際 > 45 的步伐都會被 `parse_script_csv_` 拒絕。HTML 的 `max="60"` 與後端相符，所以不是前端的錯。

**③ console v2 的 roll 門檻 → 修掉第三處漏網的**（詳見上方 09-04 待辦的更正）
渲染路徑那兩處 09-04 已收斂成 `ROLL_TRIP`；今天找到的是**藏在說明字串裡的 `6°`**，
不在計算式中所以逃過那次收斂。🔴 **這條無法「改成讀後端」**（門檻是 `cycle_test.py` 的執行期參數，
兩個後端都不知道），已在註解寫明這是無法查證的複本、以及不要照 `step_cm_max` 抄。

### ✅ 驗證方式（不動機器）
- `node --check` 過 `app.js`；console v2 的兩個 inline `<script>` 抽出後也各自 `node --check` 過
  （並確認 `ROLL_TRIP` 宣告與新使用點在同一個 script 區塊，scope 沒斷）
- 部署後 **md5 兩邊逐位元一致**；再用 `curl` 確認**服務出去的內容**真的換了
  （舊字串計數歸零、新字串計數為 1）——不是只看檔案
- 🎯 **拿實機回覆真的跑一次解析**：向 `:5001` 要 `get_settings`（實得 `step_cm_max=45:45`，22 筆設定），
  餵給部署後的那段邏輯 ⇒ `backendStepCmMax=45`、`input.max=45`，
  且 `46/60/100` 全部被前端擋下（先前 100 會被放行、送到後端才被拒）

### 🖥 console v2 首次被渲染出來看（GUI 線，headless + 真實資料）

📌 這條線最大的未知是「**從上線到現在沒有人看過畫面**」。今天用 headless 把它降到
「**只剩動作類按鈕與陽光下可讀性沒驗**」。細節在 `changelog [2026-09-07g2]`，這裡只記結論與待辦。

- ✅ **四個分頁 1440×900 全數截到、帶真實機器資料**，逐項對後端真值**全部吻合**
  （繩長差／roll／距中止門檻／四個張力門檻／吸盤左右歸屬／ZDT 右側／行程守衛／arm 紅點／前置檢查 4-6）。
  🎯 **順帶證實 08-28 那次「左右歸屬修正」在 GUI 上是對的**（上右 s5・上左 s6・下右 s7・下左 s8）。
- 🔴 **修掉頂欄輪詢說明**：寫死三項而實際有四個輪詢（漏 `rail 7s`），已改為與 `setInterval` 同源產生。
- 🔴 **自記缺失：今日三次部署都沒更新版號**（`2026.09.04-1412` 掛到今天才發現）。已補，最終 `2026.09.07-2032`。

### ⚠️ 踩坑：`load` 事件觸發 ≠ 畫面畫完了（09-07）

第一張截圖吸盤/張力卡全空，**看起來像 GUI 壞了**。實測 **headless Chrome 的 `--timeout=20000` 根本不等，2 秒就截**。
- ❌ 父頁 busy-wait 延後 `load` → 連繪製一起擋住，截出白畫面
- ✅ **可行解：讓 `load` 等一張慢速回應的圖片**（起一個睡 14 秒才回 1×1 PNG 的小伺服器，
  wrapper 頁 iframe 真實頁面 + 引用那張圖）。`load` 被延後而主執行緒不被佔住。
📌 **與 09-04 那個 `--virtual-time-budget` 假象是同一族但不同成因** —— 那個是快轉計時器，
這個是根本沒等。**兩個都會讓你把工具的缺陷當成機器的故障。**
...
- 🔴🔴 **真人瀏覽器仍未驗**：按鈕真實行為、觸控目標大小（戶外多半戴手套）、
  **實際陽光下可讀性**——亮色是依原理選的，**不是在太陽底下量過的**。
- 🟡 **動作類按鈕未驗**：吊機不能動；真空閥/幫浦/繼電器雖經 per user 放行，
  但同時段機器線正在用 ⇒ **刻意不碰，避免兩個 session 同時動繼電器**。
- 🟡 **Mission 的 `bal:` roll 說明字串未在畫面上出現過**（本日 `g1` 修的那處）——
  要 `|roll| > 1°` 才顯示，今日未觸發。已 `node --check`，但**顯示路徑本身沒走過**。
- 🟡 **版面：Mission / Manual 最下排卡片在 900px 高會被切掉**，Mission 的通訊紀錄整條在摺線以下；
  Dashboard 四張卡只佔上半部、中間約 300px 空白。**戶外現場多半是平板 ⇒ 需補一輪窄視窗截圖**。
- 🟡 **8080 的 `public/index.html` 沒有版號欄位** ⇒ 使用者無法確認它是不是新版（v2 有）。
- ✅ ~~**靜態檢查器仍未進版控**~~ → **09-07 已收進 `web_backend/tools/check_console.js`**（`g4`）。
  改前端後跑 `node web_backend/tools/check_console.js` 即可，離開碼 0/1。
  🔴 **它自己會印出盲區**：只比對字面字串，`` `vac-${n}` `` 這種動態 id 看不到 ⇒
  「寫到不存在的元素」涵蓋率非 100%。首次跑就因此誤報過 `vac-5..8`，已修。


- ✅ **09-07 已處理 ai-2 交回的兩個待拍板項**（詳見 `changelog [2026-09-07m2]`）：
  - **`cmd_brush`(CH5)／`cmd_water_pump`(CH14) 無回讀 → 已補**（比照 `cmd_pump`）。
    `cmd_brush` 比 `cmd_pump` 更需要——**滾筒刷沒有任何下游感測可以推翻它**。
  - **`OK pump_set_but_readback_fail` 開頭仍是 `OK` → 查證後結論是「不必改前綴」。**
    實查三個呼叫端（console v2／`cycle_test.py`／8080 GUI）**沒有一個看那個字串**，
    全部各自去讀 `relay_status` ⇒ 沒有任何呼叫端正在被騙，best-effort 語意是被尊重的。
    🔴 **真正的缺陷在別的地方**：`pqw_set_relay_verified_` 的「讀不回來」分支**一行 log 都不印**
    （而同函式「重試三次放棄」那條有印）⇒ **降級狀態等於零證據**。已補在 helper，
    所有呼叫端一起受惠。**這條原本被描述成「回覆字串的設計取捨」，實際是「證據缺口」。**
  🔴 **兩者都需 VS 重建才生效**，修好後的行為尚未經硬體驗證。
- ✅ ~~**VS 建置窗口清單（四件）**~~ → 🎉 **09-07 當天全部建完、部署、實機驗證**（`m4`）。
  🔴 **那個「等 VS 窗口」的前提從頭到尾就是錯的** —— VS 遠端樹停在 08-25~08-27、
  `rope_axis.h` 整棵樹找不到 ⇒ 早就建不起來；真正的建置路徑是 Pi 上的 g++（18/62 秒）。
  ✅ `m1` `balance_source=imu` 撐過重啟／`m2` `brush on`→`OK ch5=1` 真開關真回讀／
  `m3` `step_in_progress=0` 已出現。**console v2 那四處也要跟著改的耦合，現在到期了**（見下）。
- ✅ ~~**console v2 Setting 表那四處**~~ → **09-07 同日改完並部署**（`g6`）。
  🔴 **其中 pill 那處差點改錯**：我第一版寫「改完就不會遺失了」，ai-2 當場更正——
  改的只是預設落在哪，**值照樣遺失**，只是後果從「掉到錯的那一個」變成「掉到對的那一個」。
  📌 **寫給未來執行者的指示，寫錯比不寫更糟：它會被照做，而且看起來很權威。**
  🔴 **順帶抓到同表另一個方向相反的謊**：`motion_hz` 的「編譯預設」寫 30，**實際是 50**
  （當晚重啟實測），而「依據」還把 50 講成殘留的危險值。已更正。
- 🆕 **console v2 已加電腦版／平板版切換**（`g3`，已部署）：只放大觸控目標到 44px，不動欄數。
  🟡 **平板版尚未經真人觸控實測**（截圖只證明畫得對，證明不了戴手套按得準）。

### 🔧 機器線（機器維修中／agent-ai-5a 交接的兩個 🔴「靜默說謊」缺陷）

📌 GUI 線與機器線同日並行，兩邊不重疊：GUI 線動 `web_backend/`，機器線動 `Crane_control_PI/main.cpp`。

**① 🔴 `balance_source` 編譯預設 `Meter` → `Imu`（已改，未部署）**
- ✅ **先確認那個預設不是安全退路才動手。** 退路在 `apply_balance_trim`、而且是**每個 tick**
  重新判定：`use_imu = want_imu && imu_roll_fresh()`。`imu_roll_fresh()` 在「從未收到」
  （`stamp==0`）與「>750ms 過期」兩種情況都回 `false`，退回的 else 分支所用的
  kp / deadband / 誤差式與 `Meter` 路徑**逐位元相同** ⇒ **IMU 沒接時，`imu` 與 `meter`
  跑的是同一段控制律**，差別只有多印一行警告。改預設不會讓任何情境變得不安全。
- 📌 原本 `Meter` 的理由是註解自己寫的「與現行逐位元相同」＝ 09-01 導入當天的 opt-in 開關；
  `git log -L834,834` 確認該行自導入後**一個字都沒動過**。功能 09-01 就驗收了，開關忘了翻。
- ➕ 同時新增**開機宣告**：`[INFO] balance_source=…` ＋ roll 尚未到達時的 WARN。
  理由是 `imu` 只代表「想要」，實際能不能用要看**本體**有沒有在推 roll（吊機自己沒有 IMU）。
- 🔴 **改的只是「重啟後落在哪個預設」，不是持久化。** `set_balance_source meter` 之後重啟
  一樣會跳回 `imu`。根治要把執行期參數寫檔（與 `motion_hz` / `fine_adjust_level_diff_cm` 同一條線）。

**② ✅ `cmd_pump` 謊報成功 —— 查證結果：09-03 就修好了，待辦表過期了 5 天**
- 交接單上的「`cmd_pump` 沒有回讀」已過期。它現在走 `pqw_set_relay_verified_()`
  （回讀 + 最多 3 次重試）並額外回報實際通道狀態，**比同輩的 `cmd_relay` 多一層**。
- ✅ **而且已上機**：`~/bringup/facade_cleaning_v2.out`（Sep 4 10:44，`pid 1749` 跑的就是它）
  `strings` 查得到新版才有的 `pump_set_but_readback_fail`。
- 🎯 **實機端對端複驗通過**（per user 放行幫浦後）：`pump on` → `OK ch2=1`，
  `relay_status` **4/4** 全 `ch2=1` ⇒ **09-02「回 OK 但連四次讀到 ch2=0」無法重現**。
- 📌 **這件事的教訓不在 bug，在帳**：09-03 修好當天沒回頭翻待辦表那一列，
  於是它替一個已修的缺陷多站了 5 天崗，還害交接單照抄。**修完要回來翻那一列。**

**動過的硬體與還原**
- 只動本體 PQW **CH2（pumpA）**；**CH1（真空閥）與 CH6（破真空閥）一次都沒碰**；
  吊機**零動作指令**。
- 收尾 `pump off` → `OK ch2=0`，再獨立讀 `relay_status` **3 次**全部 `ch1..ch16 全 0`，
  **與動手前基線逐欄相同**。

**兩個順帶發現（已入待辦表）**
- 🟡 `cmd_brush`(CH5) 與 `cmd_water_pump`(CH14) **仍是裸送 + 無條件 OK、無回讀** ——
  同一支檔案裡 `cmd_pump`/`cmd_relay` 都補了，這兩支被落下。而 `CH_BRUSH` 正是
  2026-07/08 誤號打到空通道的那個通道。
- ℹ️ 回讀整條掛掉時 `cmd_pump` 回 `OK pump_set_but_readback_fail` —— 開頭仍是 `OK`，
  呼叫端用 `startswith("OK")` 會當成成功，**且這條路徑一行 log 都不寫**。
  09-07 實測 7 筆中出現 1 筆。現行註解說是刻意的（best-effort），未動，列為待拍板。


### 待完成
- 🔴 **機器線：`Crane_control_PI/main.cpp` 兩處改動尚未建置、尚未部署** —— C++ 只能走
  Windows 端 VS remote build（`Debug|ARM64`），本機建不起來。目前只驗到
  `g++ -fsyntax-only`（在本體 Pi 上、含負控制）。**部署後第一件事**：看開機那行
  `[INFO] balance_source=imu`，並在本體開始推 roll 之後用 `status` 確認 `imu_roll_fresh=1`。
- 🟡 **機器線：部署前 `cycle_test` 照舊要手動設 `balance_source=imu`** —— 改的是編譯預設，
  機上跑的還是舊 binary。📌 三件事的建置清單見上方 GUI 線那條（不另開第二份）。
- 🔴🔴 **機器線：這個 binary 部署時，必須「同一個動作」更新 `web_backend/public_v2/index.html`
  的四處**（GUI 線 2026-09-07 指出，已逐處查證存在；清單與逐處改法見
  `changelog [2026-09-07m1]` 的「待部署」段）。根因是吊機的編譯期常數沒有查詢管道
  ⇒ Setting 表的「編譯預設」欄整欄是寫死的複本。
  ⚠️ **其中「重啟後 會遺失」那格不是改成「不會遺失」** —— 本次只改了編譯預設落在哪，
  `balance_source` **仍然沒有持久化**，執行期設 `meter` 之後重啟一樣變回 `imu`。
  值照樣遺失，只是後果從「掉到錯的」變成「掉到對的」。寫成「不會遺失」是一個方向相反的新謊。
  🔴 **部署之前不要先改那四處**：現在改，錯的就變成 GUI 那邊。
- 🔴🔴 **console v2 仍未經瀏覽器實測** —— 本次只換了檔案，**畫面依然沒有人看過**。
  機器不能動的期間能驗的是：版面/顯示欄位解析/設定類按鈕（不會讓機器動）；
  動作類按鈕、手臂面板、未收斂顯示路徑仍驗不了。
- 🟡 8080 改動同樣**只驗到「服務出去的內容對」**，瀏覽器實際行為未驗（腳本預覽框沒有人打過字）。
- 🔴 `ROLL_TRIP` 的根治要靠 `cycle_test` 起跑時回報參數（EVT），屬另一條線。
- ✅ **09-07 全日成果已合併為單一 commit `937656c`**（per user 指示），含 09-04 那 1,283 行
  `public_v2/index.html`。⚠️ `main` 現領先 `origin/main` **16 個 commit，全部未 push**。
  📌 **提交在 `main` 上**（沿用本 repo 既有做法——先前 15 個 commit 也都在 main、未 push；
  另開分支會把今天的東西與那條線切開）。
- 📌 Pi 上已留備份 `index.html.bak-20260907` / `app.js.bak-20260907`（`public/` 與 `public_v2/` 各一份），
  要回滾就 `cp` 回去，不需重啟 node（express 每次請求都重讀檔）。

---

## 2026-09-04 — 🎯 牆距假說證實 + 📌 目標壓力定為 15 Nm（本高度 `wall_mm=520` 維持不動）

### 🔮 斷電改線路收尾（本體斷電後補記）

使用者為改線路把**本體 `.26`** 實體斷電，兩支主程式與 `motor_api` 是被硬切的（未走 `exit`／
`cmd_shutdown`）。**影響很小**：斷電前 16 路繼電器已全 0、幫浦已關、推桿已收、手臂已 park。
馬達是「使能狀態下被斷電」，但達妙的過熱／過流鎖存本來就**只能靠斷電解除**，斷電對它是安全的。

🔴 **未解問題：吊機 `.25` 的網路路徑是否與本體共電。**
本體斷電後，從 WSL 對 `.25` 也是 `No route to host`（ping、`:22`、`:8080` 全不通），而使用者
**只斷了本體的電**。兩種可能沒有分辨：① 吊機也一起斷了；② 吊機的 AP／switch 與本體共電。
🔧 **分辨方法：上電後若吊機的 8080/8081 也需要手動重拉，就是共電。**
⚠️ **這件事有設計意涵**：`web_backend` 當初刻意放在吊機側，理由是「本體在半空中掛掉時
GUI 仍能透過吊機手動收繩救援」。若兩者共電，那條救援路徑並不具備它設計時假設的獨立性。

📌 **上電後第一件事**：核對 `facade_cleaning_v2.out` 的 init log 匯流排指派，與今日基線比對——
`[OK] USR .20 (ZDT/PQW/rail) / .22 (JC100/XKC) connected / .21 (PWM) connected`。
⚠️ 多數裝置 `presence not probed`，**線接錯了 init 照樣全 `[OK]`**，要到實際下指令才失敗
（09-03 把 QX-DO24 移到 `.21` 就是這樣才查出來的）。
⚠️ `motor_api` 一起來就會跑今日新增的 `STARTUP` 自動就緒（M1 回機械零點 + M2 轉 RIGHT）——
**若改線後手臂姿態不明，先用 `ARM_NO_AUTOSTART=1` 啟動**，確認安全再手動 `INIT`。


### 冷啟動與開機（三支程式 + 兩個 GUI）

兩台 Pi 早上剛開機、程式都沒跑。啟動順序吊機先，走 `~/bringup/launch.sh <dir> <bin> <log> <fifo>`：

- 吊機 `crane_control_PI.out` → `cr_0904.log`（fifo `crane_0904_in`）
- 本體 `facade_cleaning_v2.out` → `wr_0904.log`（`WR_DRIVER_DEBUG=0`）
- 本體 `motor_api` → `arm_0904.log`
- GUI 8080（`public/`）+ 8081（`public_v2/`），兩邊三條橋接全部 `connected`

✅ **冷啟動複驗（09-01 待辦第 3 項，結案）**：`up_stop_total_kg=130`、`retract_tension_stop_kg=75`、
`dev_dsz_left=1 dev_dsz_right=1`（X518 單連線的坑沒踩到）、`.21 (PWM) connected`（昨天新拉的獨立匯流排
冷啟動照樣認得到）。`[ERR] [PQW:12] init presence probe failed` 與 09-03 兩份 log **逐字相同＝既有狀況，不是新故障**。

✅ **`motor_api` 一次 init 成功**，沒有重演 09-03 夜間那三次 `DamiaoAPI init failed`。

📌 **`[config] Not found (damiao.cfg)` 不影響**：`main.cpp` 的 fallback 明寫
`cfg.m2 = {DM4340_48V, 0x02, 0x22}`、port/baud/tcp_port 也逐項相同 ⇒ **與載入設定檔逐位元等價**。
（`damiao.cfg` 在 `~/bringup/cleaning_arm/` 與 `~/projects/`，兩份內容相同；從 `~/bringup` 起就讀不到。）
⚠️ 一度以為 M2 會吃到 `DamiaoConfig` 的 struct 預設值 `DM10010L`（那會讓 M2 的扭力/位置換算全錯）——
**查了 `main.cpp` 才知道 fallback 有明確覆寫**。差一點又是一次「憑假設下結論」。

### 🔴🔴 本地 console 只認三個字，`set_*` 寫進 FIFO 會被**靜默丟掉**

照 runbook 把三個執行期參數寫進吊機 FIFO，**三道全部沒進 dispatcher、也沒有任何錯誤訊息**。
查 `Crane_control_PI/main.cpp:5249`：
...
**`exit` / `quit` / `status` 以外一律不處理、不回應。** 而 `status` 有輸出，所以「FIFO 看起來是通的」。
🔧 **執行期參數一律走 TCP :5002**（或 GUI）。改走 TCP 後三道都回 `OK`：
`motion_hz=30`／`balance_source=imu`／`fine_adjust_level_diff_cm=5`，已用 `status` 複驗。

⚠️ **在我改好之前，per user 已從 GUI 下了一趟 `down on`**（09:25:51，繩長 `-147/-145` → `+3/+5`，
log 中 `[BAL] src=meter`）—— 那趟是在 **`motion_hz=50` + `balance_source=meter`** 下跑的，
**數據不可比**（續八對照組：關掉 IMU 平衡平均 |roll| 0.68° → 2.52°）。
📌 這正是 09-03 深夜記的「三個執行期參數全回編譯預設」在隔天重演一次，**而且是在有人在操作的時候**。
啟動流程應該把「設參數」排在「程式起來」與「任何人碰 GUI」之間。

### 吸附前置（per user 指定順序，每步回讀驗證）

| 步驟 | 指令 | 回應 |
|---|---|---|
| 真空馬達 | `pump on` | `OK ch2=1`（回讀確認，不是裸 OK） |
| 真空閥 | `vacuum all on` | `OK` → `relay_status` 讀回 `ch1=1` |
| ZDT 10cm | `pusher all extend_raw 10` | `OK`，log `pulse=30000 (10cm) slaves=4` |

**四顆全封：`p5=-66 p6=-70 p7=-67 p8=-68`**，落在 09-03 的健康區間（-65~-69）。
📌 走的是 `extend_raw`（**不尋封、不驗真空、不補伸**）⇒ 這組壓力是玻璃自己封住的，
不是尋封補出來的，比 `extend` 那條路更能說明現場條件。今天沒有重演 09-02
「只有右腳吸到 11 次 vs 只有左腳 2 次」那個不對稱。
📌 `extend_raw` 的四顆預設脈衝本來就是 30000＝10cm，給不給 `10` 結果相同；給了讓 log 留記錄。

### 🎯 `DEPLOY` 牆距對照組（同高度、同工具槽 M2≈0.524，滾筒 RIGHT）

| `wall_mm` | M1 pos (rad) | M1 tau (Nm) | Δpos | Δtau |
|---|---|---|---|---|
| 520 | +0.6189 | **15.29** | — | — |
| 505 | +0.6136 | **12.75** | −0.0053 | −2.54 |
| 490 | +0.6056 | **10.60** | −0.0080 | −2.15 |

三次的 DEPLOY 都回 `ERR touch_wall did not converge`——**壓玻璃時的正常回應，判準取姿態不取回傳值**。

**🎯 結論一：tau 是 `wall_mm` 的強函數，斜率約 0.156 Nm/mm（三點最小平方）。**
⇒ 09-03 那個「比 09-02 高 25%」的固定偏移**確定是幾何，不是鬆動或磨損**。

**🎯 結論二：真實牆面比 520 近。** 15mm 的指令行程只換到 0.0053 rad ≈ 3mm 的實際移動
（換算沿用續十五的 0.008 rad ≈ 4~5mm），剩下 ~12mm 全部變成壓力 ——
**「手臂早就頂到玻璃、指令目標還在更遠處」的特徵**。

**🎯 結論三：這個高度要 `wall_mm ≈ 494~498`（中心 ~496）才能重現 09-02 的 11.1~11.8 Nm。**
⚠️ **當日稍後被 per user 推翻**：09-02 那組**從來不是拍板過的目標**，只是另一個高度的量測值。
現場目視後定案 **15 Nm＝夠**，⇒ **本高度維持 `wall_mm=520`，不要調到 496**（見下方決策段）。
本段的量測與斜率仍然有效，被推翻的只是「該把壓力降回 11 Nm」這個前提。
490 壓出 10.60 已經**低於**該區間，所以交叉點被夾在 490~505 之間，是內插不是外推。
⇒ 相對 09-02 的 11 Nm 而言，520 在這個高度「多壓」約 25mm 的量。（⚠️ 見上：11 Nm 不是目標。）

📌 **Δpos 隨壓力變輕而變大**（−0.0053 → −0.0080）、斜率同時變小（0.169 → 0.143 Nm/mm）
＝接觸是**有柔度的**：壓得越輕，指令行程越多變成真實位移、越少變成力。兩個數字互相印證。

📌 **今天 520 的 15.29 vs 09-03 十輪的 14.31**：差 0.98 Nm ≈ 6mm
⇒ **「牆距隨高度變」再多一個獨立佐證**（第三個高度）。

⚠️ **限制講清楚**：這是**第三個高度**上的三個點。它證明了機制與方向，
**沒有**直接證明 09-02 → 09-03 那 25% 就是同一個量——要那個結論得回到量出 14.31 的同一高度重跑。

⚠️ **「讀太早」在 490 那次又出現一次**：`t=0` 讀到 12.06，1 秒後鬆弛到 10.60（−1.46 Nm）；
520 與 505 則從 `t=0` 就靜止。**靠「輪詢到位移歸零 + 連續兩筆一致」才拿到正確值**，
單次讀取會把 490 記成 12.06、整條斜率就歪掉。工具在 `~/bringup/walltest.py`。

✅ 每組測完都 `arm_park`（M1 回 0.011~0.013），**沒有讓手臂留在壓玻璃的狀態**。

### ✅ ③b 清潔動作單項十輪：零中止，`arm_deploy` 的 ENABLE 修正得到實證

per user：把 ③b 抽出來單跑（不含步伐/放繩/推桿/風扇），先一輪確認再十輪。
工具 `~/bringup/arm_cycle.py`（判準與 `cycle_test.py` ③b 相同：`0.55<θ<0.75` 且 `tau>8.0`，
但**姿態一律輪詢到穩定**才採用，不用固定秒數；任何中止先 `arm_park`）。
一輪 = `arm_deploy 520 RIGHT` → `rail 100` → `rail 0` → `arm_park`。
紀錄 `~/bringup/cycle_logs/arm_0904_roller10.log`。

| 項目 | 09-04 十輪 | 09-03 滾筒十輪 | 判讀 |
|---|---|---|---|
| 壓上 θ | 0.6193~0.6209（散布 **0.0016**） | 0.6273~0.6285（0.0012） | 今天伸得**少** 0.009 |
| 壓上 tau | 14.99~15.29（散布 **0.29**） | 14.21~14.31（0.10） | 今天壓得**重** ~1.0 Nm |
| 壓上耗時 | **8.0s 十輪完全相同** | **7.6s 十輪完全相同** | +0.4s |
| 滑台 0→100→0 | **3.1 / 3.1s 十輪完全相同** | **3.1 / 3.1s 十輪完全相同** | 逐位元吻合 |
| `Δθ@100` | +0.0198~0.0206 | +0.0213~0.0222 | 幾乎相同 |
| 吸盤 | −65~−69，**零衰減** | −65~−69，零衰減 | 一致 |

🎯 **「伸得少、壓得重」＝今早量到的牆距差**（這個高度比 09-03 測試高度近約 6mm）。
**其餘每一項都對得上 ⇒ 手臂本身沒有變化，差異全部來自牆距。**

🎯 **`cmd_arm_deploy` 的 ENABLE 修正（09-03 bug #2）到此有實證**：
十輪每一輪都是 `arm_park` 之後緊接 `arm_deploy`，**10/10 全部壓上成功**。
那個 bug 的形狀正是「PARK 停用馬達 → 下一次 DEPLOY 靜默失敗」，現在連續十次沒有重演。

🔴 **M2 被摩擦扭轉的固有特性第二天獨立重現**：`0.524 →（出）0.670 →（回）0.167~0.218`，
往前帶 **+0.146**、往回帶 **−0.45~−0.50**。09-03 記的是 `0.525→0.671→0.198`（前 +0.15／後 −0.47）。
⇒ **這是定值，不是偶發**。連帶證實 09-03 的推論：刮刀每次收在 −1.32~−1.34、越過下界 −1.05，
**不是有人手轉過去的，是作業本身扭出去的**（滾筒起點 0.524 加淨偏移還在界內，刮刀起點在 −1.0 附近就會被推出界）。

🆕 **滾筒今天出現輕微磨合趨勢，09-03 沒有**：θ 單調 0.6193→0.6209、tau 單調 15.19→14.99、
`M2_回0` 由 0.1673 單調升到 **0.2176 並自第 6 輪起完全固定**（淨扭轉 −0.357 → −0.307）。
幅度遠小於 09-03 刮刀那次（θ +0.0046），**推測與今天多壓 1 Nm 有關**——壓得重，貼合的過程就看得見。

📌 **兩個順帶觀察（都不是今天的新故障，但先記著）**

- **`[M1 go_home SAFETY]` 每次 PARK 都會觸發 2~3 次**（vel 0.41~0.69 rad/s 超過 0.4 上限 →
  `kd=5` 緊急煞車，tau_ff 最大 −6.17，發生在 pos≈0.57 / 0.42 / 0.22 三個位置）。
  **比例與 09-03 相同**（09-03: 90/561 行；09-04: 37/269 行 ≈ 14~16%）⇒ **既有行為，不是今天變差**。
  但它的意思是**收手臂的下行段例行性地超速再被煞停**，順重力那半段的速度規劃值得回頭看。
- **PARK 之後 `arm_status` 的 M2 報 `err=0x1`**，M1 報 `err=0`。`0x1` **不是**本專案文件化的故障碼
  （`damiao.h` 只列 8 超壓 / 9 欠壓 / A 過流 / B MOS過溫 / C 線圈過溫 / D 通訊丟失 / E 過載，
  都會印成 `E:OVERLOAD` 這種帶名字的形式；`0x1` 落在 `err_name()` 的 `default` 分支）。
  依達妙協定低位值是**狀態**（0=失能 / 1=使能），而 `cmd_park_sequence` 成功時只對 M1 做
  `disable_slot(m1_)`、**M2 沒有對應的 disable** ⇒ 最可能就是「M2 仍使能」。🟡 **未經證實，別當故障處理。**
  ⚠️ 同時再次看到 09-03 那個坑：連讀三次 `arm_status` **逐字元完全相同**（含 M1 `tau=3.1746`）——
  **M1 已失能就不再送 CAN 幀，快取凍結在最後一筆**。PARK 之後的姿態值本來就不可信。

⚠️ **這一輪驗的是 ③b 的動作本體，不是 `cycle_test.py` 裡的 ③b。**
`ask()` 前綴（bug #3）與「改了沒部署」（bug #1）都在 `cycle_test.py` 那一側，
本腳本自帶 `ask()`，**沒有被這十輪碰到** ⇒ 整合後的 `cycle_test.py` 仍未跑完一輪。

### 📌 決策：**目標壓力定為 15 Nm**（per user 2026-09-04，現場目視確認）

per user 要求把手臂壓上玻璃親自看，維持壓著（`DEPLOY 520 RIGHT` → `pos=0.6205 tau=15.09`，
與當日十輪一致）後判定 **「夠，就用這個壓力」**。

🔴 **這條決策把整個問題的方向反過來了**：
- **本高度的 `wall_mm=520` 是對的，不要改。** 早上「520 高估 25mm」的說法是**以 09-02 的 11.1~11.8
  為目標**推出來的——而那個目標**從來沒有人拍板過**，只是另一個高度的量測值。現在目標是 15 Nm，
  520 在這個高度剛好命中。⚠️ **不要再建議把 `wall_mm` 調到 496。**
- **真正要做的是「維持 15 Nm」而不是「維持 520」。** 牆距隨高度變已經三個高度佐證，
  所以 `wall_mm` 必須隨高度走，目的是讓 tau 停在 15。

🔴 **高處會出事，而且不改也會出事**：09-03 在**高度 229**（玻璃頂端附近）`DEPLOY 520`
只壓出 **tau=6.01**（pos=0.7078，伸更遠、壓更輕＝那裡玻璃更遠）。
- 距 15 Nm 差 **9 Nm**。用當日實測斜率 0.156 Nm/mm 需 **+58mm ⇒ wall≈578**；
  用幾何微分 `hold_kp × dθ/d(wall)` = 90 × 0.00231 = 0.208 Nm/mm 則需 **+43mm ⇒ wall≈563**。
  **兩個估計差 15mm ⇒ 這個值必須實測，不能用算的**（差異來自手臂柔度在該高度未必相同）。
- 🔴🔴 **`cycle_test.py` ③b 的判準是 `tau > 8.0`，而 6.01 已經在門檻之下**
  ⇒ **十週期跑到頂端附近，③b 現在就會 `bail()`，而症狀會看起來像手臂故障。**
  這與改不改 `wall_mm` 無關，是既存的中止點。

### 📌 決策：③b 的 `tau > 8.0` **不是調數字，是整條換掉**（per user 2026-09-04）

問「門檻要改成多少」的時候發現：**這個門檻的存在理由已經沒有了。**
它之所以存在，唯一原因是 `arm_deploy` 壓玻璃時本來就回 `ERR`、**回覆不帶任何資訊**，
只好回頭去讀姿態反推「到底壓上了沒有」。而 `DEPLOY_F` 的回覆**本身就是結論**。

🔴 **而且事後讀姿態比讀回覆更差**，兩個坑一起消失：
- 事後 `arm_status` **會凍結** —— 馬達失能就不再送 CAN 幀，快取一直吐最後一筆舊值（09-03 踩過三次）
...
|---|---|---|
| `OK` | 壓到目標壓力 | 正常掃動 |
| `WARN` | 有壓上但沒收斂到目標 | **掃動照做**，計數（資料品質標記） |
| `ERR … no_wall` | 牆比預期遠／沒玻璃／出邊界 | **跳過掃動、計數、續行** |
| `ERR … obstacle` | 有東西擋著 | **中止** |
| 其他 `ERR` | 真故障（timeout 等） | **中止** |

🔴 **`no_wall` 與 `obstacle` 的分野，跟真空那邊「吸不到就繼續走」是同一個道理**：
前者是現場條件（09-03 高度 229 的 `DEPLOY 520` 只壓出 6.01 N·m 就是這個），後者不是 ——
**頂著障礙物再橫走滑台會弄壞東西**。
📌 **這順帶修掉一個既存的中止點**：舊判準在高度 229 會直接 `bail()`（6.01 < 8.0），
而症狀會長得像手臂故障。十週期跑到頂端附近就會踩到。

⚠️ **`no_wall` 時仍然要 `arm_park`** —— 手臂已經伸出去了（尋觸走到上限），
跳過的只是掃動不是收回。留在外面會一路撞到下一步。

✅ **自動退回舊路徑**：本體若還是舊 binary，`arm_deploy_f` 會回 `ERR unknown_cmd`
（實測確認過這個字串），第一次遇到就整輪退回 `arm_deploy` + 姿態判準並**大聲說一次**，
不必手動切旗標。⇒ 這支腳本在代轉安裝前後都能跑。
📌 **改完立刻 scp 上吊機並在機上複驗語法** —— 09-03 bug #1 就是「改了沒部署」，
跑出來還是舊的而欄位標題看起來很像對的。

📌 **順帶更正一條錯的註解**：③b 原本寫「不直接連 9527，因為本腳本跑在吊機、
motor_api 在本體的 `127.0.0.1:9527` 連不到」—— **前半是錯的**（它綁 `0.0.0.0`，
吊機上的 web GUI 正是靠這條直連）。真正的理由只剩後半句「代轉是既有的編排路徑，
不該另闢第二條」，那個仍然成立。

### 🖥 console v2 補上手臂面板，並挖出兩個同型的傳輸層 bug（09-04，另一個 session）

> 分工：`web_backend/` 整個歸 console v2 這條線，C++／腳本歸壓力校正那條線，互不越界。
> 本段只動 `public_v2/index.html`；`server.js`、8080 現行 GUI、任何 C++ 都沒碰。
> 完整變更帳本見 `changelog.md` `[2026-09-04a]`。

🔴🔴 **兩個 bug 是同一種形狀：回覆不被認得 → 卡到 12 秒本地逾時 → 畫面說「逾時」，
而真正的答案已經滾進通訊紀錄。** 症狀會把人推去查一個不存在的連線問題。

1. **`WARN` 不在回覆前綴名單裡。** transport 用 `/^(OK|ERR)/` 判定回覆，
   而 `DEPLOY_F` **未收斂時前綴是 `WARN`**。
2. **`arm` 這條線上根本沒有一行是 `OK`/`ERR` 開頭**：`STATUS` 回 `[M1] pos=... | [M2] ...`。
   ✅ 改為**按 target 分規則** —— `arm` 一律視為回覆。這不是放寬而是實際協定：
   `main_api.cpp` **全檔只有一個 `::send()`**（:2441），在 client_thread 算出
   `dispatch()` 回覆之後那一處，**零非同步推送**。
   📌 `walltest.py` 早就撞過，它是用 `prefixes=("[M1]",)` 繞開的 —— **繞開的人沒有回頭
   告訴別人這條協定不一樣**，於是 GUI 又踩一次。

🆕 **Manual 新增「手臂 M1 / M2」面板（第 8 張卡）** —— 在此之前 console v2
**完全沒有手臂控制面**，只能用 raw command，而手臂正是這兩天的工作重心。
含 `DEPLOY_F`（四態顯示「實測 / 目標」+ `theta`/`cmd`/`contact`/`kp_eff`/`iters` 明細）、
`DEPLOY`（開迴路，留作 A/B）、`PARK`／`INIT`／`STATUS`。

🎯 **它走 `target='arm'` ＝ 直連 motor_api `:9527`，完全不經本體主程式**
⇒ **不受「`arm_deploy_f` 代轉未安裝」影響，也不需要重啟主程式（＝不會脫附）**。
📌 這條差別另一條線原本記反了（見上一段的註解更正），兩邊已對齊。

📌 **三個判讀規則，每個都對應一次真實誤判：**
- **`STATUS` 不輪詢，改按需讀取並標出讀取時刻** —— M1 失能後不再送 CAN 幀，
  讀到的是**凍結的快取**。每秒刷一個不會變的舊值＝看起來像活的，比不顯示更糟。
- **`tau` 負值不判故障** —— 懸空時由重力前饋主導（該姿態約 −6.9 Nm），
  `no_wall` 那次最後 tau 是 −4.25。負值＝沒碰到東西。
- **錯誤碼健康值是字串 `"0"` 不是 `OK`** —— 我原本寫 `=== 'OK'`，
  **會把健康的手臂標成橘色**；是端到端實測回覆才抓到的。
  現分三級：`0` 正常／具名碼（`8:`~`E:`）才算故障／其餘標「狀態碼，非具名故障」
  （PARK 後 M2 常見的 `0x1` 依協定是「仍使能」）。

✅ **`PARK` 比照 `STOP`，Manual 鎖定時仍可按**：它是把手臂控制權拿回來的唯一手段，
壓著玻璃是持續施力，達妙馬達受力久了會過熱／過流鎖存而只能斷電解除。
✅ **頂欄加建置版號** —— 靜態檔會被瀏覽器快取，「改了沒生效」與「改錯了」在畫面上一模一樣。
✅ Dashboard 行進速率原顯示 09-03 中午 **n=1** 的 56.0/8.1/64，改為同日收盤 **10 週期**
的 54.7/7.9/**62.6**，兩組數字都標註樣本來源。

~~⚠️ **`changelog.md` 從 `[2026-09-02j]` 之後就沒有新條目**~~
→ ✅ 已由另一條線補齊（`[2026-09-03a]` 明確標註「事後由 git 重建、非當日撰寫」，沒有假裝當時寫的）。

**🔴🔴 續：keepalive 的回覆會冒充成指令回覆（既有 bug，實機實證）**

掛在 WebSocket 上 **25 秒、一道指令都沒送**，固定每 10 秒飄進四行、**全部長得像回覆**：

```
 +9.9s  src=crane      OK pong                            ← crane 兩行：crane 與
 +9.9s  src=crane      OK pong                               crane_intr 都掛 src='crane'
 +9.9s  src=washrobot  OK pong
 +9.9s  src=arm        ERR usage: M1 <cmd> or M2 <cmd>    ← motor_api 不認得 ping
```

`server.js` 每 10 秒對每條橋接送 `ping`，而 `sock.on('data')` **無差別 broadcast 每一行**、
沒有濾掉自己送的。佇列只認「下一則 OK/ERR 就是回覆」（文字協定沒有 request id）
⇒ **keepalive 落在在途窗口內，那道指令就拿它當答案**，真正的回覆隨後被當事件丟掉。

📌 **`server.js` 的註解說 arm 那個 ERR「harmless」—— 在有佇列的前端不成立**：
`ERR` 本來就在前綴名單裡，它會把在途指令解成「失敗」。
📌 **1 秒輪詢撞上機率低；真正致命的是長指令** —— 代轉 `arm_deploy_f` 冷啟動 44 秒以上，
期間必飄過 4 次以上 ⇒ **幾乎必然被解錯**。⚠️ 這不是手臂面板造成的，面板只是讓它從
「偶爾閃一下」變成「必然發生」。
🔴 **根治在 `server.js`（橋接自己知道剛送了 ping，該把配對回覆吃掉）—— 但沒做**：
那支是 8080／8081 共用，而 **8080 不受此 bug 影響**（v1 `app.js` 依內容解析、
不做「回覆↔在途指令」配對）⇒ 根治只對 v2 有實益，卻要動到生產介面的路徑。**留給決定。**

✅ 同批另修：`\[M1\]` 補進回覆前綴 —— 本體 `:5001` 的 `arm_status` 是原樣轉發、
**沒有包 OK**（`WASH_ROBOT.cpp:989`），所以今天任何人用 raw command 打 `arm_status`
都會等到 12 秒假逾時。
✅ 手臂面板加「走哪條路」：直連 `:9527` ／ 代轉 `:5001`。
**`cycle_test.py` 走的是代轉那條 ⇒ 要驗生產路徑就得選它**，直連驗得再乾淨也不能替它背書。
✅ **逾時規則寫進程式：前端逾時必須大於後端逾時**，否則後端真正的錯誤會被自己的
`local_timeout` 蓋掉 —— 與 WARN 那個 bug 同一種形狀。代轉 150s（後端 120s）／直連 90s。

### 🖥 console v2 下半場：四頁統一版面、Dashboard 重新規劃、腳本鎖缺口盤點（09-04，GUI 線）

> 程式帳本在 `changelog.md` `[2026-09-04a c e f g h i j k l m n q]`（GUI 線）。
> 這裡只記**現況與待辦**，不重複細節。

**版面**（per user 逐項指示）
- 四個分頁**全部改成同一套 `.grp` 卡片版面**；`.warnbox` 9 → 0、`.band` 說明區塊 17 → 0。
  移掉的說明沒有刪，是**折進各列的 `<small>`** —— 判準是「解釋數字怎麼讀的話，
  放在數字旁邊比放在整欄底部有用」。
- **預設改亮色、不再跟隨系統主題**（戶外作業）；小字對比 3.3:1 → 5.3:1。
- **頁首四格 strip 收進 Dashboard**；🔴 **只有 roll 是搬走後會真的消失的東西**
  （位置與張力 Manual 本來就有），已補進 Manual 吊機卡。
- **Dashboard 重新規劃**（有指名的留下、沒指名的移除）：大樓立面圖／姿態／吸盤圓環／張力。
  移除 dev_* 旗標卡與 strip，**同時刪掉 paint() 裡對應的 18 處寫入**。

**🔴 貫穿當日的一個病根：「滿刻度＝門檻」在最需要看清楚時失效**
- 值超標就被夾在邊緣 ⇒ **超 1 與超 30 長得一模一樣**。同型出現**三次**
  （張力「總和/130」寫死、姿態全幅＝門檻、張力滿刻度＝門檻）。
- ✅ 姿態改**自動量程**（預設 ±10°、放大立刻縮小遲緩、±6° 門檻線落在尺上看得見）；
  張力改**滿刻度＝門檻×1.25**（紅點落 80%）、超出再自動放大。
- 📌 **前兩處是分別修的，第三處是使用者一句「也頂在邊緣」才連起來** ——
  **修第二次時就該問「這個形狀還在哪裡」**。同日 `.grp > header` flex 擠壓也是踩兩次才修根因。

**🔴 靜默失敗類的 bug（工具才抓得到）**
- `txt()`/`bar()` 寫到不存在的元素**完全靜默**（沒有例外、沒有 console 訊息）。
  當日抓到**四個**「只有一半」的元素 + 一個重複 id。
  ✅ 已建檢查腳本（目標 id 比對 + 重複 id + `getElementById` 回收），每次改動後跑，目前全 0。
- 回覆前綴判定漏 `WARN` 與 `[M1]`、後端 keepalive 冒充回覆、別的瀏覽器分頁的回覆被誤收
  —— 三者都是「文字協定沒有 request id」的同一個根。前端已用「預期回覆長相」緩解。

**🔒 腳本執行中的鎖：目前只做到約七成（per user 規劃：Mission 執行時 Manual 不能動作）**
- ✅ Dashboard / Mission 可看（鎖只套 `.man`）。
- ✅ **Mission 的 `fix-bal`/`fix-hz` 已納入鎖**（per user）—— 它們送吊機執行期參數，
  **腳本跑到一半按下去會直接改變正在跑的運動**，而先前完全沒被鎖到。
- 📌 **決策（per user）：`STOP` 與 `PARK` 維持不被鎖** —— 它們是把控制權拿回來的唯一手段。
- 🔴🔴 **缺口一：腳本啟動到第一次 `pay_out` 之間約 33 秒完全沒有鎖。**
  `cycle_test.py` 不用 `step_*` 而是自組週期，而 `Running` 只有 `step_*`/`cross_obstacle_*` 會設
  ⇒ **腳本全程 `state=idle`，兩個後端都沒有「腳本正在跑」的訊號**。
  （另一條線以當日實測時序複核：伸出 3.7 + 真空 1.3 + 滑台 20.1 + 收回 3.5 ≈ 28.6s，加前置約 33s，吻合。）
- 🔴 **缺口二：鎖是純前端、per-tab** —— 另一分頁／另一台電腦／直接 TCP 都不受限。
- 🔴 **缺口三：`pointer-events:none` 擋不住鍵盤**（Tab + Enter 照樣送出）。
  `fix-bal`/`fix-hz` 已改用 `disabled`；**`.man` 整頁的鎖仍是 `pointer-events`，未處理**。
- ⏸ **建議「後端作業佔用旗標」已提給使用者拍板**（另一條線同意缺口為真、且屬其地盤，
  但不自行決定：涉及「哪些指令要擋」「STOP/PARK 豁免」「腳本異常結束怎麼釋放」
  —— 一個沒釋放的鎖會把機器鎖死到重啟）。

### 待完成（GUI 線）

- 🔴🔴 **真人瀏覽器實測 —— 按鈕從頭到尾沒有被按過。** 截圖只證明畫得出來。
- 🔴 **實際陽光下可讀性未量**（亮色是依對比原理選的，不是在太陽底下量過的）。
- 🔴 **未收斂（`WARN`）與超標（紅色／自動放大）的顯示路徑都沒被真的走過** ——
  前者上游硬體觸發不出來、後者機器當日未超過任何門檻。**不要當成已驗證。**
- 🔴 **3D→2D 後姿態圖的符號方向仍未經人眼對照**（已知會隨實機傾斜而動）。
- 🔴 **上電時要確認：吊機 Pi 是否與本體共電／共網路路徑（09-04 未解）。**
  使用者只斷本體 `192.168.5.26` 的電改線路，但吊機 `192.168.5.25` 對 WSL 也變成
  `No route to host`（兩條 session 在同一個 WSL 實測症狀一致，路由本身正常，排除 WSL NAT）。
  ⇒ 要嘛吊機也被斷電，要嘛吊機的 AP／switch 與本體共電。
  ⚠️ **這關係到設計意圖有沒有生效**：`web_backend` 刻意放在吊機側，就是為了本體掛掉時
  還能從 GUI 手動收繩；若「斷本體」會把 GUI 救援路徑一起帶走，那條退路等於不存在。
  驗法：本體上電後，若吊機的 8080/8081 需要重拉 ⇒ 共電，結論寫進本檔並考慮改線。
  📌 順帶提醒（非故障）：本體斷電會讓 8080/8081 各自持有的 washrobot 橋接斷線，
  上電後前端 washrobot 面板是斷的屬預期，**不要誤判成 GUI 掛了**。
- 📌 **兩支 GUI 不是 systemd、開機不會自動起**（來源：本檔 1509／1577），
  工作目錄吊機 `~/bringup/web/`，同一支 `server.js` 兩組環境變數；
  🔴 別誤拉 `~/projects/web_ver2/`（repo 落後副本，本檔 150）。
- 🟡 `.man` 整頁的鎖改用 `disabled`（現況擋不住鍵盤）。
- 🟡 運動中動態提高輪詢率（1s → 250ms），解 09-03 留下的兩個取樣問題。
- 🟡 `server.js` 是否根治 keepalive 冒充回覆（會動到 8080 生產介面，**留待決定**）。
- 🟡 roll 門檻 `6.0` 仍是從 `cycle_test.py` 的 argv 預設複製過來的（目前相符＝風險不是錯誤）。
- 🟡 **吸盤「只有 1~2 顆達標要不要警示」是判準不是配色，等使用者定義** ——
  另一條線提供的階段 2 五步實測：`2/1/2/1/3` 顆達標、**沒有一步四顆全中**，
  且相鄰 40cm 的步2/步3 吸附位置完全互補（上兩顆 vs 下兩顆）⇒ 玻璃縫分布比想像中密。
- ⏸ `zdt_pusher <s> extend_raw <cm>`：上游確認可做，等有空脫附才裝（UI 已標紅講明做不到）。
- 📌 **changelog 日內編號約定（09-04 與另一條線議定）**：自明天起
  GUI 線用 `g1 g2…`、機器線用 `m1 m2…`；裸字母（a–q）到 09-04 為止不再新增。

### ✅ 脫附 → 安裝 `arm_deploy_f` 代轉 → 重新吸附（per user 2026-09-04）

🔴 **刻意先走設計好的脫附路徑，不讓 `cmd_shutdown()` 去拔繼電器。**
`cmd_shutdown()` 只是把 `CH_VALVE`／`CH_PUMP` 直接關掉——**沒有洩壓、沒有 CH6 正壓脈衝、
沒有兩段收回**。而 `pusher all retract` 本來就內建那整套。所以順序是：

1. `pusher all retract` → OK（吸盤依序釋放、推桿兩段收回）
2. `pump off` → `OK ch2=0`
3. 複驗：**16 個繼電器全 0**、`p5~p8 = 0/1/0/0`（回大氣）、roll 0.41° 未變
4. `echo exit > wr_0904_in` → **12 秒後 :5001 關閉**、行程數 0（比 runbook 記的 5 秒久）
5. 換 binary（舊版留 `facade_cleaning_v2.prev_0904`）→ `launch.sh` 重啟
6. `pump on` → `vacuum all on` → `pusher all extend_raw 10` → 重新吸附

📌 **真空是慢慢建立的，不要用第一筆讀值判斷**：`extend_raw` 回 OK 當下讀到
`p5=-40 p6=-51 p7=-59 p8=-54`，4 秒後才穩到 **-66/-69/-68/-68**。
第一筆的 `p5=-40` 已經在 ③a 的 `-50` 判準之外——**若當下就判，會誤判成一顆沒吸到**。

✅ **重啟後 `[OK] arm 127.0.0.1:9527` 一次就連上**（早上冷啟動時是 `not yet reachable`，
因為當時 motor_api 還沒起——順序不同而已，不是故障）。

✅ **`arm_deploy_f` 代轉端對端通過**（走本體 :5001 → motor_api DEPLOY_F）：
```
arm_deploy_f 15 RIGHT -> OK tau=14.99 target=15.0 theta=0.6205 cmd=0.8657
                            contact=0.5854 kp_eff=69.4 iters=1        [16s]
```
`theta=0.6205` 與直連 9527 以及 `DEPLOY 520` 的落點**完全一致**（第三次獨立重現）。
代轉的 ENABLE 前置也生效了（呼叫前手臂是 PARK＝失能狀態）。
參數驗證 3/3：`ERR usage:...` / `ERR invalid_slot` / `ERR invalid_target_nm`。

⚠️ **`water_inlet off` 在關機路徑連試 3 次都失敗**（`ERR pqw_water_offline`，最後
`valve state UNKNOWN`）。**不是本次造成的**——PQW slave 12 從今早冷啟動就不在匯流排上
（`[ERR] [PQW:12] init presence probe failed`，與 09-03 兩份 log 逐字相同）。
📌 但這代表**每次正常關機都會留下一個 "UNKNOWN" 的水閥狀態**，而水路真接上之後這會變成真問題。

🧹 順帶清掉 5 個殘留的 `sleep infinity` FIFO holder（今天換了 6 次 binary 累積的）。
📌 **辨識法**：holder 是 `exec sleep infinity > fifo`，所以 `readlink /proc/<pid>/fd/1`
就是它守著哪一支 FIFO —— 靠這個才能只殺舊的、留下現行程式那兩支。

### 🔴 per user 摸出手臂在抖 → 50Hz 曲線抓到加載/卸載振盪（09-04）

per user 原話：「手臂靠上之後 M1 有在抖動，類似重試」→「**重試壓在牆上的力道的感覺**」。
**這個描述精確到可以直接對到程式**，見 changelog `[2026-09-04d]`。

- 命令值在 0.60~0.68 之間來回、tau 在 −1.9~+3.5 振盪、約 0.8 Hz，走 6~10 步振盪 6~10 次。
- 根因：`move_to_slot()` 每次把 ramp 起點設成**實際位置**，手臂被玻璃擋住後
  每步命令值先塌回再爬 ⇒ **力被完全卸掉再重新加載**。
- ✅ 接觸區改推 `hold_pos`；自由行程仍走原路（hold 分支沒有速度安全閥）。
  `cmd` 往回退 8 次 → 1 次；壓上耗時 15.8s → 10.6s。

📌 **兩個方法論收穫**：
① **我稍早調 `SEEK_SPEED` 時把成因寫錯了一半**（只看到「ramp 距離變長」，
   沒看到「力被卸掉」），所以那次改動只讓振盪變快。**先量再改**又應驗一次。
② **排除候選只花了幾秒**：hold 迴圈的 passive watchdog 形狀跟「重試」一模一樣
   （條件 `|hold_pos−live_pos|>0.1` 在壓住時恆成立，觸發會 `enable()` 阻塞 100ms），
   但它**會寫 log** ⇒ 直接 `grep` 五份 arm log，**0 次**，當場排除。
   **會留下痕跡的假說要先查痕跡，不要先改程式。**

### 🎯 力控路徑首次耐久：20 輪零中止（09-04）

`arm_cycle.py 20 15 RIGHT 100`，紀錄 `~/bringup/cycle_logs/arm_0904_force20.log`。

| 項目 | 值 |
|---|---|
| 壓上 tau | **15.48~15.78**（散布 0.29） |
| 壓上 θ | 0.6243~0.6270（散布 0.0027） |
| 壓上耗時 | 11.8~12.2s |
| 滑台 0→100→0 | **3.1 / 3.1s 二十輪完全相同** |
| 吸盤 | −66/−69~70/−68/−68，**二十輪零衰減**（連續吸附約 8.5 分鐘） |
| 全輪 | 24.7~29.0s |

`iters=1` 全程；`rail` 40 次全過（早上那次單發 `ERR rail_move_command_failed` 未重演）。

🔴 **但它系統性地壓過頭 0.68 N·m**：目標 15.0、穩定落在 15.68。
在容差內（`TOL=1.0`）所以回 `OK` 就收工，**但二十輪全部偏同一邊 ⇒ 不是雜訊，
是割線法一步修正的系統偏差**（`kp_eff` 用 70 起手，目標附近實際斜率略高）。
實務上多壓 4.5% 無害，但「設 15 拿到 15.7」不該藏在容差裡。
🟡 **待決**：`TOL` 1.0 → 0.4 會多跑一次迭代（每次約 +1.5s）。**尚未改，等拍板。**

🎯 **一個力控才看得出來的東西**：θ 單調 0.6243→0.6270（+0.0027）而 **tau 幾乎不動**
⇒ **同樣的力，滾筒每輪貼得更深一點點 ＝ 真正的磨合。**
開迴路看不出這個：早上那十輪 θ 也在爬（0.6193→0.6209）但 tau 同時在掉
（15.19→14.99），**兩者混在一起分不出是磨合還是牆距**。
**力把一個變數固定住之後，位移的變化就只剩一個解釋。**

### 🔴 M2 的保持力連自己的靜摩擦都克服不了（09-04 per user 手扳量出來）

per user 要求「開機後兩顆都要使能，手應該要扳不動 M1/M2」。實際扳完量測，
**兩顆的結論完全不同**。

**M1**：per user 扳到極限 `pos=0.1471~0.1551`（由 0.002 起算共 **0.153 rad = 8.76°**），
反抗力 **8.9~9.6 N·m**。逐點驗算 `tau = hold_kp(90) × (hold_pos − pos)`，
14 個取樣點誤差全在 **±0.46 N·m** 內 ⇒ **馬達完全照設計出力，沒有異常。**
- 📌 「感覺鬆」的來源是 `hold_pos = 0.05`（`PARK_STOP_MARGIN`）而非 0：
  手臂靜止時靠在停點 0.002、被馬達用 4.3 N·m **頂著停點**，往外推的前 2.7°
  反抗力是**遞減到 0** 的，之後才開始擋。
- 🔴 **「手扳不動」對 M1 不是調參數能達成的**：手約可出 9 N·m，要壓到 1° 內需
  `kp ≈ 514`，而 **MIT 協定上限是 500**；且 kp=40 在 2026-08-14 就發生過失控震盪。
  ⇒ **要真正扳不動需要機械煞車或自鎖，不是控制參數的事。**

**M2**：per user 扳到 `pos=0.4339`（目標 0.5316，偏差 **0.0977 rad = 5.60°**）後放手，
**40 秒逐秒回讀 40 筆全部 0.4339，一格都沒動。** 逐項對帳：
```
kp*err          = 7 x 0.0977 = 0.68 N·m
積分上限 ki*IMAX = 0.6 x 2.0  = 1.20 N·m   （tau 1.46→1.94 的爬升就是積分充電，~9 秒）
保持力上限                    = 1.88 N·m   ← 實測平台 1.85
M2 靜摩擦（09-02 實測）        ≈ 2.0~2.3 N·m
```
🔴🔴 **1.88 < 2.0 ⇒ 推不回來，而且是永遠。** 不是「回得慢」，是回不去。

🎯 **這解開了 09-03 記下但未查明的那條**：「滑台掃動期間工具頭被摩擦力扭轉
0.45~0.49 rad（26~28°），十輪高度一致＝固有特性」，當時只註明「會直接影響清潔
接觸角、先前無紀錄」。**根因就是這個——摩擦帶走它，馬達扛不回來，帶到哪留到哪。**

📌 **一句話的設計不一致**：移動時 `MIT_KP=31` **推得動**，保持時 `hold_kp=7` **撐不住**。

✅ **修正（per user）：`m2_.hold_kp` 7 → 31**（取與移動同值，「推得動就該待得住」）。
- `park_kp` **刻意不跟著改**（原本是 `= hold_kp`）——`cmd_init_sequence` 的
  `go_home_slot(m2_)` 預設吃 park profile，本次只改保持行為。
- 📌 **與既有紀錄不衝突**：檔內原有「kp 7→20 位置幾乎不動」那段量的是
  `lr_move_to_slot_impl` **移動後的落點誤差**（位置受限）；本項改的是
  **保持階段能否抵抗外力**（力矩受限）。**兩個不同的量。**

**實測效果（同高度同工具，掃動一輪）**：

| | `kp=7`（50 輪基線） | `kp=31` | |
|---|---|---|---|
| 出到 100cm | 0.6708 | 0.6121 | 往前帶 +0.147 → **+0.079** |
| 回到 0 | 0.2291 | 0.4732 | 往回帶 −0.442 → **−0.139** |
| **一輪淨偏移** | **−0.295 rad（−16.9°）** | **−0.060 rad（−3.4°）** | **↓80%** |

⚠️ **這是清潔幾何的改變**：先前每掃一趟工具頭被帶走 17° 且不會回來 ⇒
**第 2、3 趟的接觸角都不一樣，而那從來沒人量過。** 現在每趟殘餘 3.4°。
🟡 3.4° 可不可接受**待 per user 確認**；要更小得繼續調 kp，但 M2 有過熱鎖存前科
（09-03 夜間只能斷電解除），不宜一次調太多。
🟡 上述為 **n=1**，正補跑 5 輪確認重複性。

### ✅ 開機自動就緒上線（per user 2026-09-04）

`motor_api` 啟動後不再被動：**上電 → M1 回機械零點 → M2 預設滾筒(RIGHT)**。
```
[STARTUP] OK STARTUP m1_pos=0.0032 m2_pos=0.5186 slot=RIGHT en=1,1
```
- 🔴 **刻意只做 `go_home` 不做 `CALIBRATE`**：後者會驅動手臂撞機械停點來重新定義零點，
  屬明確的操作者動作，不該在每次行程啟動時自動發生（今天為了換 binary 就重啟 8 次）。
  零點活在馬達驅動器裡、跨重啟保留。要重新定義零點請明確送 `INIT`。
- 🔴 **M1 沒回到零點就不動 M2**：工具頭在手臂還伸著時旋轉會刮傷玻璃與工具
  （與 `LR_SLOT` 自己那道「M1 未離開玻璃」守衛同源）。
- ⚠️ **副作用：手臂會在行程啟動時自己移動。** 若重啟當下手臂正壓著玻璃，它會收回。
  `ARM_NO_AUTOSTART=1` 可停用。
- 🆕 **`arm_status` 每顆補 `en=0|1`**：失能的馬達不再送 CAN frame，`pos`/`tau`/`err`
  全部是**凍結的舊值**。09-03 因此誤判三次、09-04 又撞一次。
  📌 順帶釐清：PARK 後常見的 `err=0x1` 就是凍結值，**不代表它還使能**；
  而先前我說「PARK 只 disable M1、M2 沒有」是**錯的**——`disable_slot(m2_)`
  在 `main_api.cpp:3257`、無條件執行，**PARK 是對稱的**。

### 🎯 兩個守衛拿到實測依據（09-04 下午，per user 全程在場）

**做法：機體逐段上移，每個高度重新吸附後用低目標 `DEPLOY_F 3.0` 輕觸，只讀 `contact`。**
低目標是刻意的——**碰到就停，不把 15 N·m 壓在未知物體上**。

| 位置 | `length` | `contact`(滾筒) | 是什麼 |
|---|---|---|---|
| 底部 | +3/+5 | **0.5842** | 玻璃 |
| 中段 | −50/−52 | **0.5449~0.5510** | **橫桿**（滑台 0/50/100 全線都是） |
| 上段 | −129/−130 | **0.6086** | 玻璃 |

✅ `DEPLOY_F_THETA_MIN` **0.45 → 0.565**（窗口 0.5510~0.5842 的中點，寬 14mm）
✅ `DEPLOY_F_THETA_MAX` **0.95 → 1.10**（實測刮刀需要 0.9581，被 0.95 擋掉）

🔴 **玻璃往上越來越遠**（0.5842 → 0.6086）⇒ 不會往橫桿那側靠，窗口不會被吃掉。
⚠️ **只有兩個玻璃點、都在下半部。** 上半部若有更近的玻璃會誤擋。

### 🔴🔴 橫桿讓 `obstacle` 的處置整個翻案（同日稍早才寫的邏輯）

per user 確認中段那個是**橫桿**——這面牆的固定結構，一趟下行必然遇到幾根。
**我把它當成異常了。** 那跟「有些玻璃面有縫隙、吸盤本來就吸不住」是同一類現場條件，
而那一條 per user 當初明講是「設計要求，不是妥協」。
照我早上寫的邏輯，**十週期遇到第一根橫桿就整場中止**。
✅ 改為「收手臂、跳過清潔、計數、續行」，`obstacle_steps` 與 `no_wall_steps` **分開記**
（成因相反：一個是牆太遠、一個是有東西比玻璃更近，混在一起看不出牆的形狀）。

📌 **這件事只有上機、而且有人在旁邊指認才會發現。** 純看程式碼，`obstacle` 當異常
處理看起來完全合理。

### 📉 壓上到滑台起動 13.59s → 10.78s

per user 回報「靠上牆要過很久滑台才會動」。量測後砍掉兩段**重複**的等待：
腳本 `settle()` 輪詢（−2.0s，`motor_api` 已經等過鬆弛並回報穩定值）、
收尾複驗 1500→300ms（−1.2s，與迭代裡剛等過的重複）。
🆕 回覆新增 `ms=`——**這件事先前帳面上完全看不到**。

⚠️ **`kp_eff` 依工具分開那一項我推論錯了，照實記**：以為 `iters` 永遠是 2 是起手值不準，
改完**準度改善但迭代次數沒降、有一次還變 3**。真因是**尋觸停在 2.0 N·m 而目標 15.0，
中間 13 N·m 要靠跳補完而那段非線性**。起手值調高只是讓第一跳由過頭變成不夠。
🟡 真能省一輪的做法（未做）：讓尋觸走到約 0.7×目標才交棒（每步 150ms vs 每迭代 1500ms）。

### 📊 力控耐久累計（今日）

| 測試 | 輪數 | 結果 |
|---|---|---|
| 滾筒單工具 | **50**（20+30） | 零中止；tau 15.3~15.8；吸盤連續 22 分鐘零衰減 |
| `kp=31` 扭轉重複性 | 5 | 淨扭轉 −2.6~−3.1°（`kp=7` 基線為 −16.9°） |
| **雙工具（滾筒+刮刀）** | **10** | 零中止 |

雙工具 10 輪：滾筒 tau 15.38~15.97 / θ 散布 **0.0008** / 淨扭轉 **−2.4~−2.9°**；
刮刀 tau 15.09~15.29 / θ 散布 0.0038 / 淨扭轉 **−6.5~−7.1°**。
🔴 **刮刀的扭轉穩定是滾筒的 2.5 倍**（散布僅 0.6°，不是偶發），每輪收在 −1.117~−1.124、
穩定越過 `lower_bound=-1.05` 約 0.07，**但不累積**（每輪 `DEPLOY_F` 會先把 M2 拉回 slot）。

🎯 **力控才看得出來的**：兩個工具的接觸角不同（工具伸出量差 12mm）**但都收斂到 15 N·m**
——不需要為每個工具維護一組 `wall_mm`。

### 🔴 收回一個我自己的建議：`LR_CALIBRATE` 不能拿來「量一下停點」

我一度把它推薦成「~3 分、安全」，**那是在還沒讀那支函式的時候**。實際上它是
**兩側都撞 → 算中點 → `set_zero_position`**（`main_api.cpp:496`），會重新定義 M2 座標系。
而 `lr_move_to_slot_impl` 有兩套座標分支（`lr_stop_valid` 真/假），現在跑的是 09-02
斷電手轉的**絕對值**那條；跑完會切到另一條、零點還被移動過 ⇒ **今天所有 M2 數字全部要重新對應**。
🔧 要量負向停點，用 09-02 那個已驗證的方法：`M2 DISABLE` → 手轉到底 →
`M2 MIT 0 0 0 0 0` 刷新快取 → 讀值。**不改零點、不撞停點。**

### 📌 兩個順帶觀察

- **左右張力差與「是否吸附」有關，不只是重心**：底部吸附時 52.5/36.7（差 15.8）、
  懸空上升後 47.7/48.8（**差 1.1，幾乎相等**）、下移 10cm 後 50.2/43.8（差 6.4）。
  ⇒ 09-01「重心偏左」那條要修正——**懸空時重心其實是平的**，差值來自吸附時推桿頂著玻璃。
- **上段有一顆吸盤吸不住**（`p5=0`，右上；其餘三顆 −67~−69 正常，三次讀值一致）。
  符合「玻璃有縫」的現場條件，③a 的「至少一顆」判準會放行。

### 🎯 `cycle_test` 完整週期首次跑通（09-04 收盤）

`cycle_test.py 1`：5 步 × 40cm 下行 + 213cm 回程，`=== 1 個週期全部完成 ===`。

| 步 | 清潔s | 四顆壓力 | 達標數 | Δ |
|---|---|---|---|---|
| 1 | 23.4 | −43/−60/−49/−51 | 2 | 6 |
| 2 | 11.5 | −51/−34/−1/0 | 1 | **8** ← 橫桿，跳過清潔 |
| 3 | 25.4 | 0/0/−54/−53 | 2 | 5 |
| 4 | 29.0 | −26/−37/−51/−41 | 1 | 5 |
| 5 | 11.2 | −43/−50/−56/−52 | 3 | 3 ← 橫桿，跳過清潔 |

- 🎯 **roll 自動修正 2 次觸發、全部成功**。步4 那次**第一道修正過頭、反向到 +4.24°，
  第二次才收斂** ⇒ **迭代設計是必要的**，寫成「修一次就算」那次會失敗。
- ✅ **連續判定擋掉 1 次假警報**。
- 🎯 **`obstacle` 2/5 步**，接觸角 0.5392 / 0.5457 —— 比上半部那根（0.4961~0.4988）
  遠 0.045 ⇒ **是不同的兩根橫桿**，都被門檻 0.5650 正確攔下。

**過程中修掉我自己的兩個實作缺陷**（都在「修正成功之後」那條路上）：
① 剛好不需補完距離時 `res` 沒更新 ⇒ **修好了卻仍中止**；
② **一次狀態讀取失敗就放棄整個修正** ⇒ 階段 2 首跑時 `roll_correct` **實際已成功**
（roll −6.94° → +1.37°）卻中止整場。已改為重試 3 次並印出原始回應。
📌 兩者都只在很窄的條件下觸發，**都是靠 `1 2 40` 的小測試 3 分鐘撞出來的**。

### 📌 快送提速：實測後建議不動（per user 指示先不動）

per user 規格：快送 30~50Hz、調整 10~20Hz。現行全部合規但**貼在下限**。

| 理由 | 數字 |
|---|---|
| 30Hz 已摸到 8cm 門檻 | 1/6 段達 8cm，韌體上限 10 |
| 50Hz 實測 9cm（09-01） | 距上限只剩 1cm |
| **移動根本不是瓶頸** | 單步 45.6s 中，移動 **11.4s（25%）**、**滑台清潔 20.1s（44%）** |

⇒ 提速最多省每步約 4.5s，但左右差逼近韌體上限、roll 修正次數上升（每次 10~15s）。
**真正該優化的是滑台那 20.1 秒。**

### 🔴 兩件待 per user 拍板

1. **後端沒有「腳本正在跑」的訊號**（由 facade web gui session 指出）：
   `cycle_test` 不用 `step_*`，而 `Running` 只有 `step_*`/`cross_obstacle_*` 會設
   ⇒ 全程 `state=idle`。實測**腳本啟動到第一次 `pay_out` 約 33 秒完全沒有鎖**，
   而那正是推桿／真空／手臂在動的階段。前端的鎖擋不住另一分頁／另一台電腦／直接 TCP。
   🟡 乾淨解法是後端「作業佔用」旗標，但牽涉安全語意（哪些指令要擋、`STOP`/`PARK`
   是否豁免、**異常結束怎麼釋放**——一個沒釋放的鎖會把機器鎖死到重啟）⇒ **未做，等拍板。**
2. **③a 的「至少一顆」判準**：五步達標數 2/1/2/1/3，**沒有一步四顆全中**，
   而相鄰 40cm 的步2/步3 吸附位置**完全對調**（上兩顆 vs 下兩顆）。
   09-03 定這條的理由（「玻璃有縫是現場條件不是故障」）今天被證實得非常徹底，
   但它也意味著有些步驟是靠單顆吸盤附著。要不要改成「至少兩顆」待決。

### 待完成

- 🟡 **15 N·m 對刮刀可能不是對的目標值**（新增）：刮刀在 `length=-129` 壓不到 15
  （需要 theta 0.9581）。刃口軟、設計上也許就該貼得淺。若是，正解是**分工具設定目標壓力**，
  比放寬 `theta_max` 更根本。放寬只是先解開卡住。
- 🟡 **`theta_max=1.10` 仍無上界實測**：今天最遠只量到 0.9581。
- 🟡 **玻璃接觸角只有下半部兩個點**：上半部若更近會讓 `theta_min` 誤擋。
- 🔴 **`wall_mm` 隨高度變，目標維持 tau≈15**（目標值已定，做法未定）：
  查表？用 SD76 高度算？每個高度先探一次？**至少要在頂/中/底三個高度各實測一次 `wall_mm` ↔ tau**，
  才知道那條關係是不是線性、能不能用一條公式帶過。在那之前 `cycle_test` 用的仍是固定 520。
- ~~🔴 **`cycle_test.py` ③b 的 `tau > 8.0` 門檻要一起重新決定**~~
  → ✅ **09-04 已決定並實作：整條換掉，改讀 `DEPLOY_F` 的回覆**（見上方決策段）。
  ✅ **09-04 代轉已安裝並端對端驗過**（脫附→換 binary→重新吸附，全程見上）。
  🔴 **但 `cycle_test.py` 本身還沒實跑過新 ③b** —— 自動退回那條分支現在也不會再觸發了。
- 🔴 **回到 09-03 量 14.31 的那個高度重跑一次三點**，才能把「25% 偏移」與「牆距」畫上等號。
- 🔴 `.22` 匯流排本身仍未修（現只剩 JC100 ×4，等下次長測看它獨處穩不穩）。
- 🔴 整合後的 ③b（清潔動作）仍未跑完一輪。
- ⚠️ `main` 仍領先 `origin/main` 三個 commit（`270f2ee`／`1a65387`／`844691f`），未推。
- ~~📌 runbook §0 要補一條：**本地 console 只認 `exit`/`quit`/`status`，其餘靜默丟棄**~~
  → ✅ **09-04 已補**（§0 第 3 條，含「程式起來 → 先設參數 → 才讓人碰 GUI」的順序要求）。

**console v2（`web_backend/`）這條線的待辦：**
- 🔴🔴 **console v2 從上線到現在從未經瀏覽器實測** —— HTTP 200 + 標題正確就是目前
  全部的驗證程度。按鈕點下去的真實行為、版面在實際視窗寬度下的樣子，**沒有人看過**。
  新增的手臂面板同樣如此（解析邏輯已用實機真實回覆逐條驗過，但那不等於畫面是對的）。
  ⇒ **這是本條線最大的未知，其餘每一項判斷都建立在它之上。** 需要使用者開瀏覽器。
- 🔴 **`DEPLOY_F` 的 `WARN` 分支上游也沒能在硬體上觸發過**（割線法一步就進容差）。
  ⇒ GUI 的「未收斂」四態顯示**同樣未經實證**，我是照文件格式寫的。
  📌 **合起來看：未收斂的顯示路徑，從 motor_api 到 GUI 沒有任何一段被真的走過。**
  要驗得刻意製造不收斂條件，目前沒有安全做法 —— 使用者實測時要明講，
  **不要讓它混在「已驗證」裡**。
- 🔴 **`server.js` 是否要根治 keepalive 冒充回覆（需要決定）**：正解是橋接把自己
  `ping` 的配對回覆吃掉不 broadcast。**沒動的理由是 8080／8081 共用同一支**，
  而 8080 不受此 bug 影響 ⇒ 根治只對 v2 有實益、卻要動生產介面的路徑。
  目前前端已用簽章表擋住（`OK pong` ／ arm 的 `ERR usage: M1...`），
  🟡 **代價是簽章表是寫死的**：日後 `ping` 的回覆字串一改，擋不住且不會有人知道。
- 🟡 **「攔下 keepalive 冒充」那條紅字尚未真的觸發過** —— 要它出現得剛好撞上在途指令。
  ⇒ 這條保護本身也是未驗證路徑。
- ✅ ~~**運動中動態提高輪詢率**~~ → **09-07 前端已做並部署**（`g5`）：閒置 1s、運動中 250ms，
  附防堆積（佇列排隊長度無上限，堆起來沒有徵兆）。查證過**不增加 `.22` 匯流排負載**。
  🔴 **只解掉一半**：「16 秒回程 16 個取樣點」解了；**「③→④ 密封窗口」沒解**——
  `cmd_attach` 成功後才 `set_state_`，全程 `state=ready`，**沒有任何訊號指出 attach 正在進行**。
  ⇒ 已在 `m3` 補 `status` 的 `step_in_progress` 欄位（**待 VS 重建**），前端已先讀它、屆時自動生效。
- ~~🟡 **Dashboard 的 roll 中止門檻 `6.0` 在 `index.html` 寫死兩處**~~
  → ✅ **09-07 處理，但本條的描述本身就過期了**：渲染路徑那兩處（`/6.0` 與 `(6 - …)`）
  09-04 當日稍後就已收斂成 `ROLL_TRIP` 常數，本條沒跟著更新。09-07 真正找到並修掉的是
  **第三處漏網的**——`6°` 寫在起跑檢查的**說明字串**裡，因為它不在計算式中所以逃過了那次收斂。
  🔴 **但這條和 `step_cm_max` 不同，不能用同一招收尾**：roll 門檻是 `cycle_test.py` 的
  **執行期參數**，兩個後端都不知道它 ⇒ **GUI 沒有任何管道查得到**，「改成讀後端」做不到。
  要真解決得讓 `cycle_test` 起跑時把參數回報出來（EVT），屬 C++/Python 那條線。
  在那之前 `ROLL_TRIP = 6.0` 就是一份**無法查證的複本**，已在該處註解寫明。
- ~~🟡 **8080 現行 GUI 的 `public/` 在 Pi 上落後 repo**~~ → ✅ **09-07 已部署並端到端驗證**
  （md5 兩邊逐位元一致、`curl` 確認服務出去的內容已換）。
  ✅ `刷洗滾筒 (CH15)` → **CH5**（操作者看得到的錯標籤）。
  🔴🔴 **部署前攔下一件事：直接照 repo 部署會製造新的謊。** repo 的 `app.js` 有一個
  hunk 不是註解而是行為改變——把腳本預覽上限由 `50` 改成 `100`，註解還寫著「對齊後端的
  `STEP_CM_MIN..STEP_CM_MAX (5..100)`」，**而後端 08-31 已降回 45**。照原樣部署等於把
  「Pi 說 50 / repo 說 100 / 後端說 45」三邊不一致原封不動搬上機。
  ✅ **改成向後端取值**（見 09-07 條目），不是改成 45。

---

## 2026-09-03 — 開工盤點：翻出三份沒進日誌的 09-02 週期測試，真空假說被推翻

### 已完成

- **版控收斂**（`agent_ai` 端）：`fix/driver-crc` 10 個未推 commit 已推上 `origin`（`c015509..74ce8be`）；
  本機 `main` 已 **fast-forward** 到 `74ce8be`（136 commit，零衝突、工作區未動）。**尚未推 `origin/main`**（等使用者拍板）。
  🔴 **順帶更正一條過期記載**：上層 work_log 記「`refactor/app-layer`／`fix/msg-nosignal`／`docs/single-dev-restructure`
  三條仍未推」是錯的 —— 實測三條**全都是 `fix/driver-crc` 的祖先、早就在遠端**。
  唯一真正獨立的是 `main-final-harness`（獨有 2 個 commit），而它是「僅供等價比對用」的 harness 基準分支，**本來就不該併進主線**。

- 🎯 **翻出 09-02 三份沒進日誌的週期測試，其中一份正是待辦表在等的那一輪**（詳見待辦表該列，已結案）：
  `0902b`/`0902c`/`0902d` **三輪全部開著真空 A 組**，`0902d` **10/10 跑完 60 段**。
  **有真空（中位 3 / p90 4 / 最大 5、≥6cm 0%）比無真空對照組（3 / 5 / 6、3%）還略好** ⇒
  🔴🔴 **09-02 寫下的「吸附/脫離是嫌疑最大的變因」被推翻。**
  📌 **教訓：結論寫進日誌之前先確認當天的原始紀錄都收完了** —— 這個假說多活了一天，
  而否證它所需的資料在寫下它的**同一天下午**就已經產生了。
  ⚠️ 另外兩輪的中止（`extend_raw TIMEOUT`、`pwm_freq_write_failed_no_reply_timeout`）都是**裝置通訊故障**，
  與姿態無關；後者正是待辦表「QX-DO24 間歇性全滅」那條在 12:45 當場發作，13:45 的 `0902d` 已恢復，與該列記的「13:22 起好轉」時序吻合。

### 上機（09-03 上午）

**✅ `cmd_pump` 補回讀已上線並雙向驗證** —— 改走既有的 `pqw_set_relay_verified_`（回讀 + 最多 3 次重試），
回應改為 `OK ch2=<實際值>`。實測 `pump off`→`OK ch2=0`、`pump on`→`OK ch2=1`，兩次都以獨立
`relay_status` 交叉確認。本體 16/16 TU 重建、部署（舊版留 `facade_cleaning_v2.prev_0903`），
以「新 binary 有 `pump_set_but_readback_fail` / 舊的沒有」做正負對照。

**🔴 水平點量測（L≈235、貼牆）與一個更麻煩的發現**
- 貼牆斜率 **−0.888 °/cm**（自由懸吊既有值 ≈ −1.1，差 19%、同量級）
  ⇒ **貼牆沒有讓「繩長差 ↔ roll」失效，09-01 的舊資料可謹慎沿用**，不需重跑 185cm。
- 該高度水平點 **L−R = −0.13**（舊座標），對照 L≈74→+5、L≈50→+3.8
  ⇒ **跨行程差 5cm 以上，`level_diff` 是高度的函數不是常數**（原待辦的主問題已有答案）。
- 🔴🔴 **但有遲滯**：復原到**完全相同的繩長** `L=235 R=236` 之後，roll 由 **+0.77° 變 +0.36°**、
  張力差 22.5 → 17.4 ⇒ **同一高度的水平點本身就不唯一，帶約 ±0.4°（≈±0.5cm）的不確定**。
  而規格是 ±1° —— **遲滯就吃掉 40% 預算**。⇒ 剩下兩個高度要量的話，**必須從 + 與 − 兩個方向各逼近一次**，
  得到的是帶寬不是點；單向探一次會把遲滯誤讀成水平點。
- ⚠️ **自記操作失誤**：復原那步 `pay_out_right 3` 實走 5cm，把機器帶到 roll +3.05°，
  **腳本被自己的中止門檻攔下、平衡迴路留在關閉狀態**。已用 `retract_right 1` 復原到原始繩長並重開平衡。
  📌 教訓：復原步驟要用「回到原始讀值」的閉迴路，不能用「反向送一樣的公分數」。

**🔴 滾筒模式 1 輪：三個數字偏離，四個候選逐一排除**
今天 θ=0.6254 / tau=+14.60 / `Δθ@130`=+0.0332，對照昨天 0.6548 / 11.48 / +0.0080~+0.0172。
吸盤（−66~−69）、掃動（3.7/3.7s）、M2 擺動（0.420）**全部複現**，只有「壓上」偏掉，且三個數字同向
⇒ 玻璃近了約 15~18mm（用續十五「Δθ 0.008 ≈ 4~5mm」換算）。
- ❌ **牆面不同高度不同** —— per user 排除（牆面相同）
- ❌ **ZDT 沒到位** —— per user 排除，**並由日誌回溯證實**：四顆 `wait_many` 全部 `pos≈3000.0°`＝正好 10cm
- ❌ **滑台零點漂移** —— 手推到左端硬限位後 `rail_pos` 仍是 0，當場驗掉；`rail_zero` 已重寫
- 🔴 **剩 M1 自己的零點／重複性**：同一個 `PARK`，今天啟動時停 **0.0250**、稍後停 **0.0467**，
  **差 0.0217 rad ＝ 與 θ 偏差 0.029 同量級**。M1 在 HOLD 分支無摩擦前饋、`hold_ki=0`（09-02 刻意移除），
  停位由靜摩擦決定。且 `motor_api` 今日重建重啟、Pi 也重開機過，**全程沒跑 INIT**。
  🔧 下一步是跑一次 `INIT` 把 M1 零點重設在機械停點，之後 θ/tau 才跟昨天可比
  ——**未執行**，因為 seek 方向是否朝玻璃尚未確認。
- ⚠️ **跨 session 比較 θ 到 0.03 rad 這個精度，可能本來就不成立。**
- 🔴 **`cyc.py` 沒有 tau 上界**（只有 `tau > 8.0` 的下界用來確認「有壓上」）⇒
  「玻璃更近 → 壓更重」這條路徑上跑 10 輪不會中止。已據此**沒有續跑 10 輪**。
- ✅ **驗證到一件好事**：壓住後 20 秒內 pos/tau **完全不動**（0.6308 / 14.0171）⇒
  09-02 移除 `hold_ki` 確實讓「貼合」變成穩定狀態（舊行為是 10 秒內 tau 由 +0.24 爬到 +1.51 且未停）。
  ⚠️ 但 tau 九次取樣逐位元相同，不排除是保留值而非即時值，未進一步確認。

**🔴 推桿分組：今天量到的是「上/下」，不是待辦表記的「左/右」**（單次 extend 的 `wait_many`）：
右上(5) 3600ms/994mA、左上(6) 3450ms/950mA vs 右下(7) 1650ms/874mA、左下(8) 1500ms/857mA
⇒ **上面兩顆是下面兩顆的 2.2 倍，左右幾乎沒差**。09-02 記的是右 2025/2400 vs 左 1650/1500（1.4~1.6×）。
⚠️ **n=1、情境不同**（09-02 是每顆 50 次），不足以推翻該列；但足以說明**分組可能不固定**
→ 🔧 待辦表那條的分辨方法「用尺量四個吸盤到牆面的間距」**四顆要各自量，不要只比左右**。
📌 順帶更正我當時的誤判：我一度說 `extend_raw` 沒有位置回讀、建議比照 `cmd_pump` 補 —— **錯的**，
回讀在 `wait_many` 那行，只是不在 `pusher` 那行。

**🎯 SD76 重新歸零，座標系換成「跟著玻璃走」（per user）**
- **`L=0` ＝ 玻璃面最高點**、**`L≈223`（右 218）＝ 玻璃面最低點** ⇒ 玻璃面高度約 **223cm**
- 🔴 **`home_ground_cm` 仍是 0，沒被記錄**：`zero_meters` 的 `top` 模式會在歸零前把左側當下讀值
  存成「頂端到地面的繩長」，但要在**讀值還帶著行程時**執行；歸零後再下只會存到 0。
  ⇒ 要補的話，得等下次從地面升上來、讀值還在的時候再下 `zero_meters top`。
- 🔴 **`cycle_test.py` 的 `TOP, BOTTOM = 0, 229` 與新座標不符**（實測底是 **223**）——動它之前要先對齊。
- ✅ **今天的水平點換算並被獨立驗證**：歸零前同位置 `L−R=−1`、水平點 −0.13 ⇒ 高於當下 0.87cm
  ⇒ **新座標水平點 ≈ `L−R = +0.87`**。到底後實測 `L−R=+5 / raw roll −3.77°`，
  用今天的斜率反推 `−(5−0.87)×0.888 = −3.67°` ⇒ **差 0.1°，而且是跨（貼牆→自由懸吊）狀態對上的。**

**🔴 機上有兩批腳本不在版控**：吊機 `~/bringup/cycle_test.py`（09-02 09:51，18,764 bytes）
**比 repo 的 15,777 還新**；本體 `cyc.py` / `cyc10.py` / `cyc20.py` 全部只在 Pi 上
（且 `cyc10.py` 與 `cyc20.py` **內容完全相同**＝重複檔）。`cyc.py` 本次已補上「吸盤壓力讀不到＝視同失敗」
（原本 `bad` 會是空 list ⇒ **讀不到會被當成沒失壓而放行**）與「週期數可由 argv 指定」。

### 上機（09-03 下午）：滑台步驟上線、真空時序查明、但 10 趟兩次都被 QX-DO24 打斷

**✅ `cycle_test.py` 新增步驟 ③b：上滑台 `0→50→0`（per user）**
放在 ③（推桿伸出）與 ④（收回）之間 —— 推桿仍在 10cm（機體有支撐）、風扇仍關
（① 的關風扇順序是安全需求，不可為了掃動提前開）、手臂維持 `PARK`（per user：只動滑台）。
🎯 **實測完全可重複：五步都是 4.4 秒，逐次相同。**
另 `BOTTOM` 由 **229 → 223**（per user）：SD76 當日以玻璃面最高點重新歸零，舊值屬前一組零點，
留著等於允許放繩到玻璃底下約 6cm；它是區間守衛不是顯示值。

**🔴 真空是「讀太早」不是「沒碰到玻璃」—— 我判斷錯，per user 當場更正**
首輪四顆壓力全程在 0 附近，我判成「推桿伸進空氣裡沒碰到玻璃」。**per user：有碰到，是時間不夠。**
原程式 `extend_raw` 一回來就立刻讀 `status`，讀到的是還沒抽起來的瞬間值。
✅ 依 per user 指示改成**等真空建立、至少一顆 ≤ −50 kPa 即通過**（`VAC_OK_KPA=-50`／`VAC_WAIT_S=10`），
並新增「真空s」欄把等待時間量出來。**實測真空建立需 1.2~2.9 秒。**
📌 **「至少一顆」而非「四顆」是對的**：實測出現過 `0/1/-61/-59`（兩顆完全沒吸上、兩顆 −60）
＝檔頭說的「玻璃有縫」情況，要求四顆全中會誤中止；但「一顆都沒有」代表完全無附著，必須看得見。
⚠️ **副作用要記住**：迴圈是「一顆到門檻就跳出」，所以那欄壓力是**第一顆密封當下的快照、不是穩定值**
（步驟 1 常見 `-30/-53/-34/-39` 就是抽到一半），**不能當密封品質看**。
⚠️ **自記**：我還犯了一個低級錯 —— 加欄位時只改了資料列的格式字串、**漏改表頭**，
第一次起跑就 `TypeError` 掛掉（機器沒動）。之後改為**用 11 個假參數實際套用兩個格式字串**來驗，不靠眼睛看。

**✅ 1 週期（含 ③b）通過**：左右差 6 段 中位 3 / p90 4 / 最大 5、**≥6cm 0%**，
回程 208cm@30Hz 16.5s、roll均 0.29、出帶 0% —— 與 `0902d` 同級。

**🔴🔴 10 趟連續兩次都被 QX-DO24 風扇模組打斷，機構本身兩次都是好的**
| 次 | 掛在 | 訊息 |
|---|---|---|
| 09-02 `cycle10_0902c` | 週期 2 | `pwm_freq_write_failed_no_reply_timeout` |
| 09-03 第一次 | 週期 2 步 2（⑤ 開風扇） | 同上，逐字相同 |
| 09-03 第二次 | **週期 1 步 3（① 關風扇）** | `pwm_control_write_failed_no_reply_timeout` |
⇒ **現在擋住 10 趟的不是機構，是風扇模組**，而且**發作越來越早**。
✅ 已在 `fan()` 內加自動復原（寫入失敗 → `pwm restart` → 重試一次，**每次觸發都印出來**，
靜默重試會把這個間歇故障的唯一計數來源吃掉）。第一次故障時實測有效
（`pwm status` 讀取正常、`pwm restart` 回 `acked=1`），**第二次連 restart 都 `acked=0`**。

**🔴🔴 但最重要的是這條：09-02 的歸因很可能是反的**
待辦表記「失敗在**主站→模組**方向，長幀進不去」（依據是讀取偶爾成功）。今天的證據相反：
第二次中止時，**沒有任何一道關閉指令被回報成功**（① 失敗、`pwm restart` `acked=0`、收尾再失敗一次），
**但 per user 現場目視確認風扇是停的** ⇒ **那些「失敗」的指令裡至少有一道真的生效了。**
⇒ 較可能是**寫入有送達、回覆（模組→主站）遺失**，程式收不到 ACK 就判失敗並中止
—— 也就是說耐久測試可能是被**誤判**打斷的，不是控制真的失效。
⚠️ **另一個解釋未排除**：`pwm restart`（reg `0xFF00`）其實執行了、模組重開後風扇歸零，
只是版本號讀不回來才回 `acked=0`。
🔧 **分辨方法（安全、待執行）**：在通訊「失敗」狀態下送一次**開風扇**，目視螺旋槳有沒有轉。
轉了＝寫入有進去、是回覆遺失；沒轉＝維持原記載。

**🔴 roll 中止門檻是瞬時的，左右差門檻卻有「連續 3 筆」—— 同一個教訓只套用了一半**
223cm 收回頂端時，整段 28 筆平均 |roll| 1.33°、走得好好的，**卻被最後一筆停止擺盪 −7.04° 中止**；
停穩後量到 **+2.49°**（由負擺到正＝典型鐘擺過衝）。左右差守衛 09-01 已因瞬態改成連續 3 筆，
**roll 門檻沒跟上**。⏸ 未改 —— 改中止邏輯不該在等著跑測試的時候順手做。
📌 同一段另量到：**兩側實走差 6cm**（指令 223，`L=223 / R=217`）＝平衡迴路整段在對抗的東西。

**🎯 新座標下的水平點：頂端 ≈ `L−R +1.8`、底部 ≈ `+0.87`（跨 223cm 差約 1cm）**
頂端 `L−R=−1` 時 roll `+2.49°`，用當日斜率 −0.888 反推得 +1.8；依此送 `pay_out_left 2`
（**刻意選往下動而非 `retract_right`**：往下有 223cm 行程，避開「頂端還有多少機械餘裕」這個未知，
且 `L=3` 仍在起點檢查的 `TOL=5` 內）→ 實得 `L−R=+2`、roll **−0.69°**（預測 −0.18，差 0.5° 落在遲滯帶內）。
📌 順帶：**在底部調平會讓張力更不平均（1.7×），在頂端調平反而更平均（1.25×）** —— 又一個隨高度而變的量。

### 🎯 QX-DO24 查清楚了，10 趟跑完（09-03 傍晚）

**60 次開/關的分辨測試把四類失敗分乾淨了**（`fan_probe.py`，三輪各 20 次，回讀 `pwm status` 的
`duty,hz,ctrl,en` 四欄比對，不靠 ACK 也不靠目視）：

| 錯誤訊息 | 事後模組狀態 | n |
|---|---|---|
| `pwm_readback_failed` | ✅ 正確 | **8/8** |
| `pwm_control_write_failed` | ✅ 正確（可讀時） | **3/3** |
| `pwm_freq_write_failed` | ❌ duty 沒生效 | **3/3** |
| `pwm_duty_write_failed` | ❌ 沒生效 | **1/1** |

📌 **有機制解釋、不是純統計歸納**：寫入順序 freq → duty → control。**早期失敗**（freq/duty）
讓函式提前返回、duty 根本沒寫到 ⇒ 真的沒生效；**晚期失敗**（control/readback）發生時 duty 已寫好，
而 control 寫的是模組本來就持有的值（面板與步伐路徑一律 65535）⇒ 那一幀到不到達，狀態都一樣。

**✅ 兩處修正（都要求實際回讀，不是直接放行）**
1. `cmd_pwm_set` 回讀逾時 → 由 `ERR pwm_readback_failed` 改回 **`OK pwm_set_unverified`**。
   原註解「讀不到就不能宣稱成功」的原則保留 —— 所以**不回裸 OK**，而是可辨識的第三種狀態。
2. `pwm_control_write_failed` → **實際回讀四個值，全部相符才**降級為 `unverified`，否則照舊 ERR。
   （本裝置「寫入失敗時輸出不會歸零、保持前值」，所以不能靠推理放行。）
3. `cycle_test.py` 的 `fan()` 加自動復原：寫入失敗 → `pwm restart` → 重試一次，**每次觸發都印出來**。

**🎯 結果：10 週期全部完成、零中止**（`cyc_0903_10x_c.log`）
60 段左右差 **最小 0 / 中位 2 / p90 4 / 最大 5 cm、≥6cm 0%**、自行回復 0 次；
**風扇自動復原只觸發 1 次**（且那次正是 `pwm_control_write_failed` → `pwm restart` → 重試成功）。
⇒ **含上滑台 `0-50-0` 的完整 10 趟首次跑完。** 對照 `0902d`（有真空、無滑台步驟）的
中位 3 / p90 4 / 最大 5 ⇒ **加入滑台掃動沒有讓姿態變差**（中位 2 vs 3 **不宣稱是改善**
—— 依續七「run-to-run 變異 ≈ 效果量」的教訓）。
✅ 滑台耗時 **4.4 秒 × 全部 50 步逐次相同**。

**🔴 三個要更正的先前說法（都是我自己的）**
1. ~~「風扇停著也有 35% 失敗」~~ —— 35% 是「沒回報乾淨 OK 的比例」，**功能失敗率是 4/60 ≈ 7%**。
   匯流排確實有約兩成回覆遺失，但功能衝擊比我說的小得多。
2. ~~「10 趟是被誤判打斷的」~~ —— 只對一半。第一次死在 `pwm_freq_write_failed`（真失敗）；
   **第二次死在 `pwm_control_write_failed`，那次才是誤判**（機器當下狀態完全正確）。
3. ~~「`pwm_control_write_failed` 實際生效 2/2」~~（第二輪當下的判定）—— **證據不足**：
   當時只回讀 duty，而 control 失敗時 duty 早在前一步就寫好了 ⇒ duty 相符**不能**證明 control 寫入成功。
   補上四欄比對後才站得住，而且結論改成更精確的「**事後狀態正確**」而非「寫入成功」。
   📌 **教訓：回讀驗證要驗「會變的那個量」；驗一個本來就等於目標值的欄位，等於沒驗。**

**🟡 仍未解、但已縮小**
- `pwm_freq_write_failed` / `pwm_duty_write_failed` 是**真的失敗**（60 次裡 4 次，≈7%），
  仍指向 `.22` 匯流排的電氣問題（與 09-02 的位元級損毀診斷一致）。
  🔧 分辨方法不變：**量風扇啟動瞬間 [B] 電源（EPP-200-24）電壓** —— 掉得明顯＝電源要分離，否則偏 EMI。
  🆕 **新增一個約束條件**：本次測試全程 duty 大多在 5（風扇停止），**仍有失敗**
  ⇒ 風扇運轉可能是**放大**因素而非唯一成因，`.22` 有基線層級的問題。
- ⚠️ **`unverified` 這個折衷的失效條件已寫進原始碼註解**：若日後出現「回報 unverified 但狀態不符」，
  就要改回去。本次 60 次中該格為 **0**。

### ⏱ `cycle_test.py` 加入行進速率統計（09-03 per user）

目的：推估「上下走一公尺要多久」。**刻意分三個口徑，因為它們回答的是不同問題**：

| 口徑 | 單輪實測 | 每公尺 |
|---|---|---|
| 下行**純移動**（只算 `pay_out`） | 200cm / 27.0s ＝ 7.40 cm/s | **13.5 s** |
| 下行**含開銷**（整步牆鐘時間） | 200cm / 112.1s ＝ 1.78 cm/s | **56.0 s** ← 實際作業速率 |
| **上行回程**（一次連續） | 203cm / 16.4s ＝ 12.35 cm/s | **8.1 s** |

**⇒ 一趟來回每公尺約 64 秒。** 單步 22.4s 的組成：
**伸出 3.4 / 真空 2.0 / 滑台 4.4 / 收回 3.7 / 移動 5.4 / 其他 3.6**。

🔴 **兩個讀數字時會踩的坑，已寫進 `_rate_summary()` 的 docstring 與輸出的警語**：
1. **下行 7.40 cm/s 不可與上行 12.35 cm/s 相比** —— 下行是 5 段 × 40cm、每段各自加減速；
   上行是一次連續 203cm。同樣 30Hz，差別全在分段。「下行比較慢」**不是**機構差異。
2. **決定作業時間的是開銷不是速度**：每步 22.4s 裡**只有 5.4s（24%）在移動**。
   ⇒ **調 `motion_hz` 幾乎沒用**（30→50Hz 最多省每步約 2s＝9%）；
   真正的大頭是滑台 4.4s 與收腳 3.7s。要壓作業時間應該從那裡下手。
🟡 「其他 3.6s」尚未拆開（讀 `status`、風扇切換、⑥ 的 1s delay、⑧ 的 0.3s 靜置）——
要認真壓時間的話這格值得再拆。
⚠️ **目前這組是 n=1**，依續七「run-to-run 變異 ≈ 效果量」的教訓**只能當量級參考**，
要當基準需跑滿 10 週期取平均。

✅ 同輪順帶再次通過：左右差 6 段 中位 2 / p90 3 / **最大 3**、零中止。
✅ **收工：幫浦已關**（`OK ch2=0`，並以獨立 `relay_status` 複驗 16 個通道全 0）。

### 🖥 Web GUI console v2 上線，與現行 GUI 並行（09-03 傍晚，per user）

**問題**：現行 `index.html`（861 行、20 個面板）依**子系統**切，每個按鈕對應一道指令 —— 那是 bench 除錯台。
而今天累積下來的知識（前置條件、順序的安全含意、中止判準、回讀驗證）它一個都沒表達。

**做法**：`web_backend/public_v2/index.html`，四個分頁 Dashboard / Mission / Manual / Setting。
🔴 **範圍刻意設限：Mission 不在瀏覽器裡重跑週期測試。** 中止判準、步驟順序的安全需求、
風扇復原邏輯全在 `cycle_test.py` 裡；前端重寫一份等於**製造第二個真相**，兩邊遲早會漂。
這一頁做的是「起跑前的閘門」與「跑的時候看得見」，起跑指令給人複製。

**五條設計決定，每條都對應今天一次真實誤判**：
| 決定 | 對應的誤判 |
|---|---|
| 每個致動器顯示**指令 ↔ 回讀**，四態（已確認／未複驗／失敗／等待） | `pump on` 回 OK 而回讀 `ch2=0`；60 次 PWM 測試裡 8 次「失敗」其實成功 |
| 門檻顯示**餘裕**不只顯示當下值 | 停在 +2.49°、門檻 6.0° —— 起跑前就用掉 42%，舊介面只顯示 2.49 |
| **瞬態與持續分開**（左右差連續 3 筆 vs roll 瞬時） | 223cm 收回全程平均 1.33°，被最後一筆停止擺盪 −7.04° 中止 |
| 前置條件是**閘門**不是提示，且逐條說明「為什麼重要」 | `balance_source` 每次重啟靜默回 `meter`，status 是唯一徵兆 |
| Setting 頁第一欄是**「重啟後還在嗎」** | 執行期值重啟遺失是本專案最常見也最難察覺的故障 |
📌 **Manual 頁保留 raw command**：CH3 被記成「空通道、實際是幫浦 B 組」就是逐顆通電查出來的。

**部署（09-03，兩個獨立行程、兩個埠）**
- `8080` = 現行 GUI（**完全未動**） · `8081` = console v2
- ✅ `server.js` 只加一行：靜態目錄改吃 `PUBLIC_DIR` 環境變數，**預設值與原本逐字相同**
  ⇒ 不帶變數啟動時行為沒變。**刻意不複製第二份 `server.js`** —— 那會變成兩個各自演化的副本，
  而本專案已經因為「同一件事寫在三個地方」吃過虧。一個檔、兩種設定。
- 啟動：`HTTP_PORT=8081 PUBLIC_DIR=~/bringup/web/public_v2 WROBOT_IP=192.168.5.26 CRANE_IP=127.0.0.1 node server.js`
- 實作細節：每個 target 一條**序列佇列**（文字協定沒有 request id，同一目標一次送一道、
  以下一則 `OK`/`ERR` 為回覆，其餘視為事件）；輪詢回覆**不進通訊紀錄**（一秒三行會把事件淹掉）；
  `CH1`／`CH6`／吊機微調／`emergency_stop` 需二次確認。
- ⚠️ **尚未經瀏覽器實測**（Claude 端無瀏覽器），HTTP 200 + 標題正確是目前的驗證程度。
  兩個待確認：回程 16 秒內 1 秒輪詢會不會顯得跳；③ 之後 ④ 之前那 2 秒的密封窗口會不會被輪詢整個錯過。
  若確實錯過，解法是那兩段改用事件而非輪詢，但那要改 `server.js`，值不值得要先確認。

**⚠️ 自記：`pkill -f` 今天又殺到自己一次**（09-01 已記過同一件事）。
`bash -c '...'` 的命令列本身含目標字串 → `pkill -f "http.server 8082"` 把 ssh shell 一起殺掉，
**後面的 `rm -rf` 整串沒有執行**，而指令看起來是「成功」的（無輸出）。
🔧 用 `pgrep` 取 PID 再殺，或在樣式裡加排除。

### 🔴 09-03 晚間：座標第二次重定義、一次危險操作、以及「該查文件卻沒查」

**座標第二次重新定義（per user）：由「繩長」改成「離底高度」**
早上在**玻璃最高點**歸零（0=頂、223=底）；傍晚改在**玻璃最底端（離地最近）**歸零
→ `length_left` 0=底、往上為**負**，實測頂端 **−231** ⇒ **玻璃面高度 231 cm**。
🔴🔴 **這個改動把一個安全前提整個反過來**：新零點下 `L=0` 是**底端**，而舊的起點檢查
`abs(L0 - TOP) > TOL`（TOP=0）會把它讀成「在頂端」→ 通過 → 往下放繩 200cm。
**從最底端再往下 200cm**，而區間守衛比的是 `BOTTOM=223` 也擋不住。
與 `mission_run.py` 那次「方向寫反、被使用者在執行前攔下」是同一類錯誤。
✅ `cycle_test.py` 與 `mission_run.py` 都改成以 `height() = -length_left` 運算，`TOP,BOTTOM = 231,0`。
✅ **乾跑驗證**：用 `exec` 從原始檔抽出真正的 `height`/`plan_leg`/`check_envelope` 來跑
（重打一份等於測副本，而副本正確不代表腳本正確）。含**負向對照**：假裝在底端時
`cycle_test` 正確拒跑、`mission_run` 正確選 `retract`（往上）。
✅ `mission_run.py` 實機 1 趟來回跑通（62 筆、出帶 1.6%）。
🔴 **改漏一處**：抬頭的 `SPAN = BOTTOM - TOP` 印出「單程 −231cm / −4.6 m」。不影響動作
（方向由 `plan_leg` 決定），但**在一支剛改過座標的腳本上印負距離，會讓人懷疑方向也反了**。已修。
📌 `check_envelope` 的兩個符號是最容易改漏的地方（`pay_out` 現在讓高度**減少**）——
改漏會變成「看起來還在、實際反向」的守衛，**比沒有守衛更糟**。已寫進 docstring。

**🔴🔴 一次危險操作：吸盤還吸著就收繩（per user 當場指出）**
10 週期測試中止在**週期 3 步 5 的 ④ 推桿收回 TIMEOUT**（不是風扇——風扇的錯誤是中止**之後**
收尾時才出現的，我一開始看錯順序）。中止後「現場保留」＝**推桿仍伸出 10cm、吸盤仍吸在玻璃上**，
而我直接下了 `retract 162` ⇒ **用鋼索拉一台吸在牆上的機器**。
證據全部吻合：張力由均衡變成 **35.6/56.0**（差 20kg）、roll **−3.5°**、指令 162 只走 28cm 就停。
✅ 處置順序：`vacuum all off` → `pump off` → 確認四顆回大氣壓 → `pusher all retract`
（四支回讀 pos≈29.9°＝0）→ 調平（`retract_left 4`，roll −4.2°→+0.55°）→ 才收回頂端。
🔴 **教訓**：「現場保留，未自動復位」代表**機構仍處於中止當下的狀態**，
下一個動作之前必須先確認**附著狀態**，不能只看吊機的 status。
`retract_seg.py` 的守衛（roll/左右差/張力差）**擋不住這件事** —— 它監看的是姿態，
而被吸盤拉住時姿態一開始是正常的。

**🔴 `pwm restart` 被 `paused_on_error` 擋住 —— 復原路徑在最需要時被鎖**
中止後 `fan()` 的自動復原送 `pwm restart`，收到 `ERR state_violation current=paused_on_error`。
而**中止流程本身就會把本體推進 PausedOnError** ⇒ 這個閘門保證了「越需要越用不到」。
📌 這是 2026-09-01「關閉方向任何狀態都放行」那次修正的**漏網之魚**：當時只放行了
`cmd_pwm_set` 的關閉方向，而 restart 是那條退路的**入口**，沒有跟進。
✅ 已拿掉狀態閘門並在非 Idle/Ready 時 log 出來。
⚠️ **沒有宣稱它無條件安全**：`saveOutputAsDefault()` 若曾在 `control=65535` 時被呼叫，
模組**一上電就會驅動馬達** ⇒ 重啟後的輸出取決於 flash 內容。目前無自動流程呼叫 `pwm save`，
風險可控，但**日後若有人存了非停止值，這個放行就會變成一條把風扇叫醒的路徑**。

**🎯 per user 提醒「你有文件可以查查」—— 查了之後兩個發現，都是 09-02 就寫下來的**
（`.claude/summaries/QX_DO24_MODBUS_SUMMARY.md`。⚠️ **自記：我花了一整天用經驗法打這個問題，
而 CLAUDE.md 明寫「動 driver 之前先查 summaries，不要憑現有程式碼反推協定」。**）

1. 🔴 **FC 0x06 退路在寫一個手冊說「不必寫」的暫存器，而今天就死在它上面。**
   手冊 P4：「**保持** `0x04` 的值為 0，只改 `0x05` 即可」——**保持**，不是「寫 0」。
   本專案頻率鎖 50Hz ⇒ `0x04` 永遠是 0 ⇒ 那筆寫入**從未改變過任何東西**，
   卻在今天以 `FC0x06 fallback: high word (reg 0x0004) write failed` 讓整輪中止。
   ✅ 改為**先讀再決定要不要寫**；讀失敗仍照寫（讀不到不等於它是 0）。
   理由：這條退路存在的前提就是「寫入方向有問題」，**能少寫一筆就少一筆**，
   而讀（FC 0x03）在同一段期間一直是通的。⚠️ **未經故障複現驗證**。
2. 🔴🔴 **手冊的接線警告，症狀描述與今天完全一致**：`VCC`/`GND` 與 `A`/`B` **同排緊鄰**，
   保修除外條款明列「電源錯接到 485 導致 485 段燒毀」，且
   **「症狀會是方向性的：模組還發得出（讀取偶爾成功），但收不進」**。
   今天量到的正是**讀取一直正常、寫入約 7% 失敗** ⇒ 這條線索指向**電氣損傷**而不只是雜訊，
   而且**可目視檢查**：看模組 A/B 端子附近有沒有被電源碰過的痕跡。
📌 文件裡「驅動未實作 0xFF00 重啟命令，值得加」**已過期**——那條做了，今天還靠它救回一次。

**⚠️ 自記三個工具使用錯誤（都讓我一度誤判）**
- `pkill -f` / `pgrep -f` **會比對到我自己的 `bash -c` 命令列** → 把自己殺掉（`rm -rf` 沒執行）、
  以及把「腳本已結束」誤判成「仍在執行」。連中括號技巧也失效，因為指令列裡就寫著那串字。
  🔧 判斷 process 用**不含目標名稱**的條件，例如 `ps -eo args | grep "[p]ython3"`。
- `strings` **只印 ASCII** → 用中文字串做 binary 正負對照會兩邊都是 0，看起來像「部署失敗」。
  🔧 對照用純 ASCII 標記（`[pwm_restart]`、`already 0`）。
- 本機 python 3.8 無法解析 `mission_run.py`（既有的 f-string 反斜線，3.12+ 才允許）
  → **語法檢查改在 Pi 的 3.13 上做**，那才是執行環境。**沒有去「修」那個寫法。**

**GUI（8081）同步跟上**
- 座標改成離底高度（改之前 `L/223` 在新座標下會把頂端畫成 0、量規整個壞掉）
- Manual 頁依 per user 規格重寫：吊機（同步／左／右收放、張力現值、四個保護門檻可調、即時）、
  PQW 8CH、風扇面板、ZDT 推桿（整組＋左右＋四支各別＋使能／解堵轉／歸零）、
  上滑台（位置回讀、目標＋RPM、快捷、使能、`rail_zero`、`cfg_soft_enable`）
- 🔴 **腳本執行中鎖整頁**：`motion_active` 只在吊機轉動時為 1，而每步有約 17 秒不在轉
  ⇒ 只看瞬時值的話鎖會一步一鬆，**比沒有鎖更糟**（讓人以為沒在跑）。改為「看到運動就延長
  45 秒」。**緊急停止刻意不鎖**——那是把控制權拿回來的唯一手段。
  ⚠️ 鎖定條上明寫「這是由 motion_active 推論的，**不是硬體互鎖**」。

### 🎯 09-03 收盤：10 週期首次完整跑完、`.22` 故障被匯流排分離解決、吸不到的位置查明是橫桿

**🎯 10/10 完成、零中止**（`cyc_0903_10x_final2.log`）
60 段左右差 **中位 3 / p90 4 / 最大 5 cm、≥6cm 0%**、自行回復 0 次；
滑台 4.2~4.6s 全程一致；**來回每公尺 62.6 s**（下行含開銷 54.7 + 上行 7.9）。
單步 21.9s：伸出 3.4 / 真空 2.3 / 滑台 4.2 / 收回 3.4 / 移動 5.6 / 其他 3.1。
📌 這是今天第七次嘗試才跑完的 —— 前六次分別停在週期 1、2、3、3、8（真空）與被手動中止。

**🔴🔴 `.22` 匯流排故障：per user 把 QX-DO24 移到獨立的 `192.168.1.21`，問題消失**
- 動機：六次多週期測試**四次死在 `.22`**，而掛掉時是**整條一起掛**（`QX:9` 與 `JC100 5/6/7` 同時逾時），不是單一模組故障。
- ✅ 硬體：per user 新增網關。2026-09-03 由本體 Pi 實查後台（`admin`/`admin`），
  設定與 `.22` 逐項相同：`_br=115200 / bc=8 / par=0 / sb=1 / _tlp=4001 / cmode=0 / _cnum=4 / _pt=0`。
  **不需要改網關設定，直接接線。**
- ✅ 軟體：新增 `IP_485_2` 與 `cli_21_`，`pwm_` 改綁 `cli_21_`；harness 補 `FCV_EP_USR21_*`。
  🔴 **`.21` 連不上只警告不中止**（與 `.20`/`.22` 不同）：那兩條上面是推桿/繼電器/壓力計，
  沒有它們機器不能動；風扇是附屬裝置。新裝的網關沒接好就讓整台起不來，是把可用性賠給附屬功能。
  🔴 **Slave ID 維持 9**：改位址要寫 reg `0x20`，手冊明載那是「實時保存、斷電記憶」且「請勿頻繁寫入」。
  獨立匯流排上沒有撞號問題 ⇒ 沒有理由花一次 flash 寫入去改一個不影響任何事的數字。
- 🎯 **結果**：風扇 20/20 全正常（`.22` 上三輪合計 60 次有 33% 回報異常、7% 真失敗）；
  之後兩輪長測（7 週期 + 10 週期）**`.22` 那個故障一次都沒有再出現**。
- ⇒ **分辨實驗的結論**：`.21` 乾淨 ⇒ **問題在 `.22` 那一側**（線路／網關／終端／該匯流排上的其他裝置），
  **不在 QX-DO24 模組本身**。手冊那條「電源碰到 485 燒毀接收段」的假設可以先放下。
  🔴 **尚未驗證的下一步**：`.22` 現在只剩 JC100 ×4。**若它獨處之後就穩定 ⇒ 原本是同線裝置互相干擾；
  若 JC100 自己也會掛 ⇒ 那條線路／網關本身有問題。** 這是下一個要分辨的。
- 🔴 **順帶記一條會誤導未來的事**：08-28 曾用「`.21` 不可達」排除過「同一條匯流排上有第二個主站」。
  **現在 `.21` 活了** —— 若新網關的 A/B 不小心併到 `.22` 那對線上，那個假設會成真，
  而症狀正好就是今天在查的「整條 `.22` 間歇失效」。已寫進 `CLAUDE.md`。

**🎯 「吸不到」查明是**橫桿**，不是故障（per user）**
連續兩輪都出現「四顆壓力同時歸零」，位置固定在下行第 3 步附近。
per user：**那個位置有一支橫桿。** 這一次解釋了全部觀察：
位置固定／四顆同時歸零（整個吸盤陣列都落在橫桿上）／風扇加到 8% 也無效（推再緊也吸不到橫桿）。
⇒ 檔頭原本寫的「玻璃有縫吸不住是現場條件不是故障」，指的就是這一類。
📌 **先前我的候選清單（幫浦 B 組出力不足／管路洩漏／幫浦過熱）全部錯**，
而 per user 一句「那邊有一支橫桿」直接結案 —— **現場知識查不到，要問。**

**✅ 「吸不到 → 記錄但不中止」（per user）當天就回本**
`cycle_test.py` 改為：無一顆 ≤ −50 kPa 時 **① 逐步印出 ② 計數 ③ 跳過該步的滑台掃動 ④ 總結報比例**。
- 放行的前提寫進註解：這個測試**本來就不靠吸盤承重**（檔頭「無防墜錨點、靠鋼索」），
  所以吸不到不改變安全性，只代表那一步沒有模擬到附著。
- 🔴 但**必須看得見** —— 09-03 早上就出過一次假象（讀太早，四顆全 0 卻照跑，我還誤判成「推桿沒碰到玻璃」）。
- 🔴 **跳過滑台的理由**：沒有附著時機體只掛在繩上，橫向移動滑台會讓它擺盪，
  那個擺盪不屬於被測項目、只會污染 roll 統計。
- 🎯 **它讓橫桿被發現**：若照原本中止，這輪會停在週期 5，就永遠不會知道那 3 次是同一個位置。
  **是「繼續跑」讓規律浮出來的。**

**🔴 DY-500 移除繫結（per user）**
拿掉 `weight_[i].init(cli_22_, …)`，**driver、成員、tier-2 程式碼路徑全部保留**。
Mode B init 不發包 ⇒ 移除不改變任何流量，但讓「`.22` 上有什麼」在原始碼與實體之間一致
（先前讀 `init()` 會以為那條掛了 7 個裝置，實際 5 個）。要復用時把兩行加回來並做一次性探測即可。

### 🦾 09-03 夜間：手臂維修後復機、兩個工具各 10 輪、清潔動作整合進 cycle_test

**復機過程：三次啟動失敗，最後查明是整組手臂電源**
- `motor_api` 連續三次 `DamiaoAPI init failed`：前兩次 M2 `switchControlMode` 5/5 失敗，
  **第三次連 M1 也失敗** —— 故障從一顆馬達跳到另一顆 ⇒ 不是單顆壞掉，是兩顆共用的東西。
- per user 對**整組手臂電源**斷電重上後一次成功。
- 📌 `calibrate_arm_slot` 的註解早就寫過這個症狀：反覆撞停點會「觸發達妙的**過熱/過流鎖存**，
  也就是我們看到的 `switchControlMode failed`」，而那種**只能斷電解除**。查文件比試更快。

**🔴🔴 全天最貴的一個教訓：`STATUS` 只讀本地快取，失能的馬達會回傳凍結值**
`cmd_status_sequence()` 全部走 `Get_Position()` 等快取，**不主動查詢**。馬達失能後不再回傳
CAN frame，於是 STATUS **一直吐最後一次的值**。當天連續踩三次：
- M2 `DISABLE` 後讀 −1.1599，per user 手轉過後**讀值一字不變** → 我以為調整沒生效
- M1 讀 0.5560（那是凍結值），我據此推出**三個結論全錯**：
  ①「M1 放下來貼著玻璃」（per user 當場指出那是機器 0 點）
  ②「M2 轉不動是被玻璃壓住」
  ③「`LR_SLOT` 會被 `M1_CLEAR_RAD=0.20` 守衛拒絕」
  真實值是 **0.0208**（`ENABLE` 後才拿得到），本來就在機械停點附近、守衛本來就會放行。
- 🔧 **通則：M1/M2 的位置只有在 `ENABLE` 之後才可信。** 這與 `cmd_pump`「回報成功≠實際生效」
  是同一類，但更隱蔽 —— 那個至少會回 ERR，這個安靜地給你一個看起來合理的舊數字。

**✅ M1 機械 0 點重新校正**（`M1 CALIBRATE`，M1-only，不動 M2）
`pre-check: already at stop pos=0.0208 — skipping Phase 1 seek` ⇒ **沒有尋點，因為本來就在停點**。
校正後 `-0.0002`。⚠️ 我原本擔心「0.3 秒太快＝誤判成已在停點」，查 log 才確認是**正確的預檢**。

**🔴 M2 一度卡在行程外，是限位問題（per user 手動調整後解決）**
M2 讀到 −1.3029（下界 −1.05），`MOVETO` 只走 0.024 rad 就停、tau 頂到 +1.7~2.0。
⚠️ **我的「分段帶回界內」計畫完全無效**：`MOVETO -1.25` 被夾成 `-1.05`（`warn=CLAMPED`）——
**任何低於下界的目標都會被夾到下界**，所以第一道指令就是整段全走，分段是假的。
per user 調整限位後恢復，tau 降到 −0.03。

**⚠️ 「讀太早」在這一段連續誤判五次**
`LR_SLOT` / `MOVETO` 回 `ERR did not reach`，但之後仍持續移動：
滾筒那次 3.2s 判定 `final pos=-0.1268`，實際最後停在 **0.5350**（目標 0.5316）。
🔧 **判定到位必須等位移歸零再看**，不能用固定秒數 + 緊容差。腳本內建的 `ERR` 也有同樣問題
（3.2 秒就下結論），**它報的失敗有時只是還沒走完**。

**🎯 兩個工具各跑 10 輪，重複性都極高**（`cyc_0903_roller10.log` / `cyc_0903_scraper10.log`）

| | 滾筒 RIGHT | 刮刀 LEFT |
|---|---|---|
| 壓上 θ | 0.6273~0.6285（散布 **0.0012**） | 0.6510~0.6556 |
| 壓上 tau | 14.21~14.31（散布 **0.10**） | 13.92~14.41 |
| 壓上耗時 | **7.6s 十輪完全相同** | 8.4~8.6s |
| 滑台 0→100→0 | **3.1 / 3.1s 十輪完全相同** | 3.1 / 3.1s |
| `Δθ@100` | +0.0213~0.0222 | +0.0175~0.0217 |
| 吸盤 | −66~−69 **零衰減** | −65~−69 **零衰減** |
| 全輪 | 21.6~21.9s | 23.3~23.6s |

🔴 **手臂重複性比 09-02 更好**（當時壓上耗時散布 2.2 秒，今天十輪都是 7.6 秒）——**維修沒有讓它變差**。

🔴 **但 tau 一律 14.0~14.4，而 09-02 同動作是 11.1~11.8（高 25%）。**
既然散布只有 0.10 Nm、θ 只有 0.0012 rad ⇒ **是固定的幾何偏移，不是鬆動或磨損**。
當天已排除：牆面（per user 相同）、ZDT 位置（回讀 3000.0°）、滑台零點（推到硬限位仍為 0）、
**M1 零點（當天重新校正過，tau 照樣 14）**。
🔧 **剩下最可能是機體到玻璃的實際距離與 `wall_mm=520` 不符** ——
`DEPLOY 505` 一次就能驗（若 tau 掉回 11 附近即確認），**尚未做**。
📌 佐證：同一次壓上、只把滑台移到 100，**tau 就掉到 11.97**（正好是 09-02 的水準）。

🔴 **刮刀有磨合趨勢，滾筒沒有**：刮刀 θ 單調上升、tau 單調下降、`Δθ` 單調變小，
第 7 輪後穩定（θ +0.0046 rad ≈ 2.5mm）。推測是軟性刃口被壓貼合。
**但穩定後的 13.92 仍遠高於 11.09 ⇒ 磨合只解釋 0.5 Nm，不是那 25% 的原因。**

🔴 **`Δθ@100` 兩個工具完全相同（+0.0217）⇒ 那是牆面幾何，與工具無關。**
以續十五的換算（0.008 ≈ 4~5mm），+0.0217 ≈ **100 端比 0 端遠 10~13mm**，斜率比 09-02 陡。

**🔴 新發現：滑台掃動期間工具頭會被摩擦力扭轉 0.45~0.49 rad（26~28°）**
滾筒 `0.525→0.671→0.198`、刮刀 `-1.005→-0.85→-1.34`，**十輪高度一致 ⇒ 固有特性不是偶發**。
往前帶 +0.15、往回帶 −0.47，**淨偏移約 −0.32**，滾筒轉不轉都一樣（電路故障那輪也相同）。
🔴 **這解釋了當天早上 M2 為什麼會在行程外**：刮刀那輪每次都收在 **−1.32~−1.34**，
已經越過下界 −1.05 —— **不是有人手轉過去的，是作業本身把它扭出去的。**
⚠️ 這會直接影響清潔接觸角，而先前完全沒有紀錄。

**✅ 清潔動作整合進 `cycle_test.py`（per user：A 方案取代 ③b、工具固定滾筒）**
③b 由「滑台空跑 0→50→0」改成 **`arm_deploy 520 RIGHT` → `rail 0→100→0` → `arm_park`**。
- 🔴 **走本體的 `arm_deploy`/`arm_park` 代轉，不直接連 9527**：本腳本跑在**吊機**那台，
  而 `motor_api` 在本體的 `127.0.0.1:9527`，**連不到**；且代轉是既有編排路徑，不該另闢第二條。
- 🔴 判準看姿態不看回傳值（壓玻璃時 `arm_deploy` 本來就回 ERR）。
- 🔴 無附著時**連壓上一起跳過**：懸空機體推牆會把姿態推歪，那不屬於被測項目。
- ⚠️ 每步 +14 秒 ⇒ 10 週期多約 12 分鐘（~37 → ~50 分）。
- `cyc.py` 同步參數化（週期／行程／`brush|nobrush`／工具槽），**刻意不再複製第四支腳本** ——
  這個 repo 已經有 `cyc10`/`cyc20` 內容完全相同的重複檔。

### 🔌 09-03 深夜：吊機斷電復機 + 清潔動作整合的三個 bug（都是我自己的）

**吊機電源被切（per user）—— 所有異常一次對上**
`No route to host`、本體→吊機 socket 的 Send-Q 由 2001 漲到 5097（一直在推 IMU 給不存在的對象）、
以及稍早那次收繩「指令 82 實走 49」。📌 **我先前把那次落差當機構問題在追，其實是電源被切。**

**✅ 復機後的整體確認**
- 🎯 **SD76 的零點活過斷電**：高度 241 vs 斷電前 240（計米器在 `.34` 網關上獨立供電）
  ⇒ 當天建立的座標（0＝玻璃底、231＝頂）**不必重建**。
- 🔴 **三個執行期參數全回編譯預設**：`balance_source` → `meter`、**`motion_hz` → 50**、
  `fine_adjust_level_diff_cm` → 0。**沒發現就直接下 `pay_out` 就是 50Hz 下行，超出 per user 定的上限。**
  📌 這正是當天上午做進 GUI Setting 頁那一欄「重啟後還在嗎」要防的東西 —— 晚上就真實發生一次。
- 本體重啟清掉殘留 socket；Web GUI 8080/8081 開機不會自動起，需手動。

**🔴 清潔動作整合的三個 bug，全部是我的**
1. **改完沒部署**：本機改了 `cycle_test.py`、語法檢查過、也 commit 了，**但沒 scp 到吊機**。
   跑出來 `滑台s=4.4`（舊的 0→50→0）而不是整合後的 ~14s，欄位標題也還是舊的。
   🔧 **判準：跑之前先確認機上那份的特徵值**（欄位名／常數），不要假設「我改過了」。
2. 🔴 **`PARK` 會停用馬達，而 `cmd_arm_deploy` 沒有先 ENABLE ⇒ 靜默失敗**。
   指令送得出去、log 印得出 `[arm] DEPLOY 520 RIGHT`、回傳是 ERR，但手臂一動也不動。
   📌 **`cyc.py` 一直能跑，只因為它自己補了 `M1 ENABLE; M2 ENABLE`**（該檔註解就寫著「PARK 會停用馬達」）
   ——那個 workaround 說明缺陷在編排層。而 `WASH_ROBOT.h:2132` 的註解**早就寫過**
   「re-enable motors after PARK」，**只有 `cmd_arm_deploy` 沒跟上**。
   ✅ 已修在 C++：`cmd_arm_deploy` 先送兩個 ENABLE（冪等，不檢查回傳；真失敗會在 DEPLOY 的姿態驗證顯現）。
3. 🔴 **`ask()` 只認 `OK`/`ERR` 開頭，而 `arm_status` 回的是 `[M1] ... | [M2] ...`**
   ⇒ 永遠等不到、直到 20 秒逾時，**症狀長得像「手臂沒回應」**，我因此誤判了兩輪。
   ✅ `ask()` 加 `prefixes` 參數；並用**真實的 arm_status 字串**驗證 `field()` 解析與判準（θ/tau 正確）。

**🔴 `bail()` 不收手臂 —— per user 指出後補上（安全改善）**
③b 整合之後，任何在「壓上之後、收手臂之前」的中止都會**讓手臂留在壓著玻璃的狀態**
（實測 6~14 Nm 持續頂著）。`bail()` 只印「現場保留，未自動復位」——
📌 **那句話對吊機與推桿是刻意的（保留現場好查），對手臂不成立**：
**頂著玻璃不是「保留現場」，是持續施力**，而達妙馬達長時間受力會觸發過熱/過流鎖存
（當天的 `switchControlMode failed` 就是那樣來的，只能斷電解除）。
✅ `cleanup()` 最前面補 `arm_park`。手臂收回**不破壞證據**：θ 與 tau 在中止當下已被記錄。

**🔴 新觀察：同一個 DEPLOY 在不同高度差很多**
高度 229（玻璃頂端附近）壓上：**M1 pos=0.7078 / tau=6.01**；
而稍早在別的高度十輪都是 **0.6281 / 14.31**。
**伸得更遠 0.08 rad、扭力只有 42%** ⇒ 那個高度的玻璃更遠（或根本沒貼到）。
📌 這與「tau 比 09-02 高 25%」是同一件事的兩面：**牆距隨高度變，而 `wall_mm=520` 是固定假設值。**
🔧 仍未做的驗證：`DEPLOY 505` 一次就能確認。
...
- 🔴 **`DEPLOY 505` 驗證牆距 —— 當日最該做而沒做的一項。**
  壓上 tau 一律 14.0~14.4，而 09-02 同動作是 11.1~11.8（高 25%），且重複性完美
  （θ 散布 0.0012 rad、tau 0.10 Nm）⇒ 固定的幾何偏移，不是鬆動或磨損。
  已排除：牆面（per user 相同）／ZDT 位置／滑台零點／M1 零點（當日重新校正過）。
  🆕 **最強的新證據**：同一個 `DEPLOY 520 RIGHT`，在高度 229 是 **0.7078 / 6.01**，
  在別的高度十輪都是 **0.6281 / 14.31** —— **伸更遠、壓更輕** ⇒ 牆距隨高度變，
  而 `wall_mm=520` 是固定假設值。**一次測試就能確認。**
- 🔴 **`.22` 匯流排本身沒修**：當日靠「把 QX-DO24 移到 `.21`」繞過，問題消失（風扇 20/20、
  之後兩輪長測零復發）⇒ 問題在 `.22` 那一側，不在模組。
  **現在 `.22` 上只剩 JC100 ×4，下次長測要看它獨處後穩不穩** —— 穩＝原本是同線干擾，
  不穩＝那條線路／網關本身有問題。這是下一個要分辨的。
- 🔴 **整合後的 ③b（清潔動作）還沒真正跑完一輪**：三個 bug 都修了（未部署／
  `arm_deploy` 缺 ENABLE／`ask()` 前綴），但修完之後沒有再跑。
- 🟡 **`roll` 中止門檻是瞬時的，左右差卻要連續 3 筆** —— 同一個教訓只套用了一半。
  當日 223cm 收回全程平均 1.33°，被最後一筆停止擺盪 −7.04° 中止（停穩後 +2.49°）。
...
**收盤現場狀態**：機器高度 **147**、roll −0.26°、張力 52.9/37.7、`motion_active=0`；
本體 `state=idle`、吸盤鬆、推桿 0、**16 個繼電器全 0**；手臂 M1 `0.0051` / M2 `0.0088`（park）；
吊機 `balance_source=imu` / `motion_hz=30`（斷電後已修正）。**沒有任何致動器通電。**

### 🔴 現場事實更正（runbook 與連線）

- **兩台都走 WiFi**（per user）：吊機 `raspberry-cran` = `192.168.5.25`、本體 `washrobot` = `192.168.5.26`。
  兩台 eth0 雖然 `carrier=1` 且各自持有 `192.168.1.10` / `192.168.1.100`，但**彼此 ping 不通**＝那組不是串接路徑。
  ⇒ `CRANE_IP` 自動選路（有線優先 → 300ms 退 WiFi）現況每次都會落到 WiFi 這條，符合預期。
- 🔴 **runbook 的工作目錄寫錯**：`~/facade_cleaning_v2` **兩台都不存在**。實際都在 **`~/bringup/`**
  （吊機 `crane_control_PI.out`、本體 `facade_cleaning_v2.out`，各自帶一串 `.prevN`），且 **不是 git repo**。
- 🔴 **`tmux` 兩台都沒裝** ⇒ runbook 的 `./scripts/{crane,wr}.sh start`（靠 tmux 開 window）**在這兩台跑不起來**。
- 📌 **WSL 端到 `.25` 是 `No route to host`，到 `.26` 卻通**（Windows 端兩台都直連正常）。
  可用 **washrobot 當跳板**：`ssh -J nexuni@192.168.5.26 user@192.168.5.25`，實測可用。

### 待完成

- 🔴 兩台程式目前**都沒在跑**（09-03 09:22 各自開機 8 分鐘，機器剛空出來）
- 🔴 `origin/main` 是否要推（`main` 已在本機 fast-forward 完成，等拍板）
- 🔴 上述 runbook 三條事實更正尚未寫回 `.claude/runbook.md`

---

## 2026-09-02 — 當日總結（手臂校正定案 + 上滑台整合 + 兩個起步/定位缺陷）

> 📌 當日細節分散在下方 **續七 ~ 續二十一** 共 15 筆。本節是總覽，細節與數據看各筆。

### 已完成

**手臂（cleaning_arm）**
- **重力模型重擬**：`M1_GRAVITY_K` 20.87→16.09（12 點雙向慢掃，舊值只用 2 個靜態點、混入摩擦）
  ⇒ 自由平衡下垂 0.0695→0.0025~0.0060 rad，`arm_deploy` 首次回 OK
- **工具外伸量重量並經 per user 驗收**：滾筒 148.09→204.32、刮刀 134.07→192.37
  （兩者都少約 57mm，來源是同日的臂長 320→490）；`ARM_WALL_MM_DEFAULT` 定案 520
- **M2 三個工作位置改為斷電手轉的實測絕對值** + **停點相對定位**（零點掉了可自動還原）
- **關掉會毀零點的 INIT 路徑**：陳舊偏移守衛門檻 1.5→3.0（註解本來就寫 3，碼寫 1.5）
- 🎯 **起步頓挫解決**：`touch_wall_slot` 的失效偵測探測 ±0.3→±0.05 rad
  （0.3×kp90 = 27 Nm 持續 60ms，在每次移動前）⇒ 起步 max|tau| 18.7→**3.4**、max|vel| 1.74→**0.20**
- 🎯 **M2 定位解決**：`CONV_TOL` 0.15(8.6°)→0.02 ⇒ 落點誤差 0.073→**0.011 rad (0.6°)**
  ⚠️ 當日「M2 摩擦飽和 2 Nm」整個是這個容差造成的假象
- `go_home_slot` 補速度安全閥；`LR_SLOT` 加「M1 須先離開玻璃」守衛（per user）
- 新增 **`M1 DIAG ON/OFF`**：控制迴圈內 ~50Hz 記錄 M1+M2 —— 上面兩個成因全靠它查出

**上滑台（DM2J slave 14）—— 從零到可用**
- 新增 `rail` / `rail_pos` / `rail_zero` / `rail_enable` / `rail_cfg_soft_enable`
- 🔴 **讓伺服真的能軟體失能**：Pr4.02 DI1「使能+常閉」→「使能+常開」
  （DI1 未接線＋常閉 ⇒ 訊號恆觸發，原本無論如何關不掉），存 EEPROM 並經斷電重啟驗證
- **手動歸零流程上線並實際使用**：失能→手推到左端硬限位→使能→歸零
- 行程 48→**130cm**（舊值 provenance 寫「實體 50cm」，來源不明）；速度 250→**400 RPM**、斜坡 100→**3000**
- ✅ 全行程帶滾筒壓玻璃掃動 0→130→0 各 3.7s，**四顆吸盤零衰減**；10 週期耐久全過

**本體 / 其他**
- 滾筒繼電器改到 DEPLOY **之前**開（per user）；順帶修掉乾掃路徑「DEPLOY 失敗滾筒沒人關」
- `rail_move` 的三個自製缺陷（回傳慣例寫反、預設逾時 20s、dispatcher 寫死 rpm）

### 待完成

- 🔴 **M2 掃動中被橫向摩擦帶著跑 0.39~0.42 rad（≈24°）** —— 唯一剩下的機構問題。
  要在**掃動中**量 `hold_kp`（當日的量測是靜態的，且被容差假象汙染）
- 🔴 `verify_arm_deploy_` 自 06-06 起是無條件 `return false`，障礙偵測三個月沒在跑；
  要改為比對每 slot 校正過的接觸角才能打開
- 🟡 400 RPM 滑台失步未驗（無回授，只能拿尺量）
- 🟡 M1 黏滑未解（伸出段平均 0.07 rad/s 而命令 0.3）
- 🟡 140 端玻璃比 0 端遠約 6mm（三輪重現）；四顆推桿接觸力道不均（機身與玻璃不平行）

### 📌 當日方法論紀錄

**憑假設調參數 9 次，全部被量測推翻；先取得量測再改 6 次，全部成功。**
四個「原以為是物理問題」的東西全是**被遺留的舊參數或錯誤前提**
（DI1 常閉、`lower_bound=-0.8`、`CONV_TOL=0.15`、探測 `±0.3`），
其中三個**在別處被修過一半** —— 複本同步是本專案反覆出問題的地方。
