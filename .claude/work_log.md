...
- 🔴🔴 **QX-DO24 是目前的頭號阻礙**：先做上面那個「開風扇目視」分辨測試，再決定修哪一邊
  （回覆遺失 → 應改成寫入後回讀確認、不要一逾時就中止；寫入失效 → 才是電氣/接線問題）
- 🔴 10 趟仍未跑完（兩次都中止在風扇，機構部分兩次都正常）
- 🟡 `roll` 中止門檻要不要比照左右差加「連續 N 筆」
- 🟡 機器現停 `L=83 R=81`、roll −0.23°、風扇停、推桿 0、手臂 park、幫浦 ON
