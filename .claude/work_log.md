...
- 🟡 **「攔下 keepalive 冒充」那條紅字尚未真的觸發過** —— 要它出現得剛好撞上在途指令。
  ⇒ 這條保護本身也是未驗證路徑。
- 🟡 **運動中動態提高輪詢率**（`pollFast` 1s → 250ms）：一次解掉 09-03 留下的兩個未決問題
  —— 16 秒回程只有 16 個取樣點、③→④ 之間 2 秒的密封窗口 1 秒輪詢**保證會漏**。
  📌 09-03 原記「解法要改 `server.js`」**不完全對**：事件通道本來就存在（非 OK/ERR 直接推），
  真正缺的是**後端沒有密封窗口對應的 EVT**（現有只有 `init_complete`／`state_changed`／
  `obstacle_detected`／`tension_alarm`／`tension_total_limit`）。加 EVT 屬 C++ 那條線；
  提高輪詢率是純前端的替代方案。
- 🟡 **Dashboard 的 roll 中止門檻 `6.0` 在 `index.html` 寫死兩處**，
  而真正的來源是 `cycle_test.py` 的 `sys.argv[4]` 預設值 ⇒ **腳本一改參數，GUI 就在說謊**。
  這正是 v2 設計時明講要避免的「第二個真相」。目前數值仍相符，所以是風險不是錯誤。
- 🟡 **8080 現行 GUI 的 `public/` 在 Pi 上落後 repo**（Pi 是 8/28 版）。差異三項，
  其中一項是**操作者看得到的錯標籤**：`刷洗滾筒 (CH15)`，實體早已改回 **CH5**。
  🔴 另一項 repo 自己也是錯的：前端步伐上限寫 `5..100`、Pi 上寫 `5..50`，
  而後端 `STEP_CM_MAX` 09-01 起是 **45** —— **兩邊都對不上**。
  📌 後端本來就有 `cmd_get_settings` 會吐 `step_cm_max=<現值>:<預設>`，
  **前端應該去讀而不是寫死**（執行期還可用 `set_setting` 改，寫死必然脫鉤）。
