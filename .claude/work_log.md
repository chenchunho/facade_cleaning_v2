...
## 2026-09-09 — 機器開機前的純程式時段：MH300 Phase 3-a 實作；吊機 watchdog 派給 AI-2 診斷

> 🔴 **本輪未送任何會讓機器動作的指令**、未部署任何 binary。
> 機器端只做唯讀查詢（`ping`／`ssh` 查 `hostname`／`uptime`／`ss`／`ps`）。
> 兩條線：本 session `agent-ai-60` 走 MH300 driver；`AI-2` 走吊機 watchdog（**只診斷、不改碼**）。

### 開工盤點：機器正在開機，但還沒到位

| 目標 | 狀態 |
|---|---|
| 倉庫 router `192.168.5.1` | 🟢 通 |
| 吊機 `raspberry-cran` `192.168.5.25` | 🟢 **開機 4 分鐘**（10:35），但 `crane_control_PI` **未啟動**、`5002/8080/8081` 全未監聽 |
| 本體 `washrobot` `192.168.5.26` | 🔴 **不通**（router 通 ⇒ 是本體未開機，不是線路問題） |
| 工作區 | **1,668 行未 commit**（10 改 + 4 未追蹤），`main` 停在 `d969841` |

⇒ per user「現在就能做的先開始，需要機器 + 在現場的事情後續做」，本輪只做不需機器的兩件。

### ✅ MH300 Phase 3-a：冷啟動無條件解除 base-block（`changelog [2026-09-09m1]`）

實作前**回頭複驗原始碼，09-08 的診斷成立**：
`releaseBaseBlockIfNeeded_()` 第一行就是 `if (!base_blocked_) return;`（`MH300_inverter.cpp:233`），
而 `base_blocked_` 是 process-local ⇒ 急停後程式重啟，硬體 `0x2002` b2 仍 set、旗標歸零，
**B.B 永遠不清、馬達靜默不轉**。原 `init()` 只在**讀到 error code 才** `clearAlarm()`（`:88`），
而 B.B 是輔助命令位元不是 alarm，**那條 `if` 冷啟動時不會成立**。

改法：Mode B probe 成功後、`return false` 之前，無條件 `writeParam(REG_AUX_CMD, 0x0000)`。

🎯 **一個值得記的實作選擇：寫入失敗時把 `base_blocked_` 設 `true` 而不是 `false`。**
設 `false` 會把「硬體可能還 set 著」記成「已清」＝**把同一個靜默失效再造一次**；
設 `true` 則交給既有的 best-effort 重試路徑在第一個 run 命令時再清。
📌 **旗標的初值／失敗值要選「會多做一次事」的那一邊，不要選「會跳過」的那一邊。**
這正是本條缺陷的成因反過來用 —— `base_blocked_ = false` 的預設值就是它會靜默失效的原因。

🔴 **狀態：未部署、未實機驗證。** 現役是 SE3（`main.cpp:127` `CRANE_VFD_IS_SE3 1`），
這條路徑目前根本不會被執行；且吊機程式未啟動、本體未開機。
只有 `g++ -fsyntax-only` exit 0。**真正的驗收要等換上 MH300：急停 → 重啟程式 → 下 run，馬達要轉。**

#### 🔎 順帶查證：SE3（現役）沒有同型的洞 —— 這是負面結果，但值得留

依 09-08 立下的通則「凡是用軟體旗標追蹤硬體狀態，都要問一句行程重啟之後呢」，回頭查了現役 SE3：

- `cu_mode_set_` 初值 `false`，而 `false` 的語意是「**要去寫** H1000=0」⇒ 重啟後會重做，**安全方向**
- SE3 的 **MRS 與 run 同在一個命令字**（b7），寫 run 命令本身就清掉 MRS，**根本不需要旗標**

⇒ **本洞是 MH300 獨有，不是現役 live bug。**
📌 差別不在誰寫得好，在於 **MH300 把 B.B 放在跟 run 不同的暫存器（`0x2002` vs `0x2000`）**，
才被迫用跨呼叫的狀態去記。**「需要記住狀態」本身就是風險訊號。**

### 🆕 為現場的電氣測試預先備好量測腳本 `scripts/link_probe.sh`（`changelog [2026-09-09m2]`）

09-08 那組受控實驗是臨場拼出來的、**沒有留下腳本**，而它現在是**基準線**
（HOLD 期間 隧道 9/90 ＝ 90% 丟包／WiFi 90/90 ＝ 0%）。改電源後要拿來對照的就是這組數字。
📌 **對照實驗的價值在於方法相同，所以方法必須能重跑** —— 趁機器還沒到位先做好，
現場就只剩「改電源 → 跑 → 比」。

🔴 **必須在本體 `192.168.5.26` 上跑**（只有它同時看得到隧道側 `.1.10` 與 WiFi 側 `.5.25`）。

🎯 **把 09-08 踩過的兩個量測坑寫進腳本裡**：
- **跨窗口的假空窗** —— 09-08 輸出裡那句「其餘時間最長空窗 ~20s」是統計假象（把跨越 hold
  窗口的斷點算成一個空窗）。腳本只在**窗口同一側**的相鄰樣本之間算空窗。
- **負的丟包率夾到 0** —— 預期筆數是估計值，收到的可能多一兩筆。
  **不要讓一個估計誤差看起來像一個發現。**

合成資料驗證通過（隧道 88.5%／WiFi 0.0%／假空窗排除後報 0.200 秒 ＝ 正常間隔，
不排除的話會報 ~17.6 秒）。🔴 **未在實機跑過**（本體未開機）。

### 🔄 已派給 AI-2：吊機 watchdog 心跳不分連線來源（只診斷、不改碼）

邊界已明確交代：不改碼／不部署／不送會動機器的指令／**不寫 `changelog.md` 與 `work_log.md`**
（避免兩條線同時寫同一批檔），產出寫到 `tmp/ai2-watchdog-handoff.md`，由我 review 後併入並呈使用者拍板。
另要求它**以原始碼為準**，若我的假說不成立就直接說不成立。

### 🔴🔴 AI-2 的 watchdog 診斷回來了，並翻出一條更嚴重的：本體端的吊機 watchdog 是死碼

AI-2 只做診斷未改碼（依交辦邊界），產出在 `tmp/ai2-watchdog-handoff.md`。
**我逐項獨立複驗，四點全部成立**（它自己要求對 §2.3 獨立驗證，因為那是安全相關的主張）：

| 主張 | 我的複驗 |
|---|---|
| 心跳不分連線來源 | ✅ `touch_heartbeat()` **全檔單一呼叫點** `main.cpp:4984`；`last_ping_ms` 單一讀取點 `:1740` |
| web_backend 走 loopback | ✅ `runbook.md:137` 與 `:795` 兩處啟動命令都是 `CRANE_IP=127.0.0.1` |
| **本體端 watchdog 是死碼** | ✅ `crane_last_ok_ms_` ＝ 1 宣告 + 1 初始化 + **3 寫入 + 3 註解 + 0 讀取**；`WATCHDOG_TIMEOUT_MS` **只有定義與 2 行註解，零運算式** |
| roll 不在監看範圍內 | ✅ 採納它的修正（見下） |

#### ⚠️ 要修正 09-08 的一條敘述

09-08 我寫的是「roll 斷了 7.2 秒、watchdog 該響卻被 GUI 蓋住」。**前半不成立。**
roll 有自己的獨立時效檢查（`imu_roll_fresh()`／`IMU_ROLL_STALE_MS=750`／`main.cpp:1581` 每 tick 重判），
而且**當天它正常運作**——「平衡全程退回 `src=meter`」正是它在動作，不是失效的徵兆。

📌 **正確說法：watchdog 沒響不是因為 roll 的問題被蓋住，而是 watchdog 從來不看 roll。**
它問的是「還有沒有人跟我講話」，而因為 web_backend 跑在吊機自己身上（`CRANE_IP=127.0.0.1`，
刻意設計，為了本體掛掉時還能救援），那個答案**永遠是有**。
⇒ 缺陷比 09-08 寫的更徹底：**這個 watchdog 在結構上不可能偵測任何網路故障**，不只是被蓋住。

#### 🔴🔴 更嚴重的新發現：反方向根本沒有存活偵測

本體端「監看吊機」的 watchdog **是死碼**。它**曾經存在**（`9f33c6a`，2026-04-15，有完整的
`ping` + `elapsed > WATCHDOG_TIMEOUT_MS → abort_flag`），到 `4d1409c`（06-22 那個把一個月壓成一筆的
大 commit）就沒了。**而餵它的機構全部留著、還在長大**：1Hz keepalive、`motion_progress` 刷新
（2026-05 為了「避免 false-abort」特地加的）、三段解釋「為什麼要避免誤觸發」的註解。

⇒ **一個沒有消費者的計時器，養著一整套餵食機構，還附三段解釋為什麼要餵它。**

🔴 **後果：吊機↔本體這條鏈路，兩個方向目前都沒有可運作的存活偵測。**
「隧道斷掉時本體會不會自己停下來」**目前沒有答案** —— 已另派 AI-2 追 `crane_cmd_` 的失敗路徑
是否構成替代保護；但那是「送指令才會發現」，**不是背景偵測**，hold／等待期間沒有覆蓋。

📌 **通則（值得進最上層踩坑索引）：判斷一個保護機制在不在，要找「誰讀這個值」，不是找「誰寫這個值」。**
寫入點、常數、餵食執行緒、解釋註解可以全部健在，而**比較那一行不存在** —— `grep` 到符號會讓人以為它活著。

🔴 **待拍板：補回來還是明確刪掉。不能留現狀** —— 現狀最糟，因為它**看起來像有保護**。
修法我同意 AI-2 建議的選項 2 先行（`cmd_status` 加分來源的 `peer_age_ms` + 超標發 EVT 但不 abort），
理由是**訂門檻需要「本體鏈路實際斷多久」這個數字，而目前沒有人有**（7.2 秒是 roll age 不是鏈路 age）。
📌 本條**不需要排在干擾後面**，它不涉及調鬆任何門檻。

### 🔴 WiFi 冷路徑：閒置時本體→吊機單向丟包 80~100%（per user 提示去查路由器）

本體上線後的唯讀盤點就撞到：**本體 ping 吊機 `192.168.5.25` 100% → 80% 丟包，
而同一時刻吊機 ping 本體 0%**（兩者對 router `.1` 都是 0%）。**單向。**
暖機之後 1Hz ×10 與 5Hz ×50 各測**都回到 0% 丟包 / 8.5ms** ⇒ **是冷路徑，不是故障。**

#### 排除了什麼（登入 Vigor2915ac 4.4.7 實查，per user 建議）

| 假說 | 證據 | 判定 |
|---|---|---|
| **ARP 過時** | 路由器 ARP 表兩台都在、MAC 正確、**連 HOST ID 都解出來**（`raspberry-cran`／`washrobot`）；本體自己 `ip neigh` 亦為 `REACHABLE` | ❌ **不是主因** |
| **AP client isolation** | `wl_dual stalist show`：兩台都是 status **`A`（WPA2 已連線）**、無 `B`（Access Control 擋） | ❌ 排除 |
| **WiFi 省電（station 側）** | 吊機＝**Pi 4 + `brcmfmac`**、NetworkManager 管、`802-11-wireless.powersave: 0 (default)`＝**沒有明確關閉、吃驅動預設**（brcmfmac 預設為開）；症狀（入向掉、出向不掉、反向流量一送就醒）完全吻合 | 🟡 **主要假說，未證實** |

🔴 **未證實的原因要記下來：兩台都沒裝 `iw` 也沒有 `iwconfig`，讀不到 `power_save` 實際值。**
現有結論是「預設沒被關」的推論，不是量測。**不要寫成已確認。**
決定性做法（待 per user 拍板，一次一個變因）：裝 `iw` 讀值（純觀測、最小），
或 `nmcli connection modify CP_AOA_5G 802-11-wireless.powersave 2` 關掉再測。

📌 **順帶記一個獨立的觀察**：兩台走的是 `CP_AOA_5G`，而該 5GHz radio 上**共 10 個 station**
（桌機／筆電／手機都在）。吊機的控制鏈路與辦公裝置共用同一個射頻——與 VFD 那件事無關，但是另一個脆弱點。
（`wl stalist` 是 **2.4G** 清單，只看得到 Tesla 充電樁一台；5G 要用 **`wl_dual`** ——我一開始找錯清單。）

#### 🔴 為什麼這件事比它看起來重要：它會污染電氣 A/B 測試

冷路徑丟包**長得跟 VFD 干擾一模一樣**，會把「路徑本來就冷」讀成「改電源有效／無效」。
✅ 已寫進 `scripts/link_probe.sh` 的 `preflight`（丟棄式暖機 burst → 再量；暖機後仍在丟才算真故障），
本體上實跑通過（兩條路徑皆 0%）。

🐛 **過程中修掉我自己造的一個靜默失敗**：preflight 第一版的暖機是「叫對面反向 ping」，
需要本體能 SSH 到吊機——**而它沒有吊機的 host key**（實測 `Host key verification failed`），
整段被 `|| true` 吞掉 ⇒ **暖機從來沒發生過，而輸出跟暖成功一模一樣**（當時會過只是因為路徑早被我 ping 熱了）。
📌 **防呆本身不可以有靜默失敗的分支。** 已改成不依賴 SSH 的丟棄式 burst。
（另一個自傷：改 `offset` 時 python 切片一次切掉了 `warm_and_check` 與 `cmd_preflight` 兩個函式，
`bash -n` 過但執行期才炸 —— **語法檢查不會告訴你少了一個函式**。）

🐛 **DrayTek 短時間重複開 SSH 會被拒**（`Error reading SSH protocol banner`）在 **Vigor2915ac 再次證實**
（先前記於 Vigor2925）：實測 **40 秒不夠、60 秒可以**。⇒ 要問的一次問完，分次就加**有上限**的重試。

### 🔴🔴 AI-2 第二輪：「隧道斷掉本體會不會自己停？」答案是**不會**

產出在 `tmp/ai2-crane-cmd-protection.md`。**我逐條複驗原始碼，全部成立**（含它自陳未實測的部分另標）：

| 主張 | 我的複驗 |
|---|---|
| `crane_keepalive_thread_` 自 2026-05-15 起未啟動 | ✅ `WASH_ROBOT.cpp:363-370` 整段註解，原文 `New design: no continuous ping. Each crane_cmd_ self-heals on fail.` |
| `handle_crane_evt_` 全樹單一呼叫點、在 `crane_cmd_` drain 迴圈裡 | ✅ 唯一呼叫點 `WASH_ROBOT.cpp:783` |
| 兩條急停路徑丟棄回傳值 | ✅ `wash_robot_commands.cpp:3636`、`WASH_ROBOT.cpp:3027`，皆 `crane_cmd_("stop", 2);` 無檢查 |
| `crane_attached_=off` 回合成 `"OK skipped"` | ✅ `WASH_ROBOT.cpp:728` |
| `imu_push_loop_` 讀完回覆直接丟掉 | ✅ `WASH_ROBOT.cpp:2941-2946`，`receiveData` 進 `buf` 後不處理 |
| `server.js` 不在 ws close 補送 off | ✅ **`off` 與 `hold` 皆 0 命中**（270 行）——三法交叉：ugrep／GNU grep／Python 位元組計數，**正對照 `socket` 命中 7** ⇒ 不是 grep 失效 |

📌 **關鍵結論不是「隧道斷多久」，而是「本體當下有沒有正在送指令」**：
送指令期間會發現（分四級：真的會停 7 處／中止回 ERR 3 處／只記 log 往下跑 4 處／回傳值直接丟棄 4 處），
但 **hold／閒置／等 `PausedOnError`／本體自己在跑推桿走行期間，一個 `crane_cmd_` 都不會送 ⇒ 零偵測**。
⇒ **`crane_cmd_` 的自癒是「送指令才會發現」，不是偵測。**

🔴 **最值得記的一條**：`crane_keepalive` 被拿掉的決定**本身沒錯**（是為了解決 zombie TCP socket
造成的 watchdog 誤觸發），**錯的是前提** —— 「會有 `crane_cmd_`」只在動作流程中成立。
連續 ping 拿掉的同時唯一的背景偵測也沒了，而替代方案只覆蓋動作流程。
📌 **通則：拿掉一個機制時，要問「它原本覆蓋的情境，替代方案覆蓋得到嗎」，
而不只是「替代方案能不能做到同一件事」。**

### 🔴 Phase 2.3（鏈路可見度）可行性：素樸版**不該上**，AI-2 自己推翻了上一輪的「最低成本」

派 AI-2 讀碼評估「把 `imu_push_loop_` 讀到的東西餵給 `handle_crane_evt_`」。產出 `tmp/ai2-peer-visibility.md`。
**我逐條複驗，全部成立。**

**執行緒安全本身是乾淨的**（我原本的疑慮方向對、但不是真正的成本）：`handle_crane_evt_` 碰到的東西
不是 atomic 就是已由 `crane_alarm_mtx_` 保護且今天就已跨執行緒；`crane_imu_mtx_` **全樹只有一個使用者**
（`:2995` + `.h:1573`，已複驗）⇒ 鎖序無環、不需新增任何鎖。

#### 🔴🔴 真正的成本：一個慢的瀏覽器 client 會讓吊機平衡靜默退回 `src=meter`

```
TCP_server::broadcast() → std::lock_guard lock(clients_mtx);
                          for (sock : clients) send(sock, buf, len, SEND_FLAGS);
```
🎯 **我複驗了 AI-2 沒查的那個決定性細節：`SEND_FLAGS = MSG_NOSIGNAL`，沒有 `MSG_DONTWAIT`**，
且 `TCP_server` 從未設非阻塞、也沒有 `SO_SNDTIMEO`（三者 grep 皆無命中）⇒ **`send()` 真的會阻塞。**

今天反壓落在 `crane_cmd_`（秒級 timeout，容忍度高）。接上 push loop 後會同時落在 **roll 路徑**，
而 roll 有 `IMU_ROLL_STALE_MS = 750` 的硬懸崖 ⇒ push 執行緒卡在 `send()` → 停推 roll →
吊機 750ms 後判定過期 → **平衡退回 `src=meter`**。

📌 **這是 09-08 那條待辦的反向版本**：那次是隧道讓 roll 過期，這次會變成**我們自己的 GUI 讓它過期**。
而「長時間、大半無人盯著、分頁開著沒人看」正是 10 週期長測的形狀。

#### 另外兩條

- **EVT 會被轉播兩次**（吊機推播給所有 client，本體三條 socket 各收一份）⇒ 會讓人以為發生了兩次。
- 🔴 **行緩衝是必要項不是保險**：`recv(sock, buf, bufSize-1, 0)` 配 `char buf[128]` ⇒ 單次最多 **127 bytes**，
  而吊機 `make_device_state_line()` 遠超過（已複驗）⇒ **必定被切**，`find("tension_alarm")` 對半行回 `npos`
  ⇒ **警報被靜默吃掉 —— 正好是我們要補的那個洞的同一種失敗形狀。**

#### ✅ 修正後的建議（Option B，我同意）

把 `handle_crane_evt_` 拆成 **`record_crane_evt_`**（atomic + 短鎖，**不印不轉播**，任何執行緒可呼叫）
與 `handle_crane_evt_`（記錄+印+轉播，只留在 `crane_cmd_` 路徑）。push loop 只叫前者
⇒ **三個成本同時消失，安全效益完全保留。**

🔴 **順帶查出一個獨立的既有 bug**：`imu_push_loop_` 自己就有**無逾時的 `connectToServer`**
（`WASH_ROBOT.cpp:2995-3001`，已複驗）—— 與我今天從 `crane_stop_estop_` 拿掉的**一模一樣**。
掉包情境沒事，但**主機不可達時每輪卡 ~127 秒 ⇒ 拿它當探針時，在最該報警的情境下是瞎的。**

📌 **AI-2 說明了它上一輪為何漏掉**：那時是從「呼叫點在哪」看這個函式（只被呼叫一次 ⇒ 加一個呼叫者很便宜），
**沒看它裡面做了什麼**。⇒ 🔴 **通則：「誰讀這個值」問的是機制在不在；
「這個函式對它的呼叫者索取什麼」問的才是成本。** 兩個問題都要問。

⚠️ **未實測**：反壓是結構性推導。我複驗了 `SEND_FLAGS`／非阻塞／`SO_SNDTIMEO` 三項、**機制成立**，
但**沒有實際製造一個慢 client 量過**。


### 🎯 上機：今日改動端對端驗證通過 ＋ 一次差點往上拉的座標事故

**啟動照 runbook §A0**（吊機 → 兩個 WEB → 本體，`<D>=0909`，帶 `FCV_EP_CRANE_HOST=192.168.5.25` 走 WiFi）。

#### ✅ 今日三個改動全部實機證實

| 驗收點 | 結果 |
|---|---|
| `[OK] crane estop bypass channel connected` | ✅ 出現（`m3` 的 init 預熱） |
| `[OK] crane IMU push channel connected` | ✅ 出現（`m5`） |
| `cmd_status` 新欄位 | ✅ `crane_peer_age_ms=198`／`crane_peer_fresh=1`（對得上 250ms 推送週期） |
| roll 推送未被行緩衝改壞 | ✅ 吊機 `imu_roll_fresh=1`／`imu_roll_age_ms=213` |
| `balance_source` | ✅ `imu` —— **實跑讀回，正面推翻待辦表「已改但尚未建置部署」那條過期記載** |

#### 🔴🔴 座標基準：`zero_meters top` 之後，腳本會叫機器往上拉 —— 而它已經在頂端

per user 依序設地面點與頂樓點：`zero_meters ground`（底端，`left_after=0 right_after=0`）
→ 上到頂端 → `zero_meters top` ⇒ **`home_ground_cm = 221`**。

🔴 **但那使座標變成「0＝頂」，而兩支測試腳本要的是「0＝底、往上為負」。**
用實際讀值模擬 `plan_leg`：

```
機器在頂端(height=0) → 腳本決定 retract 231cm (上)  區間守衛=通過 → 預期高度 231
```

⇒ **這正是 `mission_run.py` 註解裡記載的 09-01 事故，一字不差**
（「機器當時在頂端（L=0），retract 等於從頂端再往上拉 229cm，會把機器拉進吊機/屋頂結構」）。

📌 **09-01 補的那道防線（「方向由實測位置推導，不再假設起點」）防不了這個** ——
它推導用的 `height = -length_left` **本身就預設了「0＝底」**。
🔴 **通則：把「假設」換成「推導」只有在推導的前提穩定時才是進步。座標基準就是那個前提。**
（`cycle_test.py` 的起點檢查 `abs(L0-TOP)>TOL` 會擋下＝守衛有效；`mission_run.py` 不會。）

✅ 處置：降到底端 → `zero_meters ground` → 回頂端，座標還原為腳本要的語意。

#### 📌 兩條記載更正

- ❌ 我說「`zero_meters ground` 會把 `home_ground_cm` 沖掉」—— **實測沒有**。
  只有 `top` 模式會寫那個值，`ground` 完全不碰 ⇒ **韌體的 home/remaining 與腳本座標可以並存**，不必二選一。
- 🔴 **跨距量到三個值**：09-03 記 **231**／今日往上量 **221**／往下再往上 **224**。
  同一趟來回差 2cm（對應 09-01 記的「單側收繩固定過衝約 1cm」）。
  ⇒ **`TOP` 不是精確常數，是每次來回會漂 ±2cm 的量**；端點容差 `TOL=5` 涵蓋得住，但要知道它會漂。

#### 🔴 我量錯了時機：靜止值 ≠ 運動中的值

手動分步升降時我每步**停穩後**才取樣，得到「左右差 ≤2cm、傾角 ≤0.75°」，並據此說「下降過程非常乾淨」。
**那是錯的** —— 腳本在**運動中**以 4Hz 取樣，第一趟就抓到 `roll -5.06°`。
📌 **通則：宣稱一個過程「乾淨」之前，先確認自己取樣的時機涵蓋那個過程。**
停穩後的值只證明它會收斂，不證明過程中發生什麼。

#### 起步瞬態與兩支腳本的中止語意不一致

見 `changelog [2026-09-09m6]`。要點：5,768 筆取樣中 >5° 只有 2 筆（0.0%），
是**起步瞬態**（`[BAL]` 拍到 250ms 內 0.72°→5.3°，trim 打到 15Hz）；
而 `mission_run.py` **1 筆就中止**，`cycle_test.py` 09-04 就改成**連續 3 筆**並驗證過。
已移植。🔴 **但今日未被驗證**（nearmiss 計數 0 ⇒ 那條路徑沒被走到）。

#### 🐛 我自己踩的三個坑（都是本專案已記載的形狀）

1. **`pgrep -x crane_control_PI.out` 永遠回零匹配**（程序名 >15 字元），它印了 🟢 但根本沒檢查。
   真正的證據是啟動前 `ss` 沒有 `:5002`。
2. **查 `motor_api` 的指令被自己的錯誤訊息命中** —— `|| echo "...motor_api 沒有在跑"`
   字面上就含 `motor_api`。與「`grep "curl/"` 命中『已移除 curl/』」同族。
3. **本機 python3 是 3.8，我自己的診斷腳本也踩到 f-string 反斜線** —— 整段 SyntaxError、
   指令一個都沒送出（機器沒動）。與 `mission_run.py` 那條同一個限制。
📌 三個都是「檢查看起來通過、但不可能失敗」或「量到的不是你以為的東西」。


### 🎯 `mission_run.py 10 224` 第一輪：20/20 完成（走 WiFi，非有線驗收）

完整數據見 `changelog [2026-09-09 實測]`。四個要點：

1. **20/20 無中止**，最大 |roll| **4.49°**、平均 1.67°、單趟下 ~16.7s / 上 ~18.6s
2. 🎯 **`m6` 的 diff 半邊被驗證**：第 7 趟下 `Δmax=9 > 8` 但 3 筆內回復 ⇒ 繼續。
   **改之前會在第 7 趟掛掉。** 🔴 roll 半邊仍未驗證（全程未達 5.0）
3. 📌 **大 roll 與大左右差是同一個事件的兩面**（平衡迴路靠製造左右差修 roll）——
   第 7 趟同時出現 4.49° 與 9cm，證實了 `cycle_test.py` 註解裡早就寫過的機制
4. ⚠️ **離 ±1° 規格還有距離**：10.1% 取樣超出，而這是**最寬鬆條件**（空載／未貼牆／無清潔動作）

#### 🔴 我更正了自己兩次的統計框架

先前說「>5° 只有 2 筆、佔 0.0%」—— **分母錯了**。瞬態只持續一瞬，用取樣筆數當分母必然稀釋它。
用事件數：**23 次下降起步中 2 次明顯事件 ≈ 1/10**。
📌 **通則：瞬態的發生率要用事件數當分母。選錯分母會讓常見事件看起來罕見。**

另外「靜摩擦／靠牆被釋放」那個假說，靜置 5 分鐘刻意重現**失敗**（max 1.52°）⇒ **已排除**。

### 第二輪 10 趟 + 姿態累積量測（📌 已知設計前提，非故障）

第二輪 20/20 完成，但姿態指標全面較差（出帶 10.1% → **24.2%**、各趟最大 roll 平均 1.67° → **2.54°**）。
⚠️ 兩輪起跑姿態不同（−0.27° vs +2.01°），依「run-to-run 變異 ≈ 效果量」**不逕稱退化**。

**頂/底交叉量測**（完整表在 `changelog`）：頂端 **3.72°** vs 底端 **1.38°**、靜置 1.7 分不回復；
頂端四次量測單調上升 `−0.27 → 2.01 → 2.99 → 3.72°`，張力差 `19.0 → 29.5 kg`，
**而繩長讀值始終相等**。

🔴 **框架由 per user 當場更正**：我原本寫成「不是隨機變異、值得追」，語氣像找到故障。
**捲筒本來就是這樣，`計米器 + IMU 修正` 正是為了補償它。**
⇒ 本次數據的價值是**把這個條件量化**（每 10 趟來回約 +1°／+3kg，頂底差 2.3°），不是發現缺陷。

📌 **我自己的兩次修正**：① 先說「下降讓傾角掉一半 ⇒ 排除不可逆偏移」——推論太快，
位置相關性正好是預期表現；② 框架錯置（見上）。

**`m6` 的 roll 半邊在 40 次橫越後仍未驗證**（1,409 筆無一超過 5.0°）。

### 🔴 `cycle_test` 10 週期在週期 1 步 4 中止 —— 兩個根因都定位了

完整數據見 `changelog`。**中止是正確的**（roll 連續 3 筆 >6°、最後 8.00°，持續傾斜非擺盪）。

**根因一：`roll_recover` 的守衛前提錯誤。** 它用「繩長齊平」當「繩長已經沒問題」的代理指標，
而本日**五個獨立資料點**證明這台機器**齊平時就是歪 +2.9~3.8°**
（其中一組是**平衡迴路自己動出來的**，不是我下的指令）。
⇒ 守衛把繩長往齊平修＝往最歪的方向修，然後在最歪處宣告「不是繩長問題」。
🟢 修法：改用**「這次修正有沒有讓 |roll| 改善」**當停止條件——不需要知道水平點在哪。

**根因二：平衡迴路在繩子不動時根本不跑。**
`hold_loop()` 的 `apply_balance_trim` 被 `cur_sync_dir != 0` 圈住
⇒ **滑台橫走 0→100→0 的 6.3 秒完全沒有平衡**。機體被甩到 +3~4° 而沒人修，
要等下一個 40cm 移動才補一部分 —— 這同時解釋了「每步停後 roll +3~4°」與「左右差惡化」。

✅ 風扇全程正常（`0902c` 的中止原因未重現）、推桿無 TIMEOUT、手臂未動。
⚠️ 真空吸附不穩（同輪 4 顆／2 顆／2 顆）。

收尾：`roll_correct +1` 修到 +0.63°、分段收回頂端，最終 **223 / roll +0.75° / 差 2cm**。
本體 `state=error` per user 保留。

### 🎯 `roll_correct` 的解析度：1cm ≈ 3~4°，**±1° 的目標帶比一個最小步還窄**

2026-09-09 收尾時實地撞到：
```
+3.27° → roll_correct +1 → +1.06°   (繩長差 3)
+1.06° → roll_correct +1 → −2.96°   (繩長差 7)   ← 越過零點
🔴 新守衛攔下：沒改善，停
```
⇒ **1cm 的修正約 −3°，而我把收斂目標設在 ±1°** —— 那個帶窄於一個最小步，**本質上打不中**。
（再 `roll_correct -1` 拉回 **+0.33°**。）

📌 **這解釋了 `cycle_test.py` 的 `ROLL_RECOVER_OK` 為什麼是 3.0 而不是 1.0** ——
**3° 才是 1cm 步距打得到的精度**。我自己的收尾迴圈用 1.0 是錯的門檻。
🔴 **通則：控制目標的容差不能比致動器的最小步還小。** 設了就會來回越界，
而症狀看起來像「修不好」而不是「目標訂錯」。

✅ 附帶：**新的 `roll_recover` 守衛在真實過衝上生效**（`+1.06 → −2.96` 判定沒改善並停手）——
舊版的「繩長齊平」守衛在這個情境下**不會攔**（差 7cm，離齊平很遠），會繼續往下修。

📌 **`roll_correct` 增益的第三個資料點**：本次兩步分別是 −2.2°/cm 與 −4.0°/cm
（記載 −0.85、稍早實測 −4.26）。⇒ **增益本身不是常數**，待辦表那條「換算定義待釐清」更該追。

### 🎯 切 192.168.1 有線鏈路：IMU 掉 1/3，但 `level_diff` 撐住了

完整數據見 `changelog`。四個要點：

1. **切換成功**：不帶 `FCV_EP_CRANE_HOST` 讓自動探測選有線，`ss` 確認三條連線全在 `192.168.1.x`。
   閒置基準比 09-08 記載好（age 平均 203ms、20/20 fresh、0 筆破門檻）。
2. 🔴 **運動中 IMU 掉三分之一**（`src=meter` 33%），代價：移動 5.4 → **11.0 s/步**、
   下行速度掉一半、`roll 超標後自行回復` 0 → **5 次**。
3. ✅ **但 1/1 完成、兩個門檻都沒被逼近** ⇒ **`level_diff=6` 撐住了**。
   🎯 **這是 per user「IMU 連不上就放棄」設計原則的實測驗證**：主線不依賴會斷的訊號。
4. 🔴🔴 **放寬 `IMU_ROLL_STALE_MS` 到 1250 實測更糟、已回退**（見待辦表該列）。

### 🎯 兩段式水平修正第一次完整跑通

`roll_correct +1`（+5.40°→+2.21°）→ `roll_trim_ms +300`（→+1.07°）→ `+150`（→**+0.62°**）
**在 ±1° 規格內**，而稍早只用 `roll_correct` 時 1cm 步階讓我在 +1.06° 與 −2.96° 之間跳、進不了。
📌 繩長差自然收斂到 **6**＝量出來的 `level_diff`，三者彼此印證。

### 🐛 我的操作失誤：另造的監看腳本被背景收掉，資料全失

用 `&` 丟背景，外層指令結束時一起被收。所幸吊機 log 的 `[BAL]` 本來就記 `roll_age` 與 `src=`，
重建的資料**比我要量的更好**（163 個真實 tick，不是抽樣）。
📌 **要量的東西如果系統已經在記，先找現成的，不要另造一個會失敗的量測。**

### 待完成
- 🔴 **未 commit** —— 現為 11 改 + 5 未追蹤（新增 `MH300_inverter.cpp`、`scripts/link_probe.sh`），停在可檢視狀態等 per user
- ✅ ~~AI-2 的 watchdog 診斷~~ → **兩輪都已回報、逐條複驗、併入待辦表（新增 5 列）與本條目**
- 🔴🔴 **等 per user 拍板的安全項（依急迫性排序）**：
  ① **兩條急停路徑丟棄回傳值 + 走會被 `crane_mtx_` 卡住的主通道**（IMU 45° 自動急停尤其）
  ② **本體端零背景偵測** —— 最低成本修法是把 `imu_push_loop_` 已在讀的那條 socket
     丟給 `handle_crane_evt_`（**不新增任何流量**，避開 05-15 的 zombie socket 問題）
  ③ **hold 期間操作者掉線繩子不會停** —— `server.js` 在 `ws close` 補送 off，可獨立先做
  ④ 吊機端 watchdog 修法（選項 2：`cmd_status` 加 `peer_age_ms` 觀測，先不改判定）
  ⑤ 本體端死碼 watchdog：**補回來還是明確刪掉**（留現狀最糟，它看起來像有保護）
- 🟡 **WiFi 省電未證實** —— 要不要裝 `iw` 讀值（純觀測）或直接關 powersave 再測，等 per user
- 🔴 **AI-2 標為未實測的一條要獨立驗證**：`crane_mtx_` 持有期間急停是否真的送不出去
  （由鎖的持有範圍推導，未實機驗證）；另 `water_inlet_watchdog_` 在吊機失聯時的行為 AI-2 未追完
- 🎯🎯 **隧道干擾的電氣測試待現場**：吊機端 Fathom-X 改獨立電源再量一次（供電與 VFD 共用是最強候選）
- 🔴 **console v2 三個 confirm 待拍板**（`pwm save` 寫壽命僅 1~2 千次的 flash）
- 🔴 JC-100 `_pt` 0→5ms、手臂 `motor_api` 未啟動 —— 都要實機
- ⚠️ 上機時**本體啟動一定要帶 `FCV_EP_CRANE_HOST=192.168.5.25`**（不帶不會報錯，只會安靜走上掉 90% 封包的有線）

---

## 2026-09-08（續二）— session 被中斷後的盤點與補缺口；順帶抓到 runbook §A0 已經是錯的

> 🔴 **本輪未送任何會讓機器動作的指令**，也未動任何程式碼（診斷只用 `ping`／`ssh` 唯讀查詢
> `ps`／`ss`／`md5sum`／`nm`／`ls`／`grep`，加上本機 `g++ -fsyntax-only`）。
> 兩條線：本 session `agent-ai-3e` 走盤點與日誌／changelog；`AI-2` 走 runbook 與 build README。

### 起因與盤點結果

使用者的 session 被意外中斷，回報「沒寫到日誌」。**盤點後這個初判要修正一半：**

| 以為 | 實況 |
|---|---|
| 兩個 C++ 修正沒寫進日誌 | ❌ **寫了，而且很完整** —— 在同日「復機」條目裡（20:12 寫入） |
| — | 🔴 **真正的缺口是 changelog 完全沒有** —— 最新條目停在 `[2026-09-08g1]`（19:26，GUI 線），機器線的 `m*` 從缺 |
| — | 🔴 **全部未 commit** —— `main` 停在 `d969841`（09-07），工作區 9 改 + 4 未追蹤，1,322 行 |

📌 **「沒寫到日誌」與「沒寫到 changelog」在中斷當下看起來一樣** ——
都只是「我印象中沒做完」。**分辨方法是去看檔案時間戳，不是回想。**

### ✅ 中斷前那筆工作其實已經全部完成 —— 逐項複驗

被中斷的是**收尾**，不是工作本身：

| 複驗項目 | 結果 |
|---|---|
| 本機 ↔ Pi 原始碼 `md5` | ✅ 三檔全一致（`WASH_ROBOT.cpp`／`.h`／`endpoints.h`） |
| 現役 binary 含新符號 | ✅ `nm -C` 命中 `crane_endpoint_ip_` |
| 行程 | ✅ PID 6624，起於 **20:10:57**，binary 建於 **20:10** |
| 備份鏈 | ✅ `.prev-20260908-2010`（只含 m1）／`.prev-20260908-1950`（09-07 原版）都在 |
| 本機 `g++ -fsyntax-only` | ✅ 重跑 exit 0 |
| 🎯 啟動 log | `[crane] 位址由環境變數覆蓋 = 192.168.5.25（不做有線探測）` → `[OK] crane 192.168.5.25:5002` |

⇒ 最後一行就是 **m2 修好了的證明**：舊寫法在覆蓋值等於 `CRANE_IP` 時印不出這行。

### 🔴🔴 盤點時抓到的新問題：runbook §A0 在寫下 3 小時後就變成錯的

§A0（19:19 寫入）第 3 步的本體啟動指令**沒有帶 `FCV_EP_CRANE_HOST=192.168.5.25`**，
而 19:50／20:10 上線的 `m1`＋`m2` 讓**有線自動探測開始會成功**。照 §A0 跑的後果：

- 走上有線 `192.168.1.10` ⇒ **VFD 一運轉掉 90% 封包**
- 且 §A0 自己的驗收判準寫「`[OK] crane 192.168.5.25:5002`」⇒ **指令與判準互相矛盾**
- 🔴 **失敗沒有徵兆**：init 全 `[OK]`、埠正常開、閒置一切正常，**只有機器真的動起來才壞**

📌 **這是「文件過期」的第四型，而且是最快的一型** ——
前三次是文件落後於實作（VS 建置路徑／runbook 兩處不一致／MH300 計畫書差三個 Phase），
這次是**文件寫下之後、被同一天稍晚的改動作廢**。⇒ 🔴 **當天上線的改動，要回頭掃當天寫的文件。**

### 已補的缺口

| 檔案 | 補了什麼 | 由誰 |
|---|---|---|
| `.claude/changelog.md` | 🆕 `[2026-09-08m1]`（單一位址來源，含 5 處表與兩個部署坑）＋ `[2026-09-08m2]`（`has_host_override`，含三個 binary 對照與「必須記得帶的參數」） | `agent-ai-3e` |
| `.claude/work_log.md` 待辦總表 | 🆕 **7 列**：隧道干擾（電氣）／`IMU_ROLL_STALE_MS` 排序／watchdog 心跳不分來源／MH300 Phase 3-a／`FCV_EP_CRANE_HOST`／console v2 三個 confirm 待拍板／位址單一來源（✅ 結案） | `agent-ai-3e` |
| `.claude/work_log.md` 同日條目 | 修掉 **3 條假待辦**（見下） | `agent-ai-3e` |
| `.claude/runbook.md` | §A0 第 3 步補環境變數 + 「不可省」專節 + 驗收判準補兩種開機 log 原文對照 + 開跑前第 2 點補註；**另修 §連線資訊**（「不是有線比較爛」那句已被受控實驗推翻） | `AI-2` |
| `scripts/build/README.md` | 「埠關掉 ≠ 程式退出」；`pgrep` 反方向陷阱（數行程會**多**數） | `AI-2` |

### 🔴 三條假待辦（都是同一天之內產生又過期的）

| 原文 | 實況 |
|---|---|
| 🟡「`mh300_migration_plan.md` … **尚未改文件**」 | ❌ 同日 **19:21** 由（續）條目做完，且 Phase 3 描述也被推翻改寫 |
| 🟡「runbook §啟動順序第 631 行 … **尚未改**」 | ❌ 同日 **19:19** 做完，處置比原記載更徹底（新增 §A0） |
| 🔴🔴「`IMU_ROLL_STALE_MS=750` **切有線前必須改**」 | ❌ **排序被自己的實驗推翻** —— 同條目稍後已寫「絕對不要先調」「我原本排在第一位的 ①，現在確定是錯的順序」；另「每 5.5 秒破一次」也已收回 |

📌 **前兩條是「兩條線並行時，各自的待完成會互相過期」** ——
機器線 20:12 寫下的待辦，GUI 線 19:19–19:21 就已經做完了。
⇒ 🔴 **收工前的待完成清單，要先掃一次同日另一條線的條目。**
📌 **第三條更值得記：一份條目的「待完成」與它自己的正文互相矛盾。**
正文推翻了前提（不是週期性）與處置順序（先電氣不是先門檻），但清單沒跟著改。
⇒ 🔴 **結論改了，要回頭改由它產生的待辦** —— 與 09-07「`cmd_pump` 修好當天沒回頭改那一列」同型。

### 待完成
- 🔴 **未 commit** —— 停在可檢視狀態等使用者決定（9 個修改檔 + 4 個未追蹤的硬體摘要）
- ✅ ~~本體 `.26` 的 FIFO 殘留~~ → **已清（per user），主程式全程未受影響**
  🔴🔴 **但過程中證實 runbook 收尾那條 `pkill -f "sleep infinity"` 拿來清殘留會關掉機器**：
  主程式 PID 6624 正靠 `wr_0908e_in` 當 stdin，全殺 ⇒ 收到 EOF **當場關機**。
  🐛 **`-f` 的命中數會騙人**：`pgrep -f "sleep infinity"` 命中 **12 個，只有 5 個是真 writer**
  （其餘是命令列含該字串的啟動器 `bash -c`，**連我當下那條查詢指令自己都被算進去**），
  而 5 個真 writer 裡**有 1 個是活的**。
  ✅ 正解：`readlink /proc/<pid>/fd/1` 逐一驗明寫的是哪個 FIFO，比對主程式 `/proc/6624/fd/0`，
  **只殺對不上的**（1622／4694／5410／6162），其啟動器一殺 writer 就自己結束了。
  **複驗全過**：6624 存活 57 分、`fd/0` 仍是 `wr_0908e_in`、writer 6621 仍在、`:5001` listening、
  真 writer 剩 1、log 無 `[SHUTDOWN]`。FIFO 檔只留 `wr_0908e_in`。
  📌 **已補進 `runbook.md` 收尾段**（🔴🔴 警語 + 三步驟的安全做法 + 動手後複驗清單）
  與最上層 `work_log.txt` 踩坑索引。
  📌 **這條的性質**：同一條指令在**收尾情境安全、清殘留情境是事故** ——
  差別不在指令而在「當下有沒有東西正靠它活著」。
- 🔴 **JC-100 通訊錯誤仍在**（本輪 20:16:04、20:17:21、20:23:10 各一筆 `TIMEOUT`，約每 3~6 分鐘一筆）
  ⇒ 承接 `_pt` 0→5ms 那條
- ✅ ~~上層 `agent_ai/.claude/work_log.txt` 的 facade 摘要停在 09-07~~ → **已同步**（per user）：
  新增 09-08 條目（隧道干擾為主軸 + 兩個 C++ 修正 + `FCV_EP_CRANE_HOST` + 三個文件過期踩坑 + 7 項待辦），
  另於**踩坑索引**新增 `pkill -f` 那條。

---

## 2026-09-08（續）— 清掉兩處文件矛盾；MH300 遷移實際狀態差三個 Phase；console v2 繼電器手感

> 🔴 **本輪未送任何會讓機器動作的指令**（診斷只用 `ps`／`ss`／`ping`／`strings`／`grep`；
> 唯一的寫入是 `scp` 一支前端靜態檔到吊機的 `~/bringup/web/public_v2/`，未重啟任何行程）。
> 使用者當時正在現場用 8080 做單項操作。
>
> 本輪兩件事：**(1)** 承接同日「尚未改」的兩條文件待辦，實作前先逐項打開原始碼複驗
> ——**結果與日誌裡的推斷不一致**；**(2)** 承接機器線 `agent-ai-8b` 移交的 console v2 GUI 任務。

### 開工狀態確認

| 項目 | 結果 |
|---|---|
| `192.168.5.1`（倉庫 router） | ✅ 通（先驗它才驗目標，見同日 IP 判準） |
| 吊機 `.25` / 本體 `.26` | ✅ 都在，uptime 1h39m / 1h08m |
| 吊機 `crane_control_PI.out` + 兩個 `node` | ✅ 仍在跑（:5002 / :8080 / :8081） |
| 本體 `facade_cleaning_v2.out` | ✅ 仍在跑（:5001） |
| 手臂 `motor_api` | ⚪ **仍未啟動**（維持等 per user 確認的狀態） |

⚠️ **`pgrep -af motor_api` 會命中自己那條 `bash -c`** —— 看起來像「有在跑」。
📌 **同族踩坑**（`ls -1 \| wc -l` 數到目錄、`grep "curl/"` 命中註解）：**量到的不是你以為的東西。**

### ✅ runbook：補上 §A0「日常啟動／收尾」，堵掉「照目錄找會起到舊 binary」

**根因比原本記的那條更嚴重。** 原記載是「§啟動順序第 631 行仍指向 `~/main_20260831/`，與 §0 第 71 行矛盾」，
但實際結構是：

- `~/bringup/` 的做法**只以文字寫在 §0 的警語裡**，全檔**沒有任何可複製的指令**
- 全檔唯一叫「啟動順序」的段落在 **§A4**，而 §A4 是 `main` **分支快照**專用（那是它的正當用途，不是筆誤）

⇒ 照目錄找「啟動順序」的人一定落到 §A4，起到 **08-31 的舊 binary**（吊機 417,024 bytes），
而 init 全 `[OK]`、埠正常開、`status` 有回應 ⇒ **零徵兆**。
📌 **這不是「有一行寫錯」，是「對的做法沒有可執行的形式」** —— 只改那一行不會解決問題。

🔧 **處置**：新增 **§A0（日常啟動／收尾）**，指令由**執行中的 `ps` 逐字取回**（不是憑記憶寫）：
兩台的日期化 FIFO 啟動式、驗收判準表（含 `[OK] crane` 不是 `WARN`）、
`exit` 正規收尾與 FIFO writer 清理。同時把 §A4 的標題改為「⚠️ `main` 分支快照專用」並加警語。

🔴 **順帶更正的事實**：**web 兩個埠都跑在吊機的 `~/bringup/web/`**，不是 §A4 寫的
`~/main_20260831/web_backend/`；靠 `HTTP_PORT` + `PUBLIC_DIR` 區分（v2 指 `public_v2`）。

### ✅ runbook 連線資訊段：09-08 的量測其實推翻了三條記載（原本只登記了一條）

| 原記載 | 實況 |
|---|---|
| 09-03「兩台 eth0 彼此 ping 不通 ⇒ 有線沒串起來」 | ❌ 09-08 雙向各 20 次 **0% 丟包**，已串起來 |
| `CRANE_IP = "192.168.5.17"` | ❌ 實際是 **`192.168.5.25`**（`strings` 實測） |
| 「**eth 串接後要回頭改它**」 | ✅ 已結案 —— eth 通了，但 bench 刻意維持 WiFi |

三條都補進 §連線資訊，並把隧道的延遲分佈表與 `IMU_ROLL_STALE_MS=750` 的關聯一併寫進去
（**理由要換掉**：不是「有線比較爛」，是 bench 上 WiFi 更好且隧道無流量）。

### 🔴🔴 MH300 遷移：計畫書比實作落後三個 Phase，而 Phase 3 的內容本身也是錯的

**逐項打開原始碼複驗**（不是照計畫書或照日誌推斷）：

| Phase | 計畫書原本 | 實測 | 證據 |
|---|---|---|---|
| 1 build 清單 | 待做 | ✅ 已完成 | `scripts/build/build_crane.sh:10-11` 兩支 driver 都在 |
| 2 型別 swap | 待做 | ✅ 已完成 | `main.cpp:127` `#define CRANE_VFD_IS_SE3 1` + `using CraneVFD` 的 `#if` 切換 |
| 4 識別碼改名 | 待做 | ✅ 已完成 | `se3_left=`／`dev_se3_` 在 `main.cpp` 命中 **0**；`public/` 與 `public_v2/` 同樣 0 |
| **3 邏輯差異** | 待做 | 🔴 仍待做，**但描述要整段改寫** | 見下 |

🎯 **換硬體現在只要翻 `main.cpp:127` 那一個巨集。**

🔴 **Phase 3 的兩點都被實測推翻：**
1. 計畫書列的六個 callsite 行號（L951-952／L985／L2429／L3069-3070／L3214-3215）**全是分層重構前的**。
   現行位置是 L1417-1418／L1457-1458／L3163-3164／L4020-4021／L4104-4105／L4122-4123／L4190-4191。
2. **「急停後馬達被 base-block 卡死」在正常路徑上不會發生** —— driver 早就補過：
   `emergencyStop()` 設 `base_blocked_`，`runForward/Reverse()` 進來先 `releaseBaseBlockIfNeeded_()`
   寫 `0x2002 = 0`（`MH300_inverter.cpp:233/240/244/254`），`clearAlarm()` 也會清（`:282`）。
   ⇒ **SE3 語意已在 driver 層還原，app 端那七處 callsite 不必改。**

🔴🔴 **但洞還在，而且更隱蔽：`base_blocked_` 是 process-local 的軟體旗標。**

| 情境 | 硬體 `0x2002` B.B | 軟體 `base_blocked_` | 結果 |
|---|---|---|---|
| 急停 → 下一個 run（同一次執行） | set | `true` | ✅ 自動清掉 |
| **急停 → 程式重啟 → run** | **仍 set** | **`false`**（新物件） | 🔴 `releaseBaseBlockIfNeeded_()` 直接 return，**B.B 永遠不清** |

**重啟包含 `exit` 正常收尾、crash、Pi 重開機。** 症狀：馬達不轉、**Modbus 寫入回報成功、
無 fault code、無任何錯誤訊息**。
🔴 **`init()` 補不了**：它只在**讀到 error code 才** `clearAlarm()`，而 base-block 是輔助命令位元、**不是 alarm**。
✅ **修法**：`init()` Mode B probe 成功後**無條件** `writeParam(REG_AUX_CMD, 0x0000)`。代價是每次 init 多一筆寫入。

📌 **這一條的通則**：driver 補 parity 補得對，但**補在軟體旗標上 ＝ 把硬體狀態鏡射成行程狀態**，
於是缺陷從「一直會發生」變成「**只在跨行程邊界發生**」——更難遇到，也更難查。
**凡是用軟體旗標追蹤硬體狀態的地方，都要問一句「行程重啟之後呢」。**

📌 **同型第三次**：09-07「VS 早就不是建置路徑而三份文件都說它是」／09-08 上午「runbook 兩處不一致」／
本輪「計畫書把已完成的說成待做」。**落後的方向都是同一個：文件把已完成的事說成待做，
於是真正的風險被埋在假待辦中間。**⇒ 🔴 **文件描述的待辦狀態本身要定期實測。**
...
**已部署 `:8081`，版號 `2026.09.07-2222` → `2026.09.08-1925`。** 細節見 changelog `[2026-09-08g1]`。

🔴 **使用者的假設不成立**（機器線實測，本輪沿用未重測）：延遲**不是** bus 被輪詢佔住——
`status` p50 = 1ms、`relay_status` 9~10ms，且繼電器在 `.20`、JC-100 在 `.22`＝不同閘道不同連線。
**真因是前端 `fire()` 結尾那行寫死的 `setTimeout(..., 700)`**：指令 ~30ms 就完成，按鈕卻要等重讀完才翻。

**② 的關鍵事實**：後端回覆本身就是回讀結果（`pqw_set_relay_verified_()` 寫入→回讀→重試 3 次，
`OK ch<N>=<0|1>` 的數字讀自 PQW）⇒ 新增 `applyRelayEcho()` 立刻套用。
📌 **不是樂觀更新**，用的是回讀真值，沒有違背原註解「不相信回傳字串」的原意。

🐛 **實作時踩到的坑（移交單未提）**：不能用 `paintRelay()` 那個寬鬆的 `/ch(\d+)=([01])\b/` 掃回覆——
`pwm status` 回 `ch1=5,50,65535,1`，**duty 為 0 或 1 時 `\b` 在逗號前成立** ⇒ 會跑去翻繼電器 CH1 的開關。
已窄化為「只認 `chKey` 指名的那一個通道」。另 `delete pending[chKey]` 必須排在 echo 之前，
否則 `paintRelay` 的 `if (pending['ch'+ch]) return` 會把該列跳過。

🔴 **副作用，已交回使用者決定（未自行處置）**：通用 `[data-cmd]` handler 是共用的，
拿掉 confirm 之後**三個非繼電器按鈕也失去確認**——
`pwm save`（🔴 寫 QX-DO24 flash，非 `0x00~0x0F` 暫存器**實時寫 flash 且壽命僅 1~2 千次**，
誤觸消耗不可回復的寫入壽命）／`zdt_zero`（重新定義原點，與**仍保留** confirm 的手臂 `INIT` 同類風險）／
`rail_cfg_soft_enable`。
⚠️ 且產生一處不一致：`rail-zero` 就在隔壁但有**自己** hardcoded 的 confirm ⇒ **它還是會跳窗**。
📌 使用者原話範圍是「操作繼電器」，這三個是被共用 handler 順帶掃到的。

✅ 驗證：`check_console.js` 四項全過 exit 0／本機↔Pi md5 逐位元一致／`:8081` 實取 200 · 155,003 bytes ·
版號正確 · `applyRelayEcho` 命中 3 處／`:8080` 未受影響（200 · 47,577 bytes）／未重啟 node（PID 1744/1745 未動）。
Pi 上備份 `index.html.bak-20260908-1925`。🔴 **尚未經真人瀏覽器**。

### 待完成
- 🔴 **console v2 需使用者重新整理 `:8081` 後實按一次繼電器**確認手感（已請機器線轉告）
- 🔴 **`pwm save` / `zdt_zero` / `rail_cfg_soft_enable` 的確認要不要補回**（見上，等使用者拍板）；
  順帶決定 `rail-zero` 那個 hardcoded confirm 要不要一起拿掉
- 🔴🔴 **MH300 Phase 3-a：`init()` 無條件清 `0x2002`**（換硬體前必做，一行等級，本輪只診斷未改碼；**屬 C++ 線**）
- 🔴🔴 ~~**`IMU_ROLL_STALE_MS=750`**（切有線前必改）~~ → **排序已被同日的受控實驗推翻**：
  真正的第一順位是**隧道干擾（電氣）**，VFD 一運轉隧道掉 90% 封包。**先調門檻等於把它蓋住。**
  詳見同日「復機」條目的 #2 結案與其待完成 —— 承同日上午
- 🔴 **JC-100 通訊錯誤持續中** ⇒ 重新評估 USR 網關 `_pt` 0→5ms —— 承同日上午
- 🟡 MH300 Phase 3-b／3-c（keepalive 簡化、fault code 對照表）仍照原計畫，未動
- 🟡 量四個吸盤到牆面的間距／吊機 `set_imu_roll` log 節流 —— 承同日上午
- 🔴 手臂 `motor_api` 未啟動，等 per user 確認 —— 承同日上午
- ⚪ 本輪所有改動**僅文件**，無程式碼變更 ⇒ 不需重編、不需部署

---

## 2026-09-08 — 復機：吊機 + 兩個 WEB（本體未開機）

### 啟動結果

per user「把吊機程式跑起來 還有 WEB」。**本體（washrobot）今天沒開**，所以只起吊機那一側。

| 項目 | 結果 |
|---|---|
| 吊機 `crane_control_PI.out` | ✅ `command server :5002`；VFD keepalive 10 tick 全 `fail=0` |
| WEB 8080（`public/`） | ✅ `listening 0.0.0.0:8080`，`[crane] connected 127.0.0.1:5002` |
| WEB 8081（console v2 `public_v2/`） | ✅ 同上；外部 `curl` 200 / 152,068 bytes，版號 `2026.09.07-2222` |
| 本體 `:5001` | ⚪ 未開機（web 顯示 washrobot disconnected＝正確反映） |

🔴 **跑的是 `~/bringup/` 的 09-07 新版 binary**（502,744 bytes，含當天那四個 C++ 改動），
**不是** `~/main_20260831/` 的舊快照（417,024 bytes）。
⚠️ **runbook 這兩處不一致**：§0 第 71 行寫「現行實際做法是直接跑 `~/bringup/`」（對），
但 §「啟動順序」第 631 行的指令仍指向 `~/main_20260831/launch.sh`（舊快照）。
照後者跑會起到 08-31 的 binary，**而且不會有任何徵兆**。

### 🎯 09-07「部署後第一件事」結案：`balance_source=imu` 撐過重開機

開機 log 逐字出現：
`[INFO] balance_source=imu (編譯預設；執行期可用 set_balance_source 改，**不持久化**)`
＋ 隨後的 WARN（roll 從未收到 → 平衡實走計米器路徑，資料一來會自動切）。
`status` 回讀 `balance_source=imu`、`imu_roll_fresh=0`。
⇒ 09-07 `m1` 那個編譯預設改動**第一次經過真正的冷重開機驗證**，待辦表該列可結。
📌 `imu_roll_fresh=0` 是**正確**的，不是故障——本體沒開就沒有人推 roll。

### 🎯 順帶實測確認兩條記載

- `motion_hz=50` —— 09-07 更正 console v2 Setting 表「編譯預設寫 30、實際是 50」，**今日重開機再次確認是 50**。
- `EVT device_state` 與 09-07 逐欄相同，含既有的 `pqw_water=0`
  （`[PQW:12] init presence probe failed` 是既有狀態，不是今天新壞的）；`length_middle=ERR` 亦為既有（SD76 middle 未安裝）。

### ⚠️ 重開機後回到預設的執行期值

`status` 實測 `fine_adjust_level_diff_cm=0`，**不是 09-04/09-07 用的 5**。
⇒ 待辦表「跑 `cycle_test` 前必須先確認」那列**仍然有效**，且現在只剩這一項
（`balance_source` 已由編譯預設解決，`motion_hz` 本來就是 50）。

### 📌 IP 沒有漂，只是開機慢

一開始 `.25`／`.26` 都 ping 不到，`.27`／`.29` 反而在。**不是 IP 漂了**——
吊機上電後 `.25` 就回來了（`hostname` 確認 `raspberry-cran`）。
📌 **VPN 通不通要用倉庫 router `192.168.5.1` 判斷，不要用目標主機**：
當時 `.1` 與 WAN `1.34.178.194` 都 OK，`.25`/`.26` 不通 ⇒ 立刻知道是「機器沒開」不是「VPN 沒接」。
📌 per user 提示：**也可以登入 Vigor2915ac 查 DHCP 表**確認誰在線上（本次靠 `hostname` 直接確認，未動用）。

### 🔴 `mh300_migration_plan.md` 有兩個 Phase 其實早就做完了

今天讀 `status` 輸出時發現 wire token 已經是 `dev_vfd_left`／`vfd_left=`，而計畫書
Phase 4 還寫著「要把 `se3_*` 改名成 `vfd_*`」。回頭查原始碼證實：

- **Phase 2（型別 swap）已完成**：`main.cpp:127` 有 `#define CRANE_VFD_IS_SE3 1`
  ＋ `using CraneVFD = SE3_inverter/MH300_inverter` 的 `#if` 切換，
  全檔用的是 `CraneVFD&` 不是 `SE3_inverter&`，連 fault 輪詢差異都已抽成
  `vfd_poll_fault_()`／`vfd_status_is_fault_()` 兩個 `#if` 分支。**換硬體只要翻那一個巨集。**
- **Phase 4（識別碼改名）已完成**：全檔 `grep` **找不到任何 `se3_left=` / `dev_se3_*` wire token**。
- **Phase 1（build 清單）已完成**：兩支 driver 都在 `scripts/build/build_crane.sh` 裡。

🔴 **真正還沒做的是 Phase 3（邏輯差異）**，而它正是計畫書標「重點，非機械替換」那一段：
`stopDecel()` 目前仍被當成「清 MRS」用（`main.cpp:3164`、`4191` 的註解都明寫 `clear MRS`），
而 MH300 的 B.B 在 `0x2002`、run 在 `0x2000`，**`stopDecel` 清不掉 B.B** ⇒ 急停後馬達被 base-block 卡死。
⇒ **這個遷移的剩餘工作量比計畫書看起來小得多，但剩下的那一塊是唯一會咬人的那塊。**
📌 **又一次同型**：文件描述的待辦狀態比實際落後，而落後的方向是「把已完成的說成待做」，
於是真正的風險（Phase 3）被埋在三個假待辦中間。**計畫書待更新。**

### 本體（18:09 開機後補起）

per user「本體開機了」。啟動前**先驗編譯期 `CRANE_IP`** 再開跑（這台的 IP 漂掉只能改碼重編，
且對不存在的主機是 blocking connect ⇒ 會卡滿 SYN timeout 約 2 分鐘、看起來像當機）：
`strings ~/bringup/facade_cleaning_v2.out` 得 `192.168.5.25` ✅，且本體端 `/dev/tcp/192.168.5.25/5002` 通。

| 項目 | 結果 |
|---|---|
| 本體 `facade_cleaning_v2.out` | ✅ `command server :5001`；`.20`/`.21`/`.22` 三條匯流排全 connected |
| 對吊機 | ✅ `[OK] crane 192.168.5.25:5002`（**是 OK 不是 WARN**，runbook 驗收條件）；有線 `192.168.1.10` 300ms 探測不通 → 走 WiFi，符合預期 |
| 兩個 WEB | ✅ 8080／8081 都出現 `[washrobot] connected 192.168.5.26:5001` |
| 手臂 `motor_api` | ❌ **未啟動**（見下） |

🎯 **init log 與 09-07（`wr_0907b.log`）逐字比對：只差 IMU 那一行**
（`roll=0.884399/pitch=-0.285645` → `roll=0.900879/pitch=-0.263672`），
其餘 24 行完全相同（連檔案大小都同為 1,957 bytes）⇒ **沒有新故障**。

### 🎉 09-07「部署後第一件事」完整結案（兩段都驗到了）

09-07 交代的驗收是兩段式的，今天兩段都過：

1. **開機宣告** —— 吊機 log 逐字出現 `[INFO] balance_source=imu (編譯預設…**不持久化**)`（本體未開時）
2. **roll 到達後轉活** —— 本體起來後推 `set_imu_roll`（250ms），吊機 `status` 實測
   `imu_roll=0.91`／`imu_roll_age_ms=175`／**`imu_roll_fresh=1`**

⇒ `balance_source=imu` 這條路徑**第一次被證明「冷開機後自動落在 imu、而且真的接得到資料」**。
📌 本體 init 自報 `roll=0.900879`、吊機收到 `0.91` ⇒ 兩端數值一致，推送鏈路正確。

### ✅ 安全基線（未動任何硬體）

本體 `relay_status` 一次讀回 **`ch1..ch16` 全 0**
（`ch1=valve`／`ch2=pumpA`／`ch3=pumpB(未啟用)`／`ch5=brush`／`ch6=正壓閥`／`ch14=water_pump`）。
吊機 `motion_active=0`、張力 `L=3.19 / R=3.19 kg`（總 6.4kg ⇒ **沒吊重**）、繩長 `L=58 / R=56`。
**本次全程只送唯讀指令（`status`／`relay_status`），沒有下過任何動作指令。**

### 🔴 手臂 `motor_api` 刻意沒有啟動 —— 等 per user 確認

- **USB-CAN 實體在**：kernel log `[4.193009] cdc_acm 3-2:1.0: ttyACM0` ＝**本次開機 4.2 秒就註冊好**。
  📌 沿用 09-07 的教訓：`/dev/ttyACM0` 節點 mtime 顯示 `Sep 7 22:19`（上次被開啟的時間），
  **拿它推論「裝置何時出現」會得到錯的答案，要問 kernel log。**
- 🔴 **不自行啟動的理由**：`motor_api` 自 09-03 起有「開機自動就緒」（commit `c5c0ce0`），
  而待辦表明載 **「沒有任何機制偵測 M2 被重新安裝過；重裝後若位置落在 ±1.5 rad 內，INIT 會**靜默**移到錯的 CENTER」**
  ⇒ **啟動它可能讓手臂實際動作**。機器剛維修完，手臂是否已裝回、周邊是否淨空，只有現場知道。
  **要跑請 per user 明示，並且人在旁邊看著。**

### 🎯 `192.168.1.x` 內網**現在真的串起來了** —— 但「串起來」不等於「該用它」

per user「試試他內部 192.168.1 的網路是不是相通」。**結論：相通，09-03 那條記載已被推翻**
（原文：「兩台 eth0 雖然 carrier=1 且各持 `.1.10`/`.1.100`，但彼此 ping 不通 ⇒ 有線沒有真的串起來」）。
今日雙向 ping **各 20 次、0% 丟包**，ARP 互相解析得到對方 MAC。

🔴 **但延遲分佈說明「中間那一跳不是有線」**（同一時段、同樣 20 次 `-i0.3`）：

| 路徑 | min / avg / max | mdev | 判讀 |
|---|---|---|---|
| 本體 → **自己**的閘道 `.20` | 0.65 / **0.71** / 1.06 ms | 0.08 | ✅ 真有線 |
| 吊機 → **自己**的閘道 `.33` | 0.09 / **0.11** / 0.22 ms | 0.02 | ✅ 真有線 |
| 本體 → 吊機 `192.168.1.10`（跨段） | 4.5 / **114** / 349 ms | 124 | ⚠️ |
| 吊機 → 本體 `192.168.1.100`（跨段） | 6.6 / **186** / 389 ms | 152 | ⚠️ |
| 吊機 → 本體的閘道 `.20`（跨段） | 5.9 / **167** / 583 ms | 169 | ⚠️ |
| 吊機 ↔ 本體 **WiFi** `192.168.5.x` | 2.5 / **7.2** / 10.0 ms | 2.4 | ✅ 穩定 |

📌 **兩段各自的本地都是乾淨的有線（0.1~0.7ms）；慢的只有跨段那一跳**（avg 114~186ms、峰值 583ms、
mdev 與 avg 同量級、**但 0% 丟包**）。0% 丟包＋巨大抖動＝不是壅塞也不是壞線材
（吊機 eth0 協商 1000Mbps、RX/TX errors 與 dropped **全 0**），特徵指向**中間是無線橋接**。

🔴🔴 **per user 揭曉中間那一跳是什麼：200 公尺的 Blue Robotics「Fathom-X Tether Interface Board」**
（規格已查證：**HomePlug AV / IEEE-1901**，把乙太網跑在單對雙絞線上，實測 80 Mbps、
實體層 200 Mbps、2–30 MHz、可達 300m+）。
⇒ **我原本猜「無線橋接」，機制猜錯了；但「共享媒介 + MAC 層重傳」這個特徵猜對了** ——
PLC 的 MAC 會自己重傳，所以**IP 層看起來 0% 丟包，代價全部轉成延遲尾巴**。

📌 **這個資訊改變了問題本身。** 這條隧道不是「不小心接歪的慢線路」，
而是**機器吊在牆上時唯一的實體連線**（200m 就是為了這個）。
所以問題不是「有線還是 WiFi 比較快」，而是「**控制迴路能不能忍受這條隧道的延遲分佈**」。

### 🔴🔴 量到了：隧道會讓 `imu_roll_fresh` 規律性掉出來

先量應用層（不是 ICMP —— 本專案的教訓就是這兩者不是同一件事），
本體 → 吊機 `:5002` 同一條連線連送 40 次 `status`：

| 路徑 | min | p50 | **p90** | max | avg |
|---|---|---|---|---|---|
| 隧道（Fathom-X 有線） | 6 | 7 | **365** | **744** | 77.6 ms |
| WiFi | 5 | 6 | 6 | 9 | 6.1 ms |

**中位數一樣好（7ms vs 6ms），差別全在尾巴。** 於是直接量「最長多久收不到東西」，
兩條路徑同時、各 120 秒、每 250ms 一次：

| 路徑 | 收到/送出 | 丟包 | **最長空窗** | **>750ms 的空窗次數** |
|---|---|---|---|---|
| 隧道 | 465 / 470 | **1.06%** | **1,198 ms** | **22 次** |
| WiFi | 479 / 479 | 0% | 271 ms | **0 次** |

🔴 **對照吊機的實際門檻**（`main.cpp:884`）：`IMU_ROLL_STALE_MS = 750`（註解寫 `3 × BALANCE_TICK_MS`），
本體的 roll 推送週期 250ms。
⇒ **在隧道上，750ms 這個新鮮度預算 120 秒內會被打破 22 次，約每 5.5 秒一次。**
⚠️ 推送走 TCP，隧道停頓時是**排隊延遲**不是丟失（head-of-line），
所以吊機那端會直接看到 `imu_roll_age_ms > 750` → `imu_roll_fresh=0`。

**後果**（`main.cpp:1581` 起）：`use_imu = want_imu && imu_roll_fresh()` 轉 false →
平衡**在運動中途**從 IMU 控制律掉回計米器控制律（kp / deadband / 誤差式全部換一組）。
✅ **這不是完全靜默** —— else 分支有 `[BAL] ⚠ balance_source=imu 但 roll 資料已過期` 的警告，
但**限流 2 秒一次**，而抖動約 5.5 秒一次 ⇒ 大部分會印出來。
📌 也就是說：**09-07 把編譯預設改成 `imu` 是對的，但那個改動的價值只在 WiFi 上成立；
一旦走隧道，它會規律地退化成 `meter`。**

### 📌 per user 拍板（09-08）：**最終還是走有線**，現階段先用 WiFi 測功能

⇒ 上面那條「上牆後 WiFi 還在不在」**已經有答案了：不會靠 WiFi**。
於是 `IMU_ROLL_STALE_MS=750` 的問題**從「可能要處理」變成「確定要處理」** ——
它不是風險評估，是一個**已知會發生**的缺陷，只是還沒輪到它。

🔴 **收進待辦，並記下它為什麼現在看不出來**：現在所有測試都在 WiFi 上跑
（0 次超過 750ms 的空窗），**IMU 平衡會一路正常**；等切到隧道才會開始每 5.5 秒掉一次。
📌 **這正是「在不具代表性的環境上驗收」的典型** —— 通過的測試不會告訴你它只在 bench 成立。

🟢 **當前結論（bench）：`CRANE_IP` 維持 WiFi `192.168.5.25`，不要改成 `192.168.1.10`。**
但**理由要換掉** —— 不是「有線比較爛」，而是「bench 上兩台在同一個房間，WiFi 是更好的那條，
而隧道現在沒有任何流量走它」。

> ⏩ **本節（09-08 中午前）的兩個判斷在同日稍晚被自己的受控實驗推翻，往下讀再行動：**
> ① 上面這句「不是有線比較爛」**不成立** —— 閒置時確實只是延遲尾巴的程度問題，但
> **VFD 一運轉隧道掉 90% 封包**，那是負載相關的失效，性質不同（見 #2 結案）。
> ② 下面「每 5.5 秒破一次」**已收回** —— 量細之後空窗間隔散佈在 0.6~10.6 秒，不是週期性。
> ③ 因此下面把 `IMU_ROLL_STALE_MS` 當成「上牆前必須先決定的參數題」的排序也**已作廢**：
> 第一順位是干擾本身（電氣），門檻排在它後面。

🔴🔴 **真正的待辦是部署前的那一題，而它現在有數字了**：
機器實際吊上牆之後，WiFi 還在不在？如果不在（隧道存在的理由就是這個），
控制流量就**必須**走隧道，而屆時：
- `IMU_ROLL_STALE_MS=750` 每 5.5 秒破一次 ⇒ IMU 平衡形同無效
- 吊機 watchdog `idle/hold 2000ms` 對上 1,198ms 的空窗，餘裕只有 1.7×
⇒ **這不是「以後再說」，是上牆前必須先決定的參數題**（放寬門檻？改推送頻率？
把 roll 的時效判斷改成隧道感知？）。**在有答案之前，不要假設 09-07 的 `imu` 預設在牆上還成立。**

### ⚠️ 我自己踩的一個坑：TCP 探測全 FAIL 不是網路壞

用 `/dev/tcp/192.168.1.33/502` 連 15 次**全部 FAIL**，看起來像閘道掛了。
實際上 **USR 閘道是單客戶端**，該埠已被吊機主程式佔用
（`ss -tnp` 實證：`192.168.1.10:40576 → 192.168.1.33:502  users:(("crane_control_P",pid=1698))`）。
📌 **同時段 ICMP 對同一台是 0.11ms** ⇒ 網路好得很。
**在跑著的系統上探測獨佔資源，「連不上」是預期行為不是故障** —— 要先問「這條連線現在歸誰」。

### ✅ 真實流量是健康的（這才是決策依據，不是 ping）

- `status` 往返（含 Modbus 快取）連測 10 次：**9~10 ms，完全穩定**
- **VFD keepalive 42 個 30 秒窗，只有 1 個有失敗**（`R: ok=47 fail=3 clears=1`），
  其餘 41 窗 `L/R 各 ok=50 fail=0` ⇒ 失敗率約 0.14%，重試層有接住
- 零星 `[ERR] [SE3:1@L/R] readParam reg=0x1001 comm fail`（17:46／17:54／17:56 各數筆）
  —— **都發生在本體啟動（18:09）之前**，且走的是吊機自己的本地閘道（0.11ms）
  ⇒ **與今天的網段合併無關**，是既有的間歇性 Modbus 失敗。

### 🖥 per user 用 8080 GUI 做單項操作（19:00~19:11，手臂未開）

**吊機零動作指令** —— 新增的 10,050 行裡 9,997 行是 `set_imu_roll`、53 行 keepalive，
**一筆 motion / hold / manual 都沒有**。全部操作都在本體。

| 類別 | 次數 | 內容 |
|---|---|---|
| `pwm_set`（風扇） | 22 | ch1 hz=50，duty 在 5 與 6/7/8/9/10 之間逐檔來回 |
| `relay` | 8 | CH1(valve)／CH3(pumpB)／CH4／CH6(正壓閥) 各 ON+OFF |
| `pusher extend_raw` | 2 | 10cm（30,000 pulse）4 顆同時 |
| ZDT 吸盤 5~8 | 16 | 伸出 `pos≈3000°` ／ 收回 `pos≈29.9°`，兩輪來回 |
| `vacuum_release` / `2stage_retract` | 2 / 2 | 含 CH6 破真空充氣 |

✅ **收尾乾淨**：`relay_status` 回到 `ch1..ch16` 全 0，與動手前基線逐欄相同。
✅ **22 次 `pwm_set` 全部「回讀確認 + 三個寫入都成功」**，零失敗
—— QX-DO24 的 PWM 回讀路徑（`[2026-09-02c]` 加的）**第一次被連續操作 22 次驗證**。

### 🎯 四顆推桿空載一致性 1.03× —— 這是 09-02 那條待辦的對照組

09-02 記著「**四顆推桿接觸力道不均：10cm 時 peakI 589~1264 mA（2.1 倍）＝機身與玻璃不平行**」，
另一條記著「**右腳推桿阻力明顯高於左腳**（右為左的 1.4~1.6 倍時間、高約 20% 電流）」。

今天同樣是 `extend_raw 10cm`，但**機器是空載自由懸掛**（張力 3.19/3.19 kg 全程沒吊重、
log 裡沒有任何 `cmd_attach`）⇒ **推桿是伸進空氣、不是壓玻璃**。結果：

| 輪次 | 右上 ZDT:5 | 右下 ZDT:7 | 左上 ZDT:6 | 左下 ZDT:8 |
|---|---|---|---|---|
| 伸出① | 1500ms / **649**mA | 1650ms / **644**mA | 1800ms / **652**mA | 1800ms / **663**mA |
| 伸出② | 1350ms / **648**mA | 1350ms / **641**mA | 1350ms / **651**mA | 1650ms / **655**mA |
| 收回 | 1200~1500ms / 557mA | 1350~1650ms / 526mA | 1200~1650ms / 544~548mA | 1200~1650ms / 552~553mA |

🎯 **四顆峰值電流 641~663 mA ＝ 1.03 倍，幾乎完全一致**（09-02 接觸時是 2.1 倍）。
🎯 **而且左右反了**：空載時右側平均 1,462ms / 644~649mA，左側 1,650ms / 651~663mA，
**左邊反而略慢略吃電** —— 與 09-02「右為左的 1.4~1.6 倍」方向相反。

📌 **這是對照組，不是「問題解決了」。** 工況不同（空載 vs 壓玻璃），不能直接比大小。
但它**排除掉一整類解釋**：如果 09-02 的不均來自馬達個體差異、推桿機構、或右側機械阻力，
**空載時就該看得到**，而空載時四顆一致且左右反轉。
⇒ **支持 09-02 已有的假說：差異來自接觸幾何（機身與玻璃不平行 / 右側離牆較近）**，
而不是致動端。🔴 **待辦「用尺量四個吸盤到牆面的間距」的價值因此更高了**——
現在它是唯一還沒被排除的解釋。

### 🔴 JC-100 真空壓力錶通訊錯誤 25 筆，而且不只是「沒回應」

| 通道 | TIMEOUT | CRC | FUNC_MISMATCH | ADDR_MISMATCH | MODBUS_EXCEPTION |
|---|---|---|---|---|---|
| JC100:7 | 19 | 1 | 1 | — | — |
| JC100:8 | 1 | 1 | 1 | 1 | 1 |

`JC100:7` 於 19:09:58 觸發 `TIMEOUT ×3 — 進入 fast-fail (recv 100ms)，log 節流至每 10 次`。
收尾後讀 `status` 仍得 `p5=-1 p6=0 p7=0 p8=0`，且**最近 2 分鐘仍有 1 筆 JC100 錯誤** ⇒ **持續中**。

🔴 **重點在錯誤的「種類」而不是次數**：`FUNC_MISMATCH (7/3) head=08 07 02 00`、
`ADDR_MISMATCH (104/8) head=68 02 00 00` —— 這些是**讀到錯位的位元組**，
不是沒收到。待辦表那條 **「USR 網關 `_pt`（串口打包時間）＝0 ⇒ 115200 下字元間隔僅 0.3ms，
是回覆被切成兩個 TCP 段的結構性根源」** 當時註明「**目前量到的失敗是 `no reply` 不是 `too short`**」
—— **今天這句話不成立了**：切段/錯位族的錯誤（CRC / FUNC_MISMATCH / ADDR_MISMATCH）**各自都出現了**。
⇒ **那條待辦的證據基礎變了，應重新評估把 `_pt` 改成 5ms**（代價：bus 上每筆交易 ≤5ms）。

### 🔴 吊機 log 被 `set_imu_roll` 洗版：訊噪比 189:1

40 分鐘產生 **9,997 行 `[dispatch] cmd='set_imu_roll …'`**（4Hz 逐筆記錄）對上 **53 行** keepalive，
檔案 **681 KB**（09-03 那份是 6 MB，同一個原因）。
**我今天要看吊機做了什麼，必須先 `grep -v` 掉它才看得見** ⇒ 真正的事件被埋掉。
🟡 建議：`set_imu_roll` 的 dispatch log 改為節流（例如每 N 筆或只在值變化超過門檻時記），
或降到 debug 層級。**不急，但每次上機都在累積成本。**

### 🔍 「按繼電器有延遲感」查因：不是 bus，是前端寫死的 700ms

per user 提出假設：**「按按鈕有延遲，是不是因為 BUS 一直在問狀態？要不要按下去時先停止問狀態，指令完成再繼續？」**

🔴 **假設不成立，實測推翻**（本體 :5001，同一條連線各 20 次）：

| 指令 | p50 | p90 | max |
|---|---|---|---|
| `status` | **1 ms** | 1 ms | 123 ms |
| `relay_status` | 9 ms | 10 ms | 10 ms |
| `get_settings` | 2 ms | 2 ms | 2 ms |

三個理由說明 bus 不是瓶頸：
1. **JC-100 的 fresh-read 早就被限流到 ≤1Hz** —— `wash_robot_commands.cpp:3697` 註解自己寫著
   「GUI polls status at 2Hz (500ms) and each fresh-read hits `cli_22_` **9 times** → bus saturation」
   ⇒ **這個問題 2026-06-02 就被發現並解決過了**，`status` 20 次裡 19 次直接吃快取（1ms）
2. **繼電器在 `.20`（PQW）、JC-100 在 `.22`** —— 不同閘道、不同 TCP 連線，本來就不搶同一條 bus
3. 開機 log 的 `[OK] pressure poll DISABLED (cmd_status fresh-reads on demand)` 也是同一件事的證據

✅ **真因在瀏覽器端**：`public_v2/index.html` 的 `fire()`（約 1767 行）結尾——
```js
// 一律以回讀為準，不相信回傳字串
setTimeout(function(){ pollRelay(); pollPwm(); }, 700);
```
指令往返 ~30ms 就結束，但按鈕要等這 **固定 700ms** 重讀完才翻。**感覺到的延遲就是它。**

📌 **而且這 700ms 是多餘的保守**：後端回覆**本身就是回讀結果** ——
`cmd_relay` 走 `pqw_set_relay_verified_()`（寫入＋回讀＋最多重試 3 次），
回的 `OK ch<N>=<0|1>` 那個數字是從 PQW 讀回來的（`wash_robot_commands.cpp:3914`）。
⇒ 可以用回覆裡的**回讀值**立刻更新，700ms 的輪詢降為二次確認 —— **不違背「不相信回傳字串」的原意**。
⚠️ 例外：`OK pump_set_but_readback_fail`（3841）／`water_pump_set_but_readback_fail`（3928）
**開頭是 OK 但沒有 ch 值**，必須當成未確認。

🔴 **順帶否決 per user 的提案本身**：「按按鈕時停止 bus 問狀態」**治不到這個延遲**（它在瀏覽器端），
而且會**在正好有東西在動作的那幾秒拿掉狀態可視性**。已向使用者說明，不做。
📌 **記在決策紀錄的理由**：這個提案很合理、日後可能被再次提出，要留下「量過、不是這個原因」。

### 📌 per user 拍板：繼電器操作的 `confirm()` 彈窗全部拿掉，按了就執行

兩處：`#relaygrp` 的 `row.dataset.danger`（CH1 真空閥／CH6 正壓閥，原文
「吸盤承重時操作它可能讓機器脫落」）、`[data-cmd]` 的通用「這會移動機器或中止動作」。
⚠️ **這是使用者明確要求移除，不是疏漏** —— 我提過替代方案
「改成只在真的吸附/承重時才跳（bench 上完全不擋）」，使用者要全拿掉。**日後不要當成 bug 修回去。**

### 🤝 GUI 線交給 `AI-2`（per user：「有些 GUI 或小工作可以丟給他，等於你的助理」）

沿用 09-04 的分工：**`web_backend/` 歸 GUI 線；C++／`scripts/`／機上測試歸我這條**。
已把上面兩件（拿掉彈窗 + 消除 700ms 延遲）連同實測數據、後端回讀事實、
邊界（不准碰 8080 的 `public/`、不准動 C++、不准送會讓機器動的指令）整包送給 `AI-2`。

### 📋 評估：兩邊通訊改走 `192.168.1`（隧道）需要改程式嗎？（per user，WiFi 保留）

**結論：切換本身不需要改任何程式碼，但有三件事必須先處理，其中一件是硬阻擋。**

#### ✅ 不需要改的 —— 機制早就在了

1. **`common/endpoints.h` 的 `ep::host()` 環境變數覆蓋**（2026-08-29 為重構等價測試而加）：
   本體啟動時加 `FCV_EP_CRANE_HOST=192.168.1.10` 即可。
   `resolve_crane_ip_()` 開頭就檢查覆蓋，**有覆蓋就直接用、完全跳過探測**（`WASH_ROBOT.cpp:690`）。
   📌 **這推翻了 runbook 與待辦表的一條記載**：「`CRANE_IP` 沒有任何 env 覆蓋，IP 一漂就只能改碼重編」
   —— 那句話在 08-29 加入 `endpoints.h` 之後就不成立了，但沒人回頭更新。
2. **`web_backend/server.js`** 的 `CRANE_IP` / `WROBOT_IP` 本來就吃環境變數，零改動。
3. **WiFi 保留**：不設環境變數就回到現行的「有線優先探測 → 退 WiFi」。
   `endpoints.h` 的設計規則明寫「沒設環境變數時行為必須**位元等價**」，所以保留 WiFi 沒有代價。

#### 🔴 必須先修的三件

**① `IMU_ROLL_STALE_MS = 750`（吊機 `main.cpp:884`）—— 唯一的硬阻擋**
實測隧道 120 秒內破 22 次（約每 5.5 秒一次）⇒ 切過去之後 IMU 平衡會規律失效、退回計米器路徑。
📌 那個值的註解寫 `3 × BALANCE_TICK_MS`，是照 250ms tick 推導的 —— **推導前提裡沒有隧道**。

**② 選路是「只在 init 呼叫一次」（`WASH_ROBOT.cpp:681` 註解自陳）**
🔴 **今天就有一個實例**：本體 18:07:57 開機、eth0 **12.4 秒**就 `Link is Up - 1Gbps`、
程式探測發生在 **18:09:46**（eth0 已就緒 96 秒），**探測仍然失敗** → 整個 run 鎖在 WiFi。
而**現在**用同樣的 300ms 門檻對同一位址測 30 次是 **30/30 全過**（放寬到 1000ms 也是 30/30）。
⇒ **失敗原因未查明**（HomePlug 關聯延遲？瞬時？），但**架構問題可以確定**：
**一次探測失敗 = 整個 run 沒有第二次機會，而且沒有任何徵兆**——log 只印一行就過去了。
- 若走環境變數強制，這條不影響（覆蓋時根本不探測）
- 若要靠自動選路，**必須加重試階梯**（例如開機後 N 秒內每 M 秒重試）

**③ 沒有執行期 failover**
`crane_connect_if_needed_()` 斷線重連用的是同一個 `crane_ip_resolved_`，**不會改走另一條**。
⇒ 要決定：隧道中途掛掉時要不要自動退回 WiFi。
📌 **對吊在牆上的機器這是安全題不是便利題** —— 但自動切換本身也有風險
（來源不確定、行為不可預測），兩個方向都要拍板，不能默認。

#### 🟡 要盯、但不擋切換

- **watchdog 餘裕只有 1.7×**：`WATCHDOG_TIMEOUT_MS_IDLE = 2000` 對上實測最長空窗 **1,198ms**。
  🔴 逾時動作是 `hold_all_off()` + `allMotionOff()`
  ⇒ **隧道打一個嗝就會直接停機**。這是切過去之後最可能先撞到的東西。
- **三條連線共用一條實體隧道**：`crane_cli_`（主）／`crane_cli_estop_`（急停旁路）／
  `crane_cli_imu_`（roll 推送，`WASH_ROBOT.h:1572`）是三條獨立 TCP，
  應用層不會互相 head-of-line —— **但 PLC 停頓會同時打到三條，包括急停旁路**。
- 隧道 1.06% 丟包（4Hz 持續量測）。

#### 📌 建議順序（風險由低到高）

1. **先修 ①**（一行常數，依據已經量到）
2. **用 `FCV_EP_CRANE_HOST=192.168.1.10` 跑一輪** —— 零程式風險、拿掉就回 WiFi
3. **觀察 watchdog 有沒有被觸發**（這是 ② ③ 要不要做的判準來源）
4. 再決定 ② ③

### 🔧 `AI-2` 回報（GUI 線）＋ 對我 MH300 判斷的更正

**GUI 兩件已完成並部署 `:8081`，版號 `2026.09.08-1925`**（check_console 四項過、md5 逐位元一致、
`:8080` 未受影響、未重啟 node、全程未送會讓機器動作的指令、Pi 上備份
`index.html.bak-20260908-1925`、changelog `[2026-09-08g1]`、未 commit）。
🐛 他踩到一個我移交單沒寫的坑：**不能用寬鬆的 `/ch(\d+)=([01])\b/` 掃回覆** ——
`pwm status` 回 `ch1=5,50,65535,1`，duty 為 0/1 時 `\b` 在逗號前成立 → 會誤翻繼電器 CH1。
已窄化成「只認 `chKey` 指名的那一個通道」。

🔴 **待 per user 決定的副作用**：通用 `[data-cmd]` handler 是共用的，confirm 拿掉之後
**三個非繼電器按鈕一併失去確認**：`pwm save`（寫 QX-DO24 flash，**壽命僅 1~2 千次**、不可回復）、
`zdt_zero`（重定推桿原點）、`rail_cfg_soft_enable`；
且產生一處不一致 —— 隔壁的 `rail-zero` 有自己 hardcoded 的 confirm，**它還是會跳**。
📌 使用者原話範圍是「操作繼電器…這類的訊息都拿掉」，這三個是被共用 handler 順帶掃到的。

#### 🔴 更正我今日對 MH300 Phase 3 的描述（`AI-2` 提出，我已逐處驗證）

我今天寫「`stopDecel` 清不掉 B.B ⇒ 急停後馬達被 base-block 卡死」——**在正常路徑上不成立**。
driver 早就補過了：`emergencyStop()` 設 `base_blocked_`，run 命令進來先
`releaseBaseBlockIfNeeded_()` 寫 `0x2002=0`（`MH300_inverter.cpp:233/240/244`）。
計畫書列的六個 callsite 行號也全是重構前的。

🔴 **但洞還在，而且比我描述的更隱蔽**（三處已逐一驗證）：
`base_blocked_` 是 **process-local 軟體旗標**（`MH300_inverter.h:205`，初值 `false`），
而 `releaseBaseBlockIfNeeded_()` 開頭就是 `if (!base_blocked_) return;`（`.cpp:234`）。
⇒ **急停 → 程式重啟（`exit`／crash／Pi 重開機）→ run**：
硬體 `0x2002` 仍 set、旗標卻歸零 ⇒ 直接 return，**B.B 永遠不清**。
**症狀：馬達不轉、Modbus 寫入全部回報成功、無 fault code、一行錯誤訊息都沒有。**
`init()` 補不了 —— 它只在**讀到 error code 才** `clearAlarm()`（已確認），
而 base-block 是輔助命令位元、不是 alarm。
✅ **修法一行等級**：`init()` 的 Mode B probe 成功後**無條件** `writeParam(REG_AUX_CMD, 0x0000)`。
📌 **這是 C++、歸機器線，`AI-2` 沒有動。** 目前 `CRANE_VFD_IS_SE3 = 1`（跑 SE3）⇒ **尚未生效，但換 MH300 當天就會踩到**。

### 🧪 實地切到隧道跑了一輪（19:30~19:45，per user「依照建議 處理」）

#### ✅ 切換本身：零程式改動，一次成功

```
[endpoints] override CRANE host = 192.168.1.10
[crane] 位址由環境變數覆蓋 = 192.168.1.10（不做有線探測）
[OK] crane 192.168.1.10:5002
```
`ss` 確認控制連線實走 `192.168.1.100 → 192.168.1.10:5002`。**評估的第一段完全成立。**

#### 🔴🔴 但先撈到一個比原評估更嚴重的既有 bug（切換前就存在）

`resolve_crane_ip_()` 決定的 `crane_ip_resolved_` **只套用到主連線**。
另外兩條**直接讀 `ep::host("CRANE", CRANE_IP)`**：
- `crane_cli_imu_`（roll 推送）—— `WASH_ROBOT.cpp:2929`
- `crane_cli_estop_`（**急停旁路**）—— `WASH_ROBOT.cpp:2517`

⇒ **若自動探測選了有線，主連線走有線、roll 推送與急停旁路仍走 WiFi**，三條分裂且無任何徵兆。
✅ 今天沒炸是因為探測失敗、三條都落在 WiFi（**巧合，不是設計**）。
📌 **反過來說，環境變數強制是目前唯一能讓三條一致的方式**（三條讀同一個 `ep::host`）。
🔴 待修：`crane_cli_imu_` / `crane_cli_estop_` 應改讀 `crane_ip_resolved_`。

#### 📊 隧道上的實測（180 秒，每秒取樣 `status`）

| 指標 | WiFi | 隧道 |
|---|---|---|
| `imu_roll_age_ms`（idle） | **175 ms** | **495 ms** |
| 180 次取樣中 `fresh=0` | — | **7 次（3.9%）** |
| **最大 age** | — | **2,867 ms** |

#### 🎯 per user 19:36:25 按 UP hold —— 兩個 tick 就演出來了

```
[BAL] src=imu err=0.88deg trim=5.28Hz L=27.36 R=32.64 (roll_age=668ms)
[BAL] ⚠ roll 資料已過期（age=940ms > 750ms）→ 本輪退回計米器
[BAL] src=meter err=2cm trim=2Hz L=29 R=31
```
**第一個 tick 就已經 668ms（貼著 750 的邊），第二個直接破。**
🔴 **而且控制輸出真的變了**：trim `5.28Hz → 2Hz`、左右分配 `27.36/32.64 → 29/31`。
⇒ **不是換個標籤，是換一組馬達命令。**

#### 🔴🔴 原評估的「watchdog 餘裕 1.7×」太樂觀，而且我的量測把它蓋住了

- 實測最大 age **2,867ms** > `WATCHDOG_TIMEOUT_MS_IDLE = 2000`
  ⇒ 我先前用 ping 空窗（1,198ms）算的 1.7× 餘裕**不成立**，實際已經超過門檻。
- ⚠️ **但 watchdog 今天一次都沒觸發，原因是我自己**：量測腳本每秒送一次 `status`，
  而 watchdog 看的是「最後一次收到指令」的心跳 —— **我的取樣一直在幫它續命**。
  📌 **量測行為改變了被量測的對象**，這條的風險**至今未經驗證**，不能當成「試過沒事」。

#### 📌 結論修正：① 一行常數**不足以**解決

原本我建議「先修 `IMU_ROLL_STALE_MS`」。實測之後：
- 提高門檻是**可以有依據的**：roll 變化極慢（idle 約 0.01°/s；09-01 pay_out 期間
  3.5° / 約 20s ≈ **0.17°/s**），而 `balance_imu_deadband = 0.5°`
  ⇒ 1,500ms 的陳舊只帶來約 0.26° 誤差，仍在死區內。**但**
- **實測最大 2,867ms 連 1,500ms 都會破**，而且 watchdog（2,000ms）也在射程內
⇒ **門檻不是主要矛盾，隧道的延遲尾巴才是。** 只調門檻＝把一個看得見的退化換成一個看不見的。

### ✅ #1 完成：連往吊機的位址收斂成單一來源（本體 C++，已建置部署驗證）

**問題**：`resolve_crane_ip_()` 的決定只套用到主連線；另外的連線各自直接讀
`ep::host("CRANE", CRANE_IP)`（＝編譯期 **WiFi** 常數）。

🔴 **實際有 5 處，不是我原先說的 2 處**（第 5 處是逐字檢查殘留時才撈到的，也是最要緊的一處）：
...
**改法**：抽出 `crane_endpoint_ip_()` 單一位址來源，5 處全部改走它。
📌 **刻意不是「把那兩處各補一份三元運算式」** —— 那會變成同一個運算式抄 5 份，
正是本專案一直在清的「同一件事寫在兩個地方」。宣告處寫明理由與 WiFi 拆除後的後果。
📌 執行緒安全：`crane_ip_resolved_` 只在 init 寫一次，IMU 執行緒與 estop 路徑都在 init 之後才啟動。

**驗證**（照 `scripts/build/README.md` 的三個隱藏步驟）：
- 同步後 `md5sum` 兩邊逐位元一致 `a02012e3…`
- `g++ -fsyntax-only` 過 → `build_body.sh` **16/16 objs**，產物 **1,086,792 bytes**
  （現役 1,086,728，**差 64 bytes ⇒ 確實是新的東西**，沒重演 09-07「大小完全相同」那個巧合）
- 停程式 → 備份 `facade_cleaning_v2.out.prev-20260908-1950` → 換上，md5 對上
...
### 🔬 #2 隧道尾巴診斷：不是週期性，而且**疑似與馬達活動相關**

300 秒 @5Hz（機器閒置）：**1492/1492，0% 丟包**；>500ms 空窗 **88 次**（約每 3.4 秒一次）。

**① 不是週期性** —— 空窗之間的間隔散佈在 **0.6s ~ 10.6s**（0.6/0.7/1.0/1.1/1.2/1.4/1.8/2.0/2.3/2.4/2.5/2.7/4.2/4.7/5.0/5.6/6.0/6.2/7.2/9.8/10.6）。
⇒ **排除「HomePlug beacon／通道適應」是唯一成因**（那會集中在固定週期附近）。
📌 我先前說「大約每 5.5 秒一次，那個規律性很可疑」—— **量細之後不成立，收回。**

**② 空窗大小集中在 500~680ms**，尾巴到 819/903ms。這個窄帶像某種**重傳/恢復計時器**，不像壅塞。

**③ 🎯 三次量測的條件差異指向馬達**：

| 時間 | 機器狀態 | 丟包 | 最大空窗／age |
|---|---|---|---|
| ~19:00–19:11 | per user 連續按鈕（繼電器/推桿/ZDT/PWM） | **1.06%** | 1,198 ms |
| ~19:36 | per user 按 **UP hold（VFD 實際驅動繩索）** | — | **2,867 ms** |
| ~19:45–19:50 | **閒置** | **0%** | 903 ms |

⇒ **假說：VFD／馬達活動會惡化隧道。** Fathom-X 是 HomePlug AV，工作頻帶 **2–30 MHz**，
而變頻器的切換雜訊正好落在這個帶內 —— 教科書等級的干擾情境。
⚠️ **這是 3 個樣本的相關性，不是因果。** 決定性測試：**一邊跑 VFD 一邊量隧道**（需 per user 授權動馬達）。

**④ 對門檻的意義**：
- **即使閒置**，最大空窗 903ms 就已經 > `IMU_ROLL_STALE_MS = 750` ⇒ 偶發退化擋不掉
- **馬達一動** 2,867ms，連放寬到 1,500ms 都破，且**超過 watchdog 的 2,000ms**
⇒ 🔴 **如果假說成立，這是電氣問題不是軟體問題**（鐵氧體磁環／隔離／隧道遠離動力線／接地），
**調門檻只是把它蓋住**。在做那個受控測試之前，不要先改門檻。

### 🎯🎯 #2 結案：**VFD 一運轉，隧道就掉 90% 封包** —— 是電氣問題，不是軟體

受控實驗（per user 執行，機器未貼牆、繩上 6.4kg ＝無吊重）：
**同時**量隧道 `192.168.1.10` 與 WiFi `192.168.5.25`，5Hz、420 秒，
中間 per user 按住 console v2 的 `▲ 拉繩`（`up on`/`up off`）一次。
hold 窗口由吊機 log 的 `HOLD-TRACE` 時間戳取得：**19:59:31.972 ~ 19:59:49.426（17.45 秒）**。
（兩台時鐘偏移實測 0.26 秒，含 SSH 往返，對 17 秒的窗口足夠。）

| 路徑 | HOLD 期間收到 / 預期 | 丟包 | 其餘時間 |
|---|---|---|---|
| **隧道** | **9 / 90** | **🔴 90%** | 1,898 筆、0% |
| **WiFi** | **90 / 90** | **0%** | 2,002 筆、0% |

⚠️ 分析輸出裡「其餘時間最長空窗 ~20s」是**統計假象**（把跨越 hold 窗口的斷點算成一個空窗），
兩條路徑都有，無意義，不要當結論。

**三個互相獨立的證據指向同一件事：**
1. **隧道 90% 丟包，WiFi 同一秒 0%** —— 同兩台機器、同時段 ⇒ **不是主機忙，是隧道專屬**
2. **roll age 單調惡化** `758 → 2,895 → 5,045 → 7,206 ms`（17 秒內）
   ⇒ 不是偶發尖峰，是**持續劣化**（像 HomePlug 的通道估計被打壞後一路重試）
3. **吊機自己對 VFD 的 Modbus 完全沒事**：同時段 `keepalive ok=50 fail=0`、`status=0x4d`（運轉中）
   ⇒ 走吊機本地閘道（0.11ms）的流量毫髮無傷

📌 **假說證實**：Fathom-X 是 HomePlug AV，工作頻帶 **2–30 MHz**，變頻器切換雜訊正好落在其中。

#### 🔴 結論與其後果

- **隧道目前不能當控制鏈路** —— 它正好在**運動期間**（唯一需要它的時候）掉 90%。
- 🔴 **絕對不要先調 `IMU_ROLL_STALE_MS`** —— 那會把一條在運動時 90% 死掉的鏈路
  用一個更寬的門檻蓋起來，蓋住的正好是機器吊在牆上時最需要看見的東西。
  **我原本排在第一位的 ①，現在確定是錯的順序。**
- 🔴 **WiFi 在干擾解決之前不能拆。**
- 整段 hold 期間平衡全程 `src=meter` ⇒ **IMU 路徑實質死亡**，不是偶爾掉一拍。

#### 🔴 watchdog 第二次沒觸發，而且原因是結構性的

roll 斷了 7.2 秒，遠超 `WATCHDOG_TIMEOUT_MS_IDLE = 2000`，watchdog 仍未響。
**原因不是我的量測（這次量測不送指令）**，而是 **GUI 的 status 輪詢經 localhost 一直在餵心跳**。
⇒ 🔴 **watchdog 的心跳不分連線來源**：roll 推送整整 7 秒沒進來，
它因為「別條連線有流量」就認為一切正常 —— **它守不到它該守的東西**。
這是獨立於隧道的既有缺陷，WiFi 上同樣成立（只是 WiFi 不會斷 7 秒所以看不到）。

#### 📌 下一步（電氣，不是軟體）

待查（我沒有現場資訊，全部需要 per user）：
1. **吊機端的 Fathom-X 板子實體位置** —— 是否貼著變頻器／動力線
2. **它的供電** —— 是否與變頻器控制電路共用電源（**共用電源是最強候選**：
   HomePlug 靠訊噪比工作，板子自己的電源軌被污染會直接殺掉 SNR）
3. **隧道線材走向** —— 是否與馬達動力線同束／平行走線
4. 便宜的決定性測試：**吊機端 Fathom-X 改用獨立電源（電池）再測一次**
   → 清乾淨＝電源耦合；沒改善＝輻射／線間耦合
5. 常規對策：兩端共模扼流圈／鐵氧體磁環、馬達線改用遮蔽線且 360° 接地、
   VFD 輸出加 dU/dt 或正弦濾波器、隧道遠離動力線

### 🔴 切回 WiFi 時撞到第二個 bug：**「強制走 WiFi」是唯一做不到的事**

per user「先切回 WIFI，改完電源再測」。設 `FCV_EP_CRANE_HOST=192.168.5.25` 之後：
```
[endpoints] override CRANE host = 192.168.5.25      ← 環境變數確實讀到了
[crane] 走**有線** 192.168.1.10:5002（探測通過）     ← 但還是走有線
```

**根因**：`resolve_crane_ip_()` 判斷「有沒有覆蓋」是**比對值**：
```cpp
const std::string overridden = ep::host("CRANE", CRANE_IP);
if (overridden != wifi) { ...用覆蓋... }
```
⇒ 覆蓋值**剛好等於**編譯期常數時，與「完全沒設」無法區分 → 掉進探測分支。
而 `CRANE_IP` 本身就是 WiFi 位址 ⇒ **「強制走 WiFi」正好是這個機制唯一表達不出來的意圖。**
🔴 **而且 log 照樣印 `[endpoints] override ... = 192.168.5.25`，看起來像生效了** ——
兩行 log 自相矛盾，但各自都不像錯的。

📌 **我先前看過這個分支並判斷成「無害的小瑕疵」，然後就被它咬了。**
教訓：**「值相同所以沒差」只在讀的那一側成立；在寫的那一側，它讓一個意圖無法表達。**

**修法**：`common/endpoints.h` 新增 `ep::has_host_override(name)`（直接看環境變數在不在），
`resolve_crane_ip_()` 改用它。兩個檔、各一處。

**驗證**：md5 兩邊一致 → `build_body.sh` **16/16**、產物 1,086,896 bytes →
備份 `facade_cleaning_v2.out.prev-20260908-2010` → 換上 md5 對上 → 重啟後：
```
[crane] 位址由環境變數覆蓋 = 192.168.5.25（不做有線探測）
[OK] crane 192.168.5.25:5002
```
`ss` 確認**兩條連線都在 `192.168.5.25:5002`**。
...
| `facade_cleaning_v2.out.prev-20260908-2010` | 只含第一個修正 |
| `facade_cleaning_v2.out.prev-20260908-1950` | 09-07 原版，兩個修正都沒有 |

🔴 **現行啟動需要帶 `FCV_EP_CRANE_HOST=192.168.5.25`** —— 不帶的話自動探測會選有線
（探測現在會成功），而有線在 VFD 運轉時掉 90% 封包。
⚠️ **這是一個「必須記得帶」的參數，也就是遲早會被忘記的那種。**
干擾修好之前，考慮把它寫進啟動腳本，或把編譯期預設暫時改成 WiFi。

### 待完成
- 🟢 ~~eth 串接後回頭改 `CRANE_IP`~~ → **09-08：bench 上維持 WiFi**（隧道現在沒流量走它）
- 🔴🔴 **隧道干擾（電氣，不是軟體）**：VFD 一運轉，隧道掉 **90%** 封包（hold 期間 9/90，
  同一秒 WiFi 90/90 全通），roll age 17 秒內 `758 → 7,206ms`，平衡全程退回 `src=meter`。
  下一步全部需要 per user 現場資訊：Fathom-X 板子實體位置／供電是否與 VFD 控制電路共用／
  線材是否與動力線同束；**便宜的決定性測試＝吊機端 Fathom-X 改用獨立電源（電池）再測一次**。
  📌 隧道 = Blue Robotics Fathom-X（HomePlug AV, **2–30 MHz**），VFD 切換雜訊正落在帶內。
- 🔴🔴 **`IMU_ROLL_STALE_MS=750` —— 但它排在干擾後面，不要先動**
  ⚠️ **本條在本日稍早被寫成「切有線前必須改」，那個排序當日就被自己的實驗推翻了**
  （見上方 #2 結案：「我原本排在第一位的 ①，現在確定是錯的順序」）。
  先調門檻＝把一條**在運動時 90% 死掉**的鏈路用更寬的門檻蓋起來，蓋住的正好是機器吊在牆上時最需要看見的東西。
  ⚠️ 另**收回**同日稍早的「每 5.5 秒破一次」——量細之後空窗間隔散佈在 0.6~10.6 秒，**不是週期性**。
  ⇒ 干擾處置完成、隧道能當控制鏈路之後，這條才輪得到（選項仍是：放寬門檻／提高推送頻率／隧道感知的時效判斷）。
  📌 **現在測不出來**——WiFi 上 120 秒 0 次超標，這個缺陷在 bench 上完全隱形。
- 🔴 **吊機 watchdog 的心跳不分連線來源（結構性，獨立於隧道）**：roll 斷了 7.2 秒、遠超
  `WATCHDOG_TIMEOUT_MS_IDLE = 2000`，watchdog 仍未響 —— 因為 GUI 的 status 輪詢經 localhost 一直在餵心跳。
  **它守不到它該守的東西。** WiFi 上同樣成立，只是 WiFi 不會斷 7 秒所以看不到。
- 🟡 隧道閒置時 1.06% 丟包（4Hz 持續量測）未查明是否可改善（線材盤繞／干擾／兩端板子設定）
- 🔴 **JC-100 通訊錯誤持續中**（09-08 單項操作期間 25 筆，含切段/錯位族）⇒ 重新評估 USR 網關 `_pt` 0→5ms
- 🟡 **量四個吸盤到牆面的間距**（09-02 待辦）—— 09-08 空載對照組排除了致動端解釋，這條的價值升高
- 🟡 吊機 `set_imu_roll` 的 dispatch log 4Hz 逐筆記錄，40 分鐘 9,997 行，把真實事件埋掉
- ✅ ~~`mh300_migration_plan.md` 的 Phase 1/2/4 標為已完成~~ → **同日 19:21 由（續）條目做完**
  （並且 Phase 3 的描述本身也被實測推翻、整段改寫）。**本列在寫下的當下就已經過期。**
- ✅ ~~runbook §「啟動順序」第 631 行仍指向 `~/main_20260831/`~~ → **同日 19:19 由（續）條目做完**
  （處置比原本記的更徹底：新增 §A0 日常啟動／收尾，§A4 標為 `main` 快照專用）。**本列同樣寫下即過期。**
  📌 **這兩列是「兩條線並行時，各自的待完成會互相過期」的實例** —— 機器線 20:12 寫下的待辦，
  GUI 線 19:19–19:21 就已經做完了。**收工前的待完成清單要先掃一次同日另一條線的條目。**
- 🔴 **手臂 `motor_api` 未啟動**，等 per user 確認手臂已裝回且周邊淨空（啟動會自動就緒＝可能動作）
- ⚪ 實機類待辦尚未動（`wall_mm` 三高度、`.22` 匯流排、console v2 真人瀏覽器 UAT）
...
### 📚 原廠資料進 `doc/`、補四份硬體摘要（09-07 收盤，per user 提供資料）

per user 放進 `tmp/` 的 12 個原廠資料夾（1.5 GB / 2832 檔），**正好補上四個
「有 driver 卻完全沒有手冊摘要」的缺口**。細節在 `changelog [2026-09-07m6]`，這裡只記現況與待辦。

- `doc/` 3 → **14 個裝置**（27 檔手冊）；`tmp/` 只剩廠商軟體包（可從原廠重下）
- `.claude/summaries/` **10 → 14 份**；`CLAUDE.md` 索引現為 **14/14 逐份對上**
  （原文寫 8 份與 9 份，兩處都不對；且 `MH300_..._SUMMARY.md` **自建立以來從未進過索引**）

### 🔴 比對手冊後發現的 driver 落差（新待辦，皆非急件）

- 🔴🔴 **`WT901BC_TTL` 的 `0x56` 氣壓路徑是死碼**：手冊無此封包格式定義、模組出廠不輸出它、
  應用層無人讀 `pressure`/`altitude`。而 `.h` 的使用範例還印 `imu.altitude`
  ⇒ **照著抄會得到恆為 0 的值**。要嘛刪掉、要嘛在 `.h` 標明未驗證。
- 🟡 `WT901BC_TTL` **不解析 `0x54` 磁場**（模組預設有送 ⇒ 約 1/4 頻寬空轉）、
  **不讀溫度**（`buf[8..9]`，IMU 有溫漂，這是免費診斷訊號）。
- 🟡 `WT901BC_TTL` 同步只靠 checksum、**沒驗封包類型**（約 1/256 誤同步）。
  加一行「`buf[1]` 落在 `0x50~0x5A`」幾乎免費。
- 🟡 **`DSZL_107` 這個 driver 名稱是誤導的** —— 它講的是 **X518 採集器**的協議，
  DSZL-107 本身是沒有數位介面的類比應變片。**改名成本高、風險低**，但至少該在 `.h` 標明。
- 🟡 **X518 有裝置端實物標定（`0x0A1E` + `0x0A20` 寫 11/12），本專案沒用**，
  改用上位機軟體係數。📌 **它剛好能回答既有待辦「單點校正、4.16 kg 外推到 30~60 kg 線性度未驗」**
  —— 裝置端可在工作範圍內直接加標準負荷標定，不必外推。⚠️ 要走**實物標定**不要走數字校準
  （手冊自己警告：靈敏度用理論值時數字校準結果不可信，而 2.0 mV/V 正是理論值）。
- 🟡 **XKC-Y25 恢復出廠會掉回 9600**，於是從 115200 的 `.22` 匯流排上**安靜消失**
  （其他裝置照常、只有它不回應）。與它 `init()` 「不探測」的現行行為疊起來，
  症狀會是「水位讀取突然開始失敗」而不是開機報錯。

### 🔴 兩件待 per user 確認
- **`編碼推杆 / DHSMS05.17.pdf`**：程式碼與所有文件裡**一個字都找不到**。
  per user 09-07 回覆「**舊硬體，先保存好**」⇒ 已收進 `doc/編碼推杆DHSMS05/`，不追查。
- **WT901BC 自己的手冊**：per user「晚點給」。拿到後請覆蓋
  `WT901BC_TTL_PROTOCOL_SUMMARY.md` 並移除該檔「來源是近親型號」那節但書。

### 🟡 `tmp/` 尚未處理（per user：最後處理）
1.5 GB 廠商軟體包，已 gitignore、不進版控、**不進雲端鏡像 ⇒ 零備份**。
📌 建議留著：`DM2J资料包.rar`／`MotionS.v2.zip`／`Witmotion.zip` 是原廠工具，重下要找對版本。
最沒有保存價值的是 `ZDT_步進電機/3. 说明书/X42S新增狂暴模式效果演示.mp4`（**110 MB 的展示影片**）。
