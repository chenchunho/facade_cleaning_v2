...
>
> 🔴 **2026-08-27 更新：暫緩的前提已經反轉，這份計畫需要重新評估。**
> 下方 §「開始前的前置決策」第 2 條寫著「現在 step 是 cup anchor mode，balance_hold 派不上用場」——
> 但 2026-07-22 起的**同步步伐 `do_step_sync_` 放繩期間四顆吸盤全放、完全靠鋼索承重、沒有任何錨定**，
> 且 GUI 已於 2026-08-26 全面改用同步步伐、交替走法下架。這正是該條所說「派不上用場」的反面，
> §「何時重啟這個計畫」第 4 條（「有 use case 真的需要機體靜止＋維持平衡」）**實質上已經成立**。
>
> 📌 新架構（`.claude/洗窗機器人設計彙整.md`）是四輪滾動＋螺旋槳貼牆＋鋼索垂吊，相關性只會更高。
> ⚠️ 但另一個重啟前提「DSZL 校好、可以信任張力讀值」**仍未達成**（見 `work_log.md` 待辦總表，🔴 未修）——
> 張力讀值不可信之前，balance hold 會建立在錯的輸入上。
