...
> ✅ **原始檔已於 2026-08-31 由 `tmp/SD76-C/` 搬到 `doc/計米器/SD76-C/`**（即本節 Source 標註的路徑）。
> 原因：`tmp/` 被排除在 gdrive 鏡像之外（harness 暫存 155M），放那裡不會進備份；
> `doc/` 雖然是 `.gitignore` 排除的（不進版控），但**不在鏡像排除清單裡 → 會被備份**。
> 一併保住的還有兩個子目錄：上位機軟體測試、Modbus Poll 測試說明。
