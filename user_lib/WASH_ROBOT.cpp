...
#if 0
std::string WashRobot::_retired_do_arm_clean_sweep_v1_(int wall_mm, int rounds) {
...
#endif  // _retired_do_arm_clean_sweep_v1_
...
#if 0  // [2026-07-31 per user] retired pre-break-vacuum two-stage retract — kept
       // for reference in case the break-vacuum-assisted single-stage retract
       // (current pusher_two_stage_retract_ above) needs to be rolled back.
       // Not compiled. To bring back: rename _retired_pusher_two_stage_retract_v1_
       // back to pusher_two_stage_retract_ (and rename/remove the current one).
//
// ---- Parameter values this version used (WASH_ROBOT.h) ----
//   PUSHER_RETRACT_PULSE      = 300    // 收腳目標 (~0.1cm, 兩版共用，沒變過)
//   PUSHER_RPM_RETRACT        = 150    // stage 1（慢脫壁）轉速
//   PUSHER_RPM_RETRACT_FULL   = 900    // stage 2（收到 0）轉速 — 新版沿用同名常數但改成 1000
//   PUSHER_ACC_RETRACT        = 255    // 兩版共用，沒變過
//   RETRACT_SLOW_PEEL_CM      = 1.0    // stage 1 慢脫壁距離
//   PUSHER_STAGE1_SAFETY_FACTOR = 1.0
//   PUSHER_STAGE1_DELAY_MS    = (RETRACT_SLOW_PEEL_CM / PUSHER_RETRACT_CM_PER_SEC)
//                               × PUSHER_STAGE1_SAFETY_FACTOR × 1000  ≈ 3896 ms
//                               （PUSHER_RETRACT_CM_PER_SEC 用 PUSHER_RPM_RETRACT
//                               算出來，≈1.54 cm/s @ 150rpm 那組換算）
//
// ---- Flow (per slave, then batched) ----
//   1. release_stall_flag() 全部 slave
//   2. 逐一讀目前位置分類：
//        - 讀取失敗或已經在 stage1_target（= preset_extend − RETRACT_SLOW_PEEL_CM
//          對應 pulse）以內 → 標記 stage=2，直接下 pos_mode_nowait(target=
//          PUSHER_RETRACT_PULSE, rpm=PUSHER_RPM_RETRACT_FULL, abs=1, sync=1)
//        - 否則 → 標記 stage=1，下 pos_mode_nowait(target=stage1_target,
//          rpm=PUSHER_RPM_RETRACT, abs=1, sync=1)
//        - cur_pulse < CUR_PULSE_FAULT_FLOOR(-1000) → 判定 encoder fault，abort
//   3. 單次 trigger_sync_move() 讓所有排隊的指令（stage1 跟 skip-to-stage2 混合）
//      同時觸發
//   4. 如果有任何 slave 是 stage1：sleep PUSHER_STAGE1_DELAY_MS（~3.9s，不 polling）
//   5. 針對 stage=1 的 slave，重新下 pos_mode_nowait(target=PUSHER_RETRACT_PULSE,
//      rpm=PUSHER_RPM_RETRACT_FULL, abs=1, sync=1) → 再一次 trigger_sync_move()
//   6. zdt_wait_motion_done_many_ 等全部到位（10s timeout）
//   7. Anti-FAKE-DONE：逐一讀 real_pos，超過 RETRACT_VERIFY_TOL_DEG(50°) 判定
//      FAKE-DONE（stall 但 wait_many 誤判完成）
//   8. sleep_ms_(PUSHER_SETTLE_MS)，return false
//
// 跟新版（破真空輔助單段直收）的差異：這版沒有任何 PQW 閥門動作（CH_BREAK_VACUUM
// 是後來才加的硬體），純粹靠「先慢慢剝一段距離，等吸附自然鬆脫，再快速收到底」，
// 沒有主動充氣破真空的機制。
bool WashRobot::_retired_pusher_two_stage_retract_v1_(const std::vector<int>& slaves) {
    if (slaves.empty()) return false;
...
#endif  // [2026-07-31] end retired pre-break-vacuum two-stage retract
...
std::string WashRobot::do_step_sync_(bool up) {
    ...
    if (!fails.empty()) {
        ...
        if (!right_ok || !left_ok) {
            ...
            // crane moves the OPPOSITE direction of the original step → vacuum
            // on + re-extend all 4 → recheck — until either a full per-side
            // seal is found or the whole original `cm` has been backed off
            // (fully back at the position we started this step from).
            std::string bad_side = (!right_ok && !left_ok) ? "both" : (!right_ok ? "right" : "left");
            std::cout << "[" << tag << "] " << bad_side << " unsealed after in-place retry"
                       << " — starting backoff retry (" << STEP_SYNC_BACKOFF_CM << "cm increments)\n";
            evt_(std::string(tag) + "_backoff_start side=" + bad_side);

            int backed_off_cm = 0;
            bool resealed = false;
            while (backed_off_cm < cm) {
                // [2026-07-27 fix] Not std::min(STEP_SYNC_BACKOFF_CM, ...) — that binds
                // the static constexpr member to std::min's const-reference parameter
                // (ODR-use), which needs an out-of-class definition pre-C++17 and was
                // linking as `undefined reference to WashRobot::STEP_SYNC_BACKOFF_CM`.
                // Plain comparison only needs its value, not its address.
                const int remaining_cm = cm - backed_off_cm;
                const int step_back = (remaining_cm < STEP_SYNC_BACKOFF_CM) ? remaining_cm : STEP_SYNC_BACKOFF_CM;

                // Full retract all 4 — currently unsealed, safe to retract.
                if (try_or_pause_([this]() { return vacuum_valve_("feet", false); },
                                   std::string(tag) + "_backoff_valve_off")) return fail("ERR aborted\n");
                if (check_abort_()) return fail("ERR aborted\n");
                if (try_or_pause_([this, &all_slaves]() { return vacuum_wait_release_(all_slaves, RETURN_VACUUM_RELEASE_MS); },
                                   std::string(tag) + "_backoff_vacuum_release")) return fail("ERR aborted\n");
                if (check_abort_()) return fail("ERR aborted\n");
                if (try_or_pause_([this, &all_slaves]() { return pusher_two_stage_retract_(all_slaves); },
                                   std::string(tag) + "_backoff_pusher_retract")) return fail("ERR aborted\n");
                if (check_abort_()) return fail("ERR aborted\n");

                // Crane moves back toward the starting point — opposite
                // direction from the original step move.
                {
                    std::ostringstream oss;
                    oss << (up ? "pay_out " : "retract ") << step_back;
                    const std::string cs = oss.str();
                    const int to = crane_motion_timeout_sec_(step_back);
                    std::cout << "[" << tag << "] backoff crane " << cs
                               << " (backed_off=" << backed_off_cm << "/" << cm << "cm)\n";
                    if (try_or_pause_([this, cs, to]() { return crane_cmd_(cs, to).rfind("OK", 0) != 0; },
                                       std::string(tag) + "_backoff_crane_move"))
                        return fail("ERR aborted\n");
                }
                if (check_abort_()) return fail("ERR aborted\n");
                backed_off_cm += step_back;

                // Vacuum on + re-extend all 4 together, same as the main attempt.
                if (try_or_pause_([this]() { return vacuum_valve_("feet", true); },
                                   std::string(tag) + "_backoff_valve_on")) return fail("ERR aborted\n");
                if (check_abort_()) return fail("ERR aborted\n");
                {
                    std::vector<int> stop_group_ids(all_slaves.size(), 0);
                    for (size_t i = 0; i < all_slaves.size(); ++i) {
                        const int s = all_slaves[i];
                        stop_group_ids[i] = (s == ZDT_RF1 || s == ZDT_RF2) ? 0 : 1;
                    }
                    if (try_or_pause_([this, &all_slaves, &stop_group_ids]() {
                            return smart_extend_subset_("all", all_slaves, /*stop_on_first_seal=*/true, &stop_group_ids);
                        }, std::string(tag) + "_backoff_extend_all")) return fail("ERR aborted\n");
                }
                if (check_abort_()) return fail("ERR aborted\n");

                right_unsealed.clear(); left_unsealed.clear();
                right_ok = group_seal_ok_("right", right_unsealed);
                left_ok  = group_seal_ok_("left",  left_unsealed);
                if (right_ok && left_ok) {
                    resealed = true;
                    std::cout << "[" << tag << "] backoff resealed after " << backed_off_cm << "cm\n";
                    evt_(std::string(tag) + "_backoff_resealed cm=" + std::to_string(backed_off_cm));
                    break;
                }
                std::cout << "[" << tag << "] backoff " << backed_off_cm << "/" << cm
                           << "cm — still unsealed (right_ok=" << right_ok << " left_ok=" << left_ok << ")\n";
            }

            if (!resealed) {
                std::string msg = std::string(tag) + "_side_unsealed_at_origin";
                std::cout << "[" << tag << "] " << msg << " — backed off full " << cm
                           << "cm, still unsealed, FAIL\n";
                evt_(msg);
...
std::string WashRobot::_retired_do_feet_realign_v1_(bool force, bool in_window) {
...
#if 0  // [v2 2026-07-08] retired v1 cmd_run continuous arm-sweep pipeline — kept
       // for reference until the arm is reinstalled. Not compiled. See plan
       // §"arm sweep 重新接回".
std::string WashRobot::_retired_cmd_run_v1_sweep_(int steps, int cm, const std::string& direction) {
    if (steps <= 0) return "ERR invalid_steps\n";
...
#endif  // [v2] end retired _retired_cmd_run_v1_sweep_
...
#if 0  // [v2 2026-07-08] retired v1 cmd_run_script arm-sweep pipeline — kept for
       // reference until the arm is reinstalled. Not compiled. See plan
       // §"arm sweep 重新接回".
std::string WashRobot::_retired_cmd_run_script_v1_sweep_(const std::string& csv) {
    std::vector<ScriptStep> steps;
    std::string perr;
    if (!parse_script_csv_(csv, steps, perr)) return "ERR " + perr + "\n";
    if (steps.empty())                        return "ERR empty_script\n";
...
#endif  // [v2] end retired _retired_cmd_run_script_v1_sweep_
...
std::string WashRobot::_retired_cmd_realign_v1_() {
    State cur = state_.load();
