...
DIHOOL_control::DIHOOL_control()
	: _client(nullptr), _ownedClient(false), debug_mode(false), _id(1), _total_motor(1)
{
	_log_tag = "DIHOOL:?";
}
...
bool DIHOOL_control::init(const std::string& ip, int port, int id, int total_motor, bool debug)
{
	...
	_ownedClient = true;
	_log_tag = "DIHOOL:" + std::to_string(id);

	_client = new TCP_client();
	if (!_client->connectToServer(ip, port))
	{
		LOG_ERR(_log_tag, "TCP connect failed %s:%d", ip.c_str(), port);
		delete _client;
		_client = nullptr;
		return false;
	}
...
bool DIHOOL_control::setMotorPosition_all(uint8_t mm)
{
	...
	for (int i = 1; i <= _total_motor; i++)
	{
		...
		char tx_tag[32];
		std::snprintf(tx_tag, sizeof(tx_tag), "TX ALL M%d", i);
		LOG_HEX(_log_tag, tx_tag, packet.data(), (int)packet.size());
		...
		{
			char rx_tag[32];
			std::snprintf(rx_tag, sizeof(rx_tag), "RX ALL M%d", i);
			LOG_HEX(_log_tag, rx_tag, buf, len);
		}
		else
		{
			LOG_WRN(_log_tag, "setMotorPosition_all M%d: no reply", i);
		}

		// avoid RS485 gateway congestion
		usleep(1000);
	}
...
//=========== control: set device ID ===========

bool DIHOOL_control::setDeviceID(uint8_t newID)
{
	std::vector<uint8_t> packet;

	packet.push_back(0x00);    // broadcast
	packet.push_back(0x06);
	packet.push_back(0x00);
	...
	if (len > 0)
	{
		LOG_HEX(_log_tag, "RX setDeviceID", buf, len);
	}
	else
	{
		LOG_WRN(_log_tag, "setDeviceID: no reply");
	}

	_id = newID;
	_log_tag = "DIHOOL:" + std::to_string(newID);
	return true;
}

//=========== utility: send command ===========
...
bool DIHOOL_control::sendRawPacket(const std::vector<uint8_t>& packet)
{
	...
	LOG_HEX(_log_tag, "TX", packet.data(), (int)packet.size());
