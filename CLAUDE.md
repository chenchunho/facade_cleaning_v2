...
> 🔴🔴 **2026-09-07：Visual Studio 已整組移除（per user）。**
> `.sln` + 4 個 `.vcxproj` + 4 個 `.vcxproj.user` 已從版控與磁碟刪除，`.vs/`（43 MB 快取）一併清掉。
> 需要時可從 git 歷史取回（刪除前的 commit：`79a2312`）。
>
> **移除的理由不是它壞，是它是第二份建置定義。** 查證過的實情：
> `ClCompile`（要編哪些 `.cpp`）**兩個主專案都正確**；錯的是
> `AdditionalIncludeDirectories` 漏了 `..\mechanism`／`..\config`，
> 以及 `ClInclude` 沒列 `rope_axis.h` ⇒ **VS 不會把它複製到遠端**，那才是遠端樹建不起來的直接原因。
> （⚠️ 我一度說「檔案清單也不完整」，**那句是錯的**，已更正。）

## Build System

**在 Pi 上用 g++ 直接編，沒有 CMake／Makefile／MSBuild。**
權威腳本在 **`scripts/build/`**，完整流程與三個隱藏步驟（同步／改名／停程式才換得動）
見 **`scripts/build/README.md`**。
...
```
  ...
  │   node server.js            :8081  console v2 │
...
| `8081` | `web/public_v2` | console v2（Dashboard / Mission / Manual / Setting） |

`server.js` 為此加了 `PUBLIC_DIR` 環境變數（預設值與先前逐字相同，不帶就是舊行為）。
🔴 **兩個行程各自持有自己的橋接連線** —— 一邊掛掉不影響另一邊，但也代表
**washrobot / crane 的命令埠會有兩條 client 連線**，查連線數時要記得這件事。
...
| 🆕 USR #2 | `192.168.1.21` | 本體 **PWM bus**（QX-DO24 獨佔）— 2026-09-03 新增 |
...
⚪ （歷史）v1 的 `.21` ZDT bus 已退役，08-28 實測當時不可達。
...
🆕 **`192.168.1.21` 的身世換過三次，讀舊註解時要注意日期**：
| 時期 | 狀態 |
|---|---|
| v1 | ZDT 推桿 bus |
| 2026-07 ~ 09-02 | **退役**（`app/WASH_ROBOT.h` 曾寫 `.21/cli_21_ retired`）。08-28 查 `.22` 間歇無回應時特地確認過它**不可達**，藉此排除「同一條匯流排上有第二個主站」的假設 |
| **2026-09-03 起** | **新網關、QX-DO24 獨佔**（per user）。設定已實查，與 `.22` 逐項相同 |

🔴 **08-28 那個「第二個主站」的假設要重新評估**：當時因為 `.21` 不可達而排除它。
現在 `.21` 活著了 —— **如果新網關的 A/B 不小心併到 `.22` 那對線上，那個假設就會成真**，
而症狀正好會是今天在查的「整條 `.22` 間歇性失效」。**接線時務必確認兩條是獨立的線對。**
...
Raspberry Pi 5（本體主控）
  ...
  └─ USR #3  192.168.1.22  (cli_22_)  ─── 「感測 bus」
        ...
        （DY-500 slave 10,11 鋼索重量感測 **2026-09-03 移除繫結**：從未安裝，
          driver 與 tier-2 程式碼路徑保留，但不再綁在這條匯流排上）
```
...
PQW 8CH 繼電器（本體 .20 slave 12）        權威：app/WASH_ROBOT.h:449-474
  ...
  └─ CH5  手臂滾筒刷馬達（2026-08-29 由 CH15 改回；15 是 2026-07-24 誤改，實機滾筒一直不轉）
```
...
**1. ~~`WASH_ROBOT.cpp` 有 3,879 行死碼~~ → ✅ 2026-08-29 已刪除**
18 個 `#if 0` 區塊、**3,993 行**（該檔 30%），最大一塊 898 行，全部是 v1 退役程式碼「留作參考」。
它們已經無法靠把 `#if 0` 改成 `#if 1` 復活——裡面引用的 `ZDT_LB1`／`ZDT_RB1`／`ZDT_C`
等符號**在 `WASH_ROBOT.h` 裡已經不存在了**（預處理器把它們吃掉才沒報錯），
所以只剩**閱讀**價值，而閱讀價值 git 歷史完整保留。
📌 **要看那些程式碼**：`git show <刪除 commit>^:app/WASH_ROBOT.cpp`。
🔴 **刪除是被證明過的 no-op，不是「看起來應該沒事」**：用 `cl /EP`（預處理但不輸出 `#line`）
比對刪除前後，去掉空行後**逐位元相同**（110,433 行 = 110,433 行），
且加一行探針確實會被抓到（負控制）。檔案 13,435 → 9,442 行。
