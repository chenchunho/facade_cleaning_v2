...
private:
    ...
    // 🔴 [2026-08-29 merge 6523b54] 3 → 1。**這不是把重試拿掉，是把它移到 driver 層。**
    //   對方在這一層加重試時，driver 還沒有重試；他們實機驗證過的行為是
    //   「最多 3 次交易、間隔 120ms」。我方 `9af86e4` 已在 QX_DO24::sendAndReceive
    //   加了 3 次重試 + 40ms backoff（理由：寫入失敗時模組**保持前一個輸出**，
    //   通訊斷掉螺旋槳不會停；左右螺旋槳共用 CH1，所以不能指望每個呼叫端都記得重試）。
    //   兩層都留 = 最壞 3×3 = 9 次交易 + 480ms 純睡眠，而這段位在 do_step_sync_
    //   的收腳／伸腳之前。合併乾淨但語意疊加，git 一個字都不會說。
    //   留 driver 層是因為它同時保護面板路徑（cmd_pwm_set），涵蓋面較廣。
    //   ⚠ 殘留差異：重試間隔 40ms 而非 120ms（總視窗 80ms vs 240ms）。
    //     若實機顯示掉包需要更長的讓路時間，調的是 QX_DO24 的 kBackoffMs，不是這裡。
    static constexpr int    PWM_STEP_WRITE_TRIES   = 1;    // 重試由 QX_DO24::sendAndReceive 負責
    static constexpr int    PWM_STEP_WRITE_RETRY_MS = 120; // 每次重試之間

    // ── [2026-08-29 merge 6523b54] 以上 PWM 常數整段取自 origin/main（實機驗證過的
    //    同步步伐 PWM 流程）。以下左右歸屬註解保留我方版本：對方那份是 08-27 改號
    //    「之前」的舊文字（right{1,2}/left{3,4}），他們沒有動到它。
    ...
    //    2026-09-02 稍後 per user 由 140 改為 **130** —— 是在跑過手動歸零流程
    //    （rail_enable off → 手推到左端硬限位 → rail_enable on → rail_zero）、
    //    零點確定落在左端硬限位之後才定的。先前的 140 是「零點起算尚可移動的距離」、
    //    不留餘裕；130 起實際保留約 10cm。
